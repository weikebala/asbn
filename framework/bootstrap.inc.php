...
if(!file_exists($configfile)) {
	if(file_exists(IA_ROOT . '/install.php')) {
		...
		echo "&nbsp;&nbsp;<a href='http://www.iseasoft.cn' style='font-size:12px' target='_blank'>Power by iseasoft &nbsp;V8 &nbsp;海软公众平台自助开源引擎</a> &nbsp; &nbsp;<a href='http://wpa.b.qq.com/cgi/wpa.php?ln=1&key=XzgwMDA4MzA3NV8zMDk2MDdfODAwMDgzMDc1XzJf' style='font-size:12px' target='_blank'>正版购买咨询</a>";
		exit();
	} else {
		header('Content-Type: text/html; charset=utf-8');
...
require $configfile;//加载数据库配置信息
require IA_ROOT . '/framework/version.inc.php';//加载版本信息
require IA_ROOT . '/framework/const.inc.php';//加载常用配置信息
require IA_ROOT . '/framework/class/loader.class.php';//加载加载类
load()->func('global');//加载函数库
load()->func('compat');
load()->func('pdo');
load()->classs('account');//加载类
load()->classs('agent');
load()->model('cache');//加载模型
load()->model('account');
load()->model('setting');
...
if(empty($_W['setting']['addons']['addons_url'])){
	$_W['setting']['addons']['addons_url'] = "http://addons.iseasoftcms.com";
}
if(empty($_W['setting']['addons']['c_url'])){
	$_W['setting']['addons']['c_url'] = "http://www.iseasoft.cn";
}
