...
if ($do == 'card') {
	...
	if(!empty($setting['fields'])) {
		...
		$reregister = 0;
		if(strlen($member_info['email']) == 39 && strexists($member_info['email'], '@iseasoft.cn')) {
			$member_info['email'] = '';
			$reregister = 1;
		}
	}
...
if($do == 'email') {
	...
	$reregister = false;
	if ($_W['member']['email'] == md5($_W['openid']).'@iseasoft.cn') {
		$reregister = true;
	}
	if(checksubmit('submit')) {
			...
			if ($reregister) {
				if (!empty($_GPC['email'])) {
					...
					if (($username_type == 'email' || $username_type == 'random') && preg_match(REGULAR_EMAIL, $username)) {
						...
						//当用户已填写重要资料,把用户@iseasoft.cn格式的邮箱重置为空,防止用户登陆反复提示重置资料
						$data['email'] = '';
					} else {
