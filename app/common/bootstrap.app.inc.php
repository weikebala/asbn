...
$_W['acid'] = $_W['uniaccount']['acid'];//公众号信息id
$isdel_account = pdo_get('account', array('isdeleted' => 1, 'acid' => $_W['acid']));
if (!empty($isdel_account)) {
	exit('指定公众号已被删除');
}
$_W['session_id'] = '';
if (isset($_GPC['state']) && !empty($_GPC['state']) && strexists($_GPC['state'], 'we7sid-')) {//state 存放session_id
...
if (!empty($_GPC['j'])) {
	...
	$_W['account'] = account_fetch($_W['acid']);//用户加盟店铺信息
...
$unisetting = uni_setting_load();//获取公众号设置信息
