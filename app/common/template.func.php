<?php
/**
 * [hairuan System] Copyright (c) 2014 iseasoft.cn
 * hairuan is NOT a free software, it under the license terms, visited http://www.iseasoft.cn/ for more details.
 */
...
function pctemplate_page($id, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$page = pdo_fetch("SELECT * FROM ".tablename('fournet_pcpage')." WHERE id = :id LIMIT 1", array(':id' => $id));
	if (empty($page)) {
		return error(1, 'Error: Page is not found');
	}
	if (empty($page['html'])) {
	...
	$compile = IA_ROOT . "/data/tpl/app/pc/{$id}.{$_W['template']}.tpl.php";
	$path = dirname($compile);
	if (!is_dir($path)) {
		load()->func('file');
		mkdirs($path);
	}
	$content = pctemplate_parse($page['html']);
	$content .= "<script type=\"text/javascript\" src=\"./resource/js/app/common.js\"></script>";
	file_put_contents($compile, $content);
	switch ($flag) {
		case TEMPLATE_DISPLAY:
		default:
			extract($GLOBALS, EXTR_SKIP);
			pctemplate('common/header');
			include $compile;
			pctemplate('common/footer');
			break;
		case TEMPLATE_FETCH:
			extract($GLOBALS, EXTR_SKIP);
			ob_clean();
...
function pctemplate($filename, $flag = TEMPLATE_DISPLAY) {
	global $_W, $_GPC;
	$source = IA_ROOT . "/app/pc/{$_W['template']}/{$filename}.html";
	$compile = IA_ROOT . "/data/tpl/app/pc/{$_W['template']}/{$filename}.tpl.php";
	if(!is_file($source)) {
		$compatFilename = template_compat($filename);
		if(!empty($compatFilename)) {
			return template($compatFilename, $flag);
	...
	if(!is_file($source)) {
		...
		$compile = IA_ROOT . "/data/tpl/app/pc/default/{$filename}.tpl.php";
	}
	...
	if(DEVELOPMENT || !is_file($compile) || filemtime($source) > filemtime($compile)) {
		pctemplate_compile($source, $compile);
	}
	switch ($flag) {
		case TEMPLATE_DISPLAY:
		default:
...
function pctemplate_compile($from, $to) {
	$path = dirname($to);
	if (!is_dir($path)) {
		load()->func('file');		
		mkdirs($path);
	}
	$content = pctemplate_parse(file_get_contents($from));
	file_put_contents($to, $content);
}
...
function pctemplate_addquote($matchs) {
	$code = "<?php {$matchs[1]}?>";
	$code = preg_replace('/\[([a-zA-Z0-9_\-\.\x7f-\xff]+)\](?![a-zA-Z0-9_\-\.\x7f-\xff\[\]]*[\'"])/s', "['$1']", $code);
	return str_replace('\\\"', '\"', $code);
}
...
function fournet_pcnavs($params = array()) {
	global $_W, $multi, $cid, $ishomepage;
	$condition = '';
	if(!$cid || !$ishomepage) {
		if (!empty($params['section'])) {
		...
		$navs = pdo_fetchall("SELECT id, name, description, url, icon, css, position, module FROM ".tablename('fournet_pcnav')." WHERE position = '1' AND status = 1 AND uniacid = '{$_W['uniacid']}' AND multiid = '{$multiid}' $condition ORDER BY displayorder DESC, id DESC");
	} else {
		$condition = " AND parentid = '".$cid."'";
		$navs = pdo_fetchall("SELECT * FROM ".tablename('site_category')." WHERE enabled = '1' AND uniacid = '{$_W['uniacid']}' $condition ORDER BY displayorder DESC, id DESC");
	}
...
function fournet_pcarticle($params = array()) {
	global $_GPC, $_W;
	extract($params);
	$pindex = max(1, intval($_GPC['page']));
	if (!isset($limit)) {
...
function fournet_pccategory($params = array()) {
	global $_GPC, $_W;
	extract($params);
	if (!isset($parentid)) {
		$condition = "";
...
function fournet_pcslide_search($params = array()) {
	global $_W;
	if(empty($params['limit'])) {
		$params['limit'] = 4;
	}
	...
	$sql = "SELECT * FROM " . tablename('fournet_pcslide') . " WHERE uniacid = '{$_W['uniacid']}' AND multiid = {$multiid} ORDER BY `displayorder` DESC, `id` DESC LIMIT " . intval($params['limit']);
	$list = pdo_fetchall($sql);
	if(!empty($list)) {
		foreach($list as &$row) {
			if (!strexists($row['url'], './')) {
...
function pc_slide($params = array()) {
	return fournet_pcslide_search($params);
}
...
function fournet_pcwidget_link($params = array()) {
	$widgetparams = json_decode($params['params'], true);
	$sql = 'SELECT * FROM ' .tablename('site_article')." WHERE uniacid = :uniacid ";
	$sqlparams = array(':uniacid' => $params['uniacid']);
	if (!empty($widgetparams['selectCate']['pid'])) {
...
function fournet_pcquickmenu() {
	global $_W, $_GPC;
	
	if ($_GPC['c'] == 'mc' || $_GPC['c'] == 'activity') {
		$quickmenu = pdo_fetch("SELECT html, params FROM ".tablename('fournet_pcpage')." WHERE uniacid = :uniacid AND type = '4' AND status = '1'", array(':uniacid' => $_W['uniacid']));
	} elseif ($_GPC['c'] == 'auth') {
		return false;
	} else {
		$multiid = intval($_GPC['t']);
		...
		$quickmenu = pdo_fetch("SELECT html, params FROM ".tablename('fournet_pcpage')." WHERE multiid = :multiid AND type = '2' AND status = '1'", array(':multiid' => $multiid));
	}
	if (empty($quickmenu)) {
		return false;
	}
