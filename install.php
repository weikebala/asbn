...
if($action == 'env') {
	...
	if(version_compare(PHP_VERSION, '5.3.0') == -1) {
		...
		$ret['php']['version']['remark'] = 'PHP版本必须为 5.3.0 以上. <a href="http://www.iseasoft.cn/">详情</a>';
	}
	...
	if($ret['php']['pdo']['ok']) {
		...
		if(!$ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['remark'] = '您的PHP环境虽然不支持 mysql_connect, 但已经支持了PDO, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://www.iseasoft.cn/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58073">详情</a>';
		}
	} else {
		if($ret['php']['mysql']['ok']) {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['pdo']['class'] = 'warning';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 但支持 mysql_connect, 这样系统虽然可以运行, 但还是建议你开启PDO以提升程序性能和系统稳定性. <a target="_blank" href="http://www.iseasoft.cn/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
		} else {
			$ret['php']['pdo']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['pdo']['class'] = 'danger';
			$ret['php']['pdo']['remark'] = '您的PHP环境不支持PDO, 也不支持 mysql_connect, 系统无法正常运行. <a target="_blank" href="http://www.iseasoft.cn/forum.php?mod=redirect&goto=findpost&ptid=3564&pid=58074">详情</a>';
	...
	if($ret['php']['curl']['ok']) {
		...
		if(!$ret['php']['fopen']['ok']) {
			$ret['php']['curl']['remark'] = '您的PHP环境虽然不支持 allow_url_fopen, 但已经支持了cURL, 这样系统是可以正常高效运行的, 不需要额外处理. <a target="_blank" href="http://www.iseasoft.cn/wiki/">详情</a>';
		}
	} else {
		if($ret['php']['fopen']['ok']) {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-warning"></span>';
			$ret['php']['curl']['class'] = 'warning';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 但支持 allow_url_fopen, 这样系统虽然可以运行, 但还是建议你开启cURL以提升程序性能和系统稳定性. <a target="_blank" href="http://www.iseasoft.cn/wiki/">详情</a>';
		} else {
			$ret['php']['curl']['value'] = '<span class="glyphicon glyphicon-remove text-danger"></span>';
			$ret['php']['curl']['class'] = 'danger';
			$ret['php']['curl']['remark'] = '您的PHP环境不支持cURL, 也不支持 allow_url_fopen, 系统无法正常运行. <a target="_blank" href="http://www.iseasoft.cn/wiki/">详情</a>';
	...
	if($ret['php']['ssl']['ok']) {
		...
		$ret['php']['ssl']['remark'] = '没有启用OpenSSL, 将无法访问公众平台的接口, 系统无法正常运行. <a target="_blank" href="http://www.iseasoft.cn/wiki/">详情</a>';
	}
	...
	if($ret['php']['gd']['ok']) {
		...
		$ret['php']['gd']['remark'] = '没有启用GD, 将无法正常上传和压缩图片, 系统无法正常运行. <a target="_blank" href="http://www.iseasoft.cn/wiki/">详情</a>';
	}
	...
	if($ret['php']['dom']['ok']) {
		...
		$ret['php']['dom']['remark'] = '没有启用DOMDocument, 将无法正常安装使用模块, 系统无法正常运行. <a target="_blank" href="http://www.iseasoft.cn/wiki/">详情</a>';
	}
	...
	if($ret['write']['root']['ok']) {
		...
		$ret['write']['root']['remark'] = '本地目录无法写入, 将无法使用自动更新功能, 系统无法正常运行.  <a href="http://www.iseasoft.cn/">详情</a>';
	}
	...
	if($ispost) {
		...
		if(empty($error)) {
			...
			if($_POST['type'] == 'remote') {
				...
				if($ins['error']) {
					die('<script type="text/javascript">alert("链接海软更新服务器失败, 错误为: ' . $ins['error'] . '！");history.back();</script>');
				}
				$archive = $ins['files'];
				if(!$archive) {
 ...
 * 版本号
 *
 * [iseasoft System] Copyright (c) 2013 iseasoft.cn
 */
...
VER;
			...
			if(file_exists(IA_ROOT . '/index.php') && is_dir(IA_ROOT . '/app') && is_dir(IA_ROOT . '/web') && file_exists($verfile) && file_exists($dbfile)) {
				...
				die('<script type="text/javascript">alert("你正在使用本地安装, 但未下载完整安装包, 请从海软官网下载完整安装包后重试.");history.back();</script>');
			}
...
function remote_install() {
	...
	$pars['version'] = '0.6';
	$pars['release'] = '';
	$url = 'http://www.iseasoft.cn/gateway.php';
	$ch = curl_init($url);
...
<html lang="zh-cn">
	<head>
		...
		<title>安装系统 - 海软 - 公众平台自助开源引擎</title>
		<link rel="stylesheet" href="http://www.iseasoft.cn/bootstrap/3.2.0/css/bootstrap.min.css">
		<style>
		...
		<!--[if lt IE 9]>
		  <script src="http://www.iseasoft.cn/html5shiv/3.7.2/html5shiv.min.js"></script>
		  <script src="http://www.iseasoft.cn/respond.js/1.4.2/respond.min.js"></script>
		<![endif]-->
	</head>
		...
		<div class="container">
			<div class="header" style="margin:15px auto;">
				<ul class="nav nav-pills pull-right" role="tablist">
					<li role="presentation" class="active"><a href="javascript:;">安装海软系统</a></li>
					<li role="presentation"><a href="http://www.iseasoft.cn">海软官网</a></li>
					<li role="presentation"><a href="http://bbs.iseasoft.cn">访问论坛</a></li>
				</ul>
				<img src="http://www.iseasoft.cn/static/image/common/logo.png" />
			</div>
			...
			<div class="footer" style="margin:15px auto;">
				<div class="text-center">
					<a href="http://www.iseasoft.cn">关于海软</a> &nbsp; &nbsp; <a href="http://bbs.iseasoft.cn">海软帮助</a> &nbsp; &nbsp; <a href="http://www.iseasoft.cn">购买授权</a>
				</div>
				<div class="text-center">
					Powered by <a href="http://www.iseasoft.cn"><b>海软</b></a> v0.6 &copy; 2014 <a href="http://www.iseasoft.cn">www.iseasoft.cn</a>
				</div>
			</div>
		</div>
		<script src="http://www.iseasoft.cn/jquery/1.11.1/jquery.min.js"></script>
		<script src="http://www.iseasoft.cn/bootstrap/3.2.0/js/bootstrap.min.js"></script>
	</body>
</html>
EOF;
	echo trim($tpl);
	...
	echo <<<EOF
		<div class="panel panel-default">
			...
			<div class="panel-body" style="overflow-y:scroll;max-height:400px;line-height:20px;">
				<h3>版权所有 (c)2014-2015，海软WZ团队保留所有权利。 </h3>
				<p>
					感谢您选择海软WZ - 微信公众平台自助开源引擎（以下简称iseasoft，iseasoft基于 PHP + MySQL的技术开发，全部源码开放。 <br />
					为了使你正确并合法的使用本软件，请你在使用前务必阅读清楚下面的协议条款：
				</p>
				<p>
					<strong>一、本授权协议适用且仅适用于海软系统(iseasoft. 以下简称海软)任何版本，海软官方对本授权协议的最终解释权。</strong>
				</p>
				<p>
					<strong>二、协议许可的权利 </strong>
					<ol>
						<li>您可以在完全遵守本最终用户授权协议的基础上，将本软件应用于非商业用途，而不必支付软件版权授权费用。</li>
						<li>您可以在协议规定的约束和限制范围内修改海软源代码或界面风格以适应您的网站要求。</li>
				...
				<p>
					...
					<ol>
						...
						<li>未经官方许可，禁止在海软的整体或任何部分基础上以发展任何派生版本、修改版本或第三方版本用于重新分发。</li>
				...
				<p>
					...
					<ol>
						...
						<li>电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始确认本协议并安装  iseasoft，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。</li>
...
function tpl_install_env($ret = array()) {
	...
	echo <<<EOF
		...
		<div class="alert alert-info">系统要求海软整个安装目录必须可写, 才能使用海软所有功能。</div>
		<div class="panel panel-default">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<div class="help-block">
							在线安装能够直接安装最新版本海软系统, 如果在线安装困难, 请下载离线安装包后使用本地安装. <br/>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input class="form-control" type="text" name="db[name]" value="iseasoft">
					</div>
				</div>
			</div>
		</div>
...
function tpl_install_finish() {
	...
	<div class="alert alert-success">
		恭喜您!已成功安装“海软 - 公众平台自助开源引擎”系统，您现在可以: <a target="_blank" class="btn btn-success" href="./web/index.php">访问网站首页</a>
		<br />
	</div>
	<div class="alert alert-warning">
		我们强烈建议您立即注册云服务，享受“在线更新”等云服务。<a target="_blank" class="btn btn-success" href="http://www.iseasoft.cn/">马上去注册</a>
		<br />
	</div>
EOF;
	tpl_frame();
...
function tpl_resources() {
	static $res = array(
		'logo' => 'http://www.iseasoft.cn/static/image/common/logo.png',
	);
	return $res;
}
