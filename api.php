...
require_once './addons/sea/core/inc/functions.php';
require_once './addons/sea/core/inc/plugin/plugin_model.php';
define("sea_CORE",'./addons/sea/core/');
define("sea_PLUGIN",'./addons/sea/plugin/');
$hash = $_GPC['hash'];
...
class hairuan {
	...
	public function start() {
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
			...
			$this->booking($message);//粉丝关注/取消关注日志 以及用户数据操作 关注送优惠券
			...
			if ($message['event'] == 'unsubscribe') {
				...
				if (empty($member)) {
					if (!isset($setting['passport']) || empty($setting['passport']['focusreg'])) {
						$data = array(
							'uniacid' => $_W['uniacid'],
							'email' => md5($message['from']).'@iseasoft.cn',
							'salt' => random(8),
                        ...
                        //第一次关注送优惠券
                        if (p('coupon')) {
                            $coupon_plugin = p('coupon');//优惠券插件类
                            $sendres = $coupon_plugin->sendRegCoupon($message['from'],$_W['uniacid']);
                            if($sendres) {
                                $couponres['is_get_regcoupon'] = 1;
                                pdo_update('mc_members', $couponres, array(
                                    'uid' => $rec['uid'],
                                ));
                            }
                        }
