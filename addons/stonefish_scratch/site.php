...
class stonefish_scratchModuleSite extends WeModuleSite {	
	...
	function Weixin(){
		...
		if($setting['stonefish_scratch_jssdk']==2 && !empty($setting['jssdk_appid']) && !empty($setting['jssdk_secret'])){
	...
	public function Seed_tmplmsg($openid,$tmplmsgid,$rid,$params) {
        global $_W;
		$reply = pdo_fetch("select title,starttime,endtime FROM ".tablename("stonefish_scratch_reply")." where rid = :rid", array(':rid' => $rid));
		$exchange = pdo_fetch("select awardingstarttime,awardingendtime FROM ".tablename("stonefish_scratch_exchange")." where rid = :rid", array(':rid' => $rid));
		$listtotal = pdo_fetchcolumn("select xuninum+fansnum as total from ".tablename("stonefish_scratch_reply")." where rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
		$tmplmsg = pdo_fetch("select * FROM ".tablename("stonefish_scratch_tmplmsg")." where id = :id", array(':id' => $tmplmsgid));
		$fans = pdo_fetch("select * FROM ".tablename("stonefish_scratch_fans")." where rid = :rid and from_user = :from_user", array(':rid' => $rid, ':from_user' => $openid));
		...
		if(!empty($tmplmsg)){
			...
			pdo_insert('stonefish_scratch_fanstmplmsg', $tmplmsgdata);
			//添加消息发送记录
			return true;
		}
		return;
	...
	function Xuni_time($reply){
		...
		if($now-$reply['xuninum_time']>$reply['xuninumtime']){
		    pdo_update('stonefish_scratch_reply', array('xuninum_time' => $now,'xuninum' => $reply['xuninum']+mt_rand($reply['xuninuminitial'],$reply['xuninumending'])), array('id' => $reply['id']));
		}
	}
	//虚拟人数据配置
	//分享设置
	function Get_share($rid,$from_user,$title,$iid) {
		...
		if (!empty($iid)) {
		    $prize = pdo_fetch("select prizename FROM ".tablename("stonefish_scratch_prize")." where id = :id", array(':id' => $iid));
			$prizename = $prize['prizename'];
		}
		if (!empty($from_user)) {
		    $fans = pdo_fetch("select realname,nickname FROM ".tablename("stonefish_scratch_fans")." where uniacid= :uniacid AND rid= :rid AND from_user= :from_user", array(':uniacid' => $uniacid,':rid' => $rid,':from_user' => $from_user));
			if (empty($iid)) {
				$prizeid = pdo_fetchcolumn("select prizeid FROM ".tablename("stonefish_scratch_fansaward")." where zhongjiang>=1 and uniacid= :uniacid AND rid= :rid AND from_user= :from_user ORDER BY RAND() LIMIT 1", array(':uniacid' => $uniacid,':rid' => $rid,':from_user' => $from_user));
				if(!empty($prizeid)){
					$prize = pdo_fetch("select prizename FROM ".tablename("stonefish_scratch_prize")." where id = :id", array(':id' => $prizeid));
			        $prizename = $prize['prizename'];
				}
			}
		}
	...
	function Get_prizename($rid,$title,$id) {
		...
		$uniacid = $_W['uniacid'];
		if($id){
			$prizename = pdo_fetchcolumn("select prizename FROM ".tablename("stonefish_scratch_prize")." where id = :id", array(':uniacid' => $uniacid,':rid' => $rid));
		}else{
			$prizename = pdo_fetchcolumn("select prizename FROM ".tablename("stonefish_scratch_prize")." AS t1 JOIN (select ROUND(RAND() * (select MAX(id) FROM ".tablename("stonefish_scratch_prize").")) AS id) AS t2 where t1.uniacid= :uniacid AND t1.rid= :rid and t1.id >= t2.id", array(':uniacid' => $uniacid,':rid' => $rid));
		}		
	...
	function Get_openid($rid) {   
		...
		$setting = $this->module['config'];
		if($_W['account']['level']<4 && $setting['stonefish_scratch_oauth']==1){
			$from_user['openid'] = $_SESSION['oauth_acid'];
		}
		if($_W['account']['level']<4 && $setting['stonefish_scratch_oauth']==2){
    ...
    public function doMobileauth2(){
        ...
        if ($entrytype == "index") { // 粉丝参与活动
            ...
            $appUrl=$this->createMobileUrl('shareview', array('rid' => $rid,"fromuser" => $_GPC['from_user'],"uid" => $uid),true);
			$appUrl=substr($appUrl,2);
			$url = $_W['siteroot'] ."app/".$appUrl;
        }
        header("location: $url");
    ...
    public function getAuthTokenInfo($code){
		...
		if ($_W['account']['level']==4){
			...
			$setting = $this->module['config'];
			if($setting['stonefish_scratch_oauth']==1 && !empty($_W['oauth_account']['key']) && !empty($_W['oauth_account']['secret'])){
				$appid = $_W['oauth_account']['key'];
                $secret = $_W['oauth_account']['secret'];
			}
			if($setting['stonefish_scratch_oauth']==2 && !empty($setting['appid']) && !empty($setting['secret'])){
				$appid = $setting['appid'];
	...
	public function Gethomeurl(){
		...
		$time = time();
		$urls = array();
		$list = pdo_fetchall("select rid, title FROM ".tablename('stonefish_scratch_reply')." where uniacid = :uniacid and starttime <= :time and endtime >= :time and isshow=1", array('uniacid' => $uniacid,'time' => $time));
		if(!empty($list)){
			foreach($list as $row){
	...
	public function doMobileListentry() {
		...
		$cover_reply = pdo_fetch("select * FROM ".tablename("cover_reply")." where uniacid = :uniacid and module = :module", array(':uniacid' => $uniacid, ':module' => 'stonefish_scratch'));
		//活动列表
		$reply = pdo_fetchall("select * FROM ".tablename("stonefish_scratch_reply")." where uniacid = :uniacid and isshow = 1 and starttime <= :time  and endtime >= :time ORDER BY `endtime` DESC", array(':uniacid' => $uniacid, ':time' => $time));
		foreach ($reply as $mid => $replys) {
			$reply[$mid]['num'] = pdo_fetchcolumn("select COUNT(id) FROM ".tablename("stonefish_scratch_fans")." where uniacid = :uniacid and rid = :rid and status=1", array(':uniacid' => $uniacid, ':rid' => $replys['rid']));
			$reply[$mid]['is'] = pdo_fetchcolumn("select COUNT(id) FROM ".tablename("stonefish_scratch_fans")." where uniacid = :uniacid and rid = :rid and from_user = :from_user and status=1", array(':uniacid' => $uniacid, ':rid' => $replys['rid'], ':from_user' => $from_user));
			$reply[$mid]['start_picurl'] = toimage($replys['start_picurl']);
		}
		//活动列表
		//查询参与情况
		$usernum = pdo_fetchcolumn("select COUNT(id) FROM ".tablename("stonefish_scratch_fans")." where uniacid = :uniacid and from_user = :from_user and status=1", array(':uniacid' => $uniacid, ':from_user' => $from_user));
		//查询参与情况
		if($this->Weixin()){
	...
	public function doMobileEntry() {
		...
		if($reply['homepictime']>0){
			...
			if((empty($_COOKIE['stonefish_scratch_hometime'.$rid]) || $_COOKIE["stonefish_scratch_hometime".$rid]<=time()) && $_GPC['homepic']!="yes"){
				switch ($reply['homepictype']){
				    case 2:
				        setcookie("stonefish_scratch_hometime".$rid, strtotime(date("Y-m-d",strtotime("+1 day"))), strtotime(date("Y-m-d",strtotime("+1 day"))));
				        break;
					case 3:
				        setcookie("stonefish_scratch_hometime".$rid, strtotime(date("Y-m-d",strtotime("+1 week"))), strtotime(date("Y-m-d",strtotime("+7 week"))));
				        break;
					case 4:
				        setcookie("stonefish_scratch_hometime".$rid, strtotime(date("Y-m-d",strtotime("+1 year"))), strtotime(date("Y-m-d",strtotime("+1 year"))));
				        break;
				}
				include $this->template('homepictime');
				exit;
		...
		if($_W['account']['level']==4){
			...
			if($token['subscribe']==1){
				...
				$appUrl=$this->createMobileUrl($entrytype, array('rid' => $rid,'fromuser' => $_GPC['from_user'],'uid' => $fans['uid']),true);
			    $appUrl=substr($appUrl,2);
			    $url = $_W['siteroot'] ."app/".$appUrl;
			    header("location: $url");
		        exit;
			...
			if($reply['power']==2){
				$appUrl= $this->createMobileUrl('auth2', array('entrytype' => $entrytype,'rid' => $rid,'from_user' => $_GPC['from_user'],'uid' => -1),true);
		        $appUrl = substr($appUrl,2);
		...
		if(empty($access_token)){
		    $setting = $this->module['config'];
		    //不是认证号又没有借用服务号获取头像昵称
		    if($setting['stonefish_scratch_oauth']==0){
				if (!isset($_COOKIE["user_oauth2_wuopenid".$rid]) && $_W['account']['level']!=4){
				   	//设置cookie信息
			    	setcookie("user_oauth2_wuopenid".$rid, time(), time()+3600*24*7);
			   	}
			    $appUrl=$this->createMobileUrl($entrytype, array('rid' => $rid,'fromuser' => $_GPC['from_user'],'uid' => 0),true);
			   	$appUrl=substr($appUrl,2);
			   	$url = $_W['siteroot'] ."app/".$appUrl;
			    header("location: $url");
		        exit;
		    ...
		    //不是认证号又没有借用服务号获取头像昵称	
		    //不是认证号 借用服务号获取头像昵称
            if ($setting['stonefish_scratch_oauth']==1 && !empty($_W['oauth_account']['key']) && !empty($_W['oauth_account']['secret'])) { // 判断是否是借用设置
                $appid = $_W['oauth_account']['key'];
                $secret = $_W['oauth_account']['secret'];
            }
			if ($setting['stonefish_scratch_oauth']==2 && !empty($setting['appid']) && ! empty($setting['secret'])) { // 判断是否是借用设置
                $appid = $setting['appid'];
                $secret = $setting['secret'];
            }
		    $appUrl= $this->createMobileUrl('auth2', array('entrytype' => $entrytype,'rid' => $rid,'from_user' => $_GPC['from_user'],'uid' => -1),true);
			...
			$appUrl=$this->createMobileUrl($entrytype, array('rid' => $rid,'fromuser' => $_GPC['from_user'],'uid' => -1),true);
			$appUrl=substr($appUrl,2);
			$url = $_W['siteroot'] ."app/".$appUrl;
			header("location: $url");
		    exit;
	...
	public function doMobileShareview() {
		...
		//获取openid
		$reply = pdo_fetch("select * from " . tablename('stonefish_scratch_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
		$template = pdo_fetch("select * from " . tablename('stonefish_scratch_template') . " where id = :id", array(':id' => $reply['templateid']));
		...
		if(!empty($fromuser)) {
			//参与分享人信息
		    $fans = pdo_fetch("select * from ".tablename('stonefish_scratch_fans')." where rid = :rid and uniacid = :uniacid and from_user= :from_user", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $fromuser));
		    if(!empty($fans)){
			    ...
			    $this->message_tips('抱歉，您的朋友没有参与本活动！请告之你的朋友，3秒后自动进入活动页！',url('entry//index',array('m'=>'stonefish_scratch','rid'=>$rid)),3);
		    }
			if($uid>=1){
				...
				$firend['headimgurl'] = '../addons/stonefish_scratch/template/images/avatar.jpg';
		...
		if($from_user!=$fromuser){
			//是否开启互助模式
			if($reply['helptype'] == 1){
				$helpcount = pdo_fetchcolumn("select count(*) FROM " . tablename('stonefish_scratch_sharedata') . " where rid=:rid and fromuser=:fromuser and from_user=:from_user", array(':rid' => $rid,":fromuser" => $from_user,":from_user" => $fromuser));
				if($helpcount){
			...
			//是否开启互助模式
			$firendhelpcount = pdo_fetchcolumn("select count(*) FROM " . tablename('stonefish_scratch_sharedata') . " where rid=:rid and fromuser=:fromuser and from_user=:from_user", array(':rid' => $rid,":fromuser" => $fromuser,":from_user" => $from_user));
			$firendfelp = pdo_fetch("select id,viewnum FROM " . tablename('stonefish_scratch_sharedata') . " where rid=:rid and fromuser=:fromuser and from_user=:from_user order by visitorstime desc", array(':rid' => $rid,":fromuser" => $fromuser,":from_user" => $from_user));
			if ($reply['limittype'] == 1) { // 每天限制                
                $dayfirendhelp = pdo_fetchcolumn("select count(*) FROM " . tablename('stonefish_scratch_sharedata') . " where rid=:rid and fromuser=:fromuser and from_user=:from_user and visitorstime >= :starttime", array(':rid' => $rid,":fromuser" => $fromuser,":from_user" => $from_user,":starttime" => strtotime(date('Y-m-d'))));
            }
            ...
            if ($reply['powertype']==0 && $res["code"] == '') {
				...
				pdo_insert('stonefish_scratch_sharedata', $insert); // 记录助力人
            }else{
				pdo_update('stonefish_scratch_sharedata', array('viewnum' => $firendfelp['viewnum'] + 1), array('id' => $firendfelp['id']));
			}
			//记录分享
			//增加分享人分享量
			$sharenum = pdo_fetchcolumn("select count(id) from ".tablename('stonefish_scratch_sharedata')." where uniacid= :uniacid and fromuser= :fromuser and rid= :rid", array(':uniacid' => $uniacid,':rid' => $rid,':fromuser' => $fromuser));
			$sharepoint = pdo_fetchcolumn("select sum(point) from ".tablename('stonefish_scratch_sharedata')." where uniacid= :uniacid and fromuser= :fromuser and rid= :rid", array(':uniacid' => $uniacid,':rid' => $rid,':fromuser' => $fromuser));
			pdo_update('stonefish_scratch_fans', array('sharenum' => $sharenum,'sharepoint' => $sharepoint), array('uniacid' => $uniacid,'from_user' => $fromuser,'rid' => $rid));
			//增加分享人分享量
	...
	public function doMobileFirendhelp(){
        ...
        if(!empty($reply)){
			...
			//查询是否互助
			if($reply['helptype']){
				$firendtohelp = pdo_fetchcolumn("select count(*) FROM " . tablename('stonefish_scratch_sharedata') . " where rid=:rid and fromuser=:fromuser and from_user=:from_user", array(':rid' => $rid,":fromuser" => $from_user,":from_user" => $fromuser));
				if($firendtohelp){
					$res["msg"] = "禁止互助!";
                    $res["code"] = 505;
				}
	...
	public function doMobileindex() {
        ...
        $reply = pdo_fetch("select * from " . tablename('stonefish_scratch_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
		$exchange = pdo_fetch("select * FROM ".tablename("stonefish_scratch_exchange")." where rid = :rid", array(':rid' => $rid));
		$template = pdo_fetch("select * from " . tablename('stonefish_scratch_template') . " where id = :id", array(':id' => $reply['templateid']));
		$share = pdo_fetch("select * from " . tablename('stonefish_scratch_share') . " where rid = :rid and acid = :acid", array(':rid' => $rid,':acid' => $acid));
        //活动状态
		$this->check_reply($reply);
		...
		if(empty($from_user)) {
			...
			if($reply['issubscribe']>=2){
				...
				if(empty($members)) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','url'=>url('entry//index',array('m'=>'stonefish_scratch','rid'=>$rid)))),3);
				}
				if($members['status']==0) {
				    $this->message_tips('会员已被锁定，请联系管理员');
				}
				if($reply['issubscribe']==3 && empty($mobile['mobile'])) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'mobile','url'=>url('entry//index',array('m'=>'stonefish_scratch','rid'=>$rid)))),3);
				}
				if($reply['issubscribe']==4 && empty($members['groupid'])) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'groupid','url'=>url('entry//index',array('m'=>'stonefish_scratch','rid'=>$rid)))),3);
				}
				if($reply['issubscribe']==5 && empty($members['districtid'])) {
				    $this->message_tips('请先验证成为会员才能参与活动！',url('entry//member',array('m'=>'stonefish_member','fstatus'=>'districtid','url'=>url('entry//index',array('m'=>'stonefish_scratch','rid'=>$rid)))),3);
				}
			}
			//验证是否为会员 
			if($uid>=1){
		...
		//获得用户资料
		//查询是否参与活动并更新头像和昵称
		$fans = pdo_fetch("select * from ".tablename('stonefish_scratch_fans')." where rid = :rid and uniacid = :uniacid and from_user= :from_user", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
		if(!empty($fans)){
			if($fans['status']==0){
				$realname = empty($fans['realname']) ? stripcslashes($fans['nickname']) : $fans['realname'];
			...
			//更新分享量
			$fans['sharenum'] = pdo_fetchcolumn("select count(id) FROM ".tablename('stonefish_scratch_sharedata')." where uniacid = :uniacid and rid = :rid and fromuser = :from_user", array(':uniacid' => $uniacid,':rid' => $rid,':from_user' => $from_user));
			pdo_update('stonefish_scratch_fans', array('sharenum' => $fans['sharenum']), array('id' => $fans['id']));
			//更新分享量
			//更新头像和昵称
			if($reply['power']==2){
				pdo_update('stonefish_scratch_fans', array('avatar' => $firend['headimgurl'], 'nickname' => $firend['nickname']), array('id' => $fans['id']));
			}
			//更新头像和昵称
			//查询是否需要弹出填写兑奖资料
		...
		if($reply['opportunity']==1){
			...
			if(empty($profile['mobile'])){
				$this->message_tips('还没有注册成为会员，无法进入刮奖', url('entry//member',array('m'=>'stonefish_member','url'=>url('entry//index',array('m'=>'stonefish_scratch','rid'=>$rid)))),3);
			}
			...
			if(!empty($doings)){
				if ($doings['status']<2) {
					$this->message_tips('抱歉，您的刮奖资格正在审核中');
                }else{
					if($doings['awardcount'] == 0){
					    $this->message_tips('抱歉，您的刮奖次数已用完了或没有获得刮奖次数');
					}else{
				...
				$this->message_tips('抱歉，您的还未获得刮奖资格');
			}
			...
			switch ($share['sharetimes']){
				case 1://赠送当天
				    if($share['sharetype']){//分享成功打开链接后
					    $sharenum = pdo_fetchcolumn("select count(id) as dd from ".tablename('stonefish_scratch_sharedata')." where uniacid='".$uniacid."' and rid= '".$rid."' and fromuser='".$from_user."'");
		...
		//更新当日次数
		//查询次数并替换变量
		$replystr = pdo_fetch("select number_times,day_number_times from " . tablename('stonefish_scratch_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
		...
		if($reply['viewawardnum']){
			$fansaward = pdo_fetchall("select * from ".tablename('stonefish_scratch_fansaward')." where rid = :rid and uniacid = :uniacid and zhongjiang>=1 group by from_user,prizeid order by createtime desc limit ".$reply['viewawardnum'], array(':rid' => $rid, ':uniacid' => $uniacid));
			foreach ($fansaward as $fansawards){
				$prizex = pdo_fetchcolumn("select count(id) from ".tablename('stonefish_scratch_fansaward')." where rid = :rid and uniacid = :uniacid and zhongjiang>=1 and from_user =:from_user and prizeid = :prizeid", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $fansawards['from_user'], ':prizeid' => $fansawards['prizeid']));
				$prizename = pdo_fetchcolumn("select prizename from " . tablename('stonefish_scratch_prize') . " where id='" . $fansawards['prizeid'] . "'");
				$awardx = '';
				if($prizex==1){
					$awardx .= $prizename.'&nbsp;&nbsp;';
				}else{
				...
				$fansinfo = pdo_fetch("select realname,nickname from ".tablename('stonefish_scratch_fans')." where rid = :rid and uniacid = :uniacid and from_user =:from_user", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $fansawards['from_user']));
	...
	public function doMobileget_award() {
        ...
        $fans = pdo_fetch("select * from " . tablename('stonefish_scratch_fans') . " where rid = :rid and from_user = :from_user", array(':rid' => $rid, ':from_user' => $from_user));
        if($fans == false){
            $fans = array(
                'rid' => $rid,
				'uniacid' => $uniacid,
			...
			//发送消息模板之参与模板
			//增加人数，和浏览次数
            pdo_update('stonefish_scratch_reply', array('fansnum' => $reply['fansnum'] + 1), array('id' => $reply['id']));
        }else{
			if($fans['awardnum']>=$reply['award_num']&&$reply['award_num']!=0){				
		...
		if($reply['opportunity']==1){
			...
			if(!empty($doings)){
				...
				$this->Json_encode(array("success"=>2, "msg"=>'抱歉，您还没有获取参与资格，不能刮奖!'));
			}
		}elseif($reply['opportunity']==2){			
		    $unisettings = uni_setting($uniacid, array('creditnames'));
		...
		foreach ($gift as $row) {
			...
			$prizeren = 0;
			//查询此奖品今日中奖数量
			$prizeday = pdo_fetchcolumn("select count(id) FROM ".tablename('stonefish_scratch_fansaward') ." where rid = :rid and uniacid=:uniacid and prizeid = :prizeid and createtime >= :daytime",array(":rid"=>$rid,":uniacid"=>$uniacid,":prizeid"=>$row['id'],":daytime"=>$nowtime));
			if($row['prizeday']==0 || $row['prizeday']>$prizeday){
				$prize_day = 1;
			}
			//查询此奖品今日中奖数量
			//查询此奖品每人中奖数量
			$prizeren = pdo_fetchcolumn("select count(id) FROM ".tablename('stonefish_scratch_fansaward') ." where from_user=:from_user and rid = :rid and uniacid=:uniacid and prizeid = :prizeid",array(":from_user"=>$from_user,":rid"=>$rid,":uniacid"=>$uniacid,":prizeid"=>$row['id']));
		...
		if($isgift && ($fans['awardnum']<$reply['award_num'] || $reply['award_num']==0)){
            //参与次数
            pdo_update('stonefish_scratch_fans', array('totalnum' => $fans['totalnum'] + 1,'todaynum' => $fans['todaynum'] + 1,'lasttime'=>$nowtime), array('id' => $fans['id']));
			//查询是否为分享赠送次数
			$sharereply = pdo_fetch("select number_times,day_number_times from " . tablename('stonefish_scratch_reply') . " where rid = :rid", array(':rid' => $rid));
			if(($sharereply['day_number_times']>0 && $sharereply['day_number_times']-$fans['todaynum']<=0) || ($sharereply['number_times']>0 && $sharereply['number_times']-$fans['totalnum']<=0)){
				pdo_update('stonefish_scratch_fans', array('tosharenum' => $fans['tosharenum'] + 1), array('id' => $fans['id']));
			}
			//查询是否为分享赠送次数
			//商家赠送增加使用次数
		    if($reply['opportunity']==1){
			    ...
			    mc_credit_update($_W['member']['uid'], $reply['credit_type'], -$reply['credit_value'], array($_W['member']['uid'], '兑换刮刮卡活动消耗：'.$reply['credit_value'].'个'.$creditnames));
            ...
            if($prizeid>0){
				...
				$consumetime = '';
				//查询奖品名称和类型				
				$awardinfo = pdo_fetch("select * from ".tablename('stonefish_scratch_prize')." where id = :id", array(':id' =>$prizeid));
				if(!empty($awardinfo['prizepic'])){
				    $awardinfo['prizepic'] = toimage($awardinfo['prizepic']);
				}
				//减少库存
				if($exchange['inventory']==1){
					pdo_update('stonefish_scratch_prize', array('prizedraw' => $awardinfo['prizedraw'] + 1), array('id' => $prizeid));
				}
				//减少库存
				if($awardinfo['prizetype']!='physical' && $awardinfo['prizetype']!='virtual' && $awardinfo['prizetype']!='spaceprize' && $awardinfo['prizetype']!='againprize'){
			        ...
			        //添加积分到粉丝数据库
			        mc_credit_update($_W['member']['uid'], $awardinfo['prizetype'], $awardinfo['prizevalue'], array($_W['member']['uid'], '刮刮卡刮中'.$awardinfo['prizevalue'].'个'.$credit_names));
			        //添加积分到粉丝数据库
				...
				if($awardinfo['prizetype']=='againprize'){
				    //参与次数减少一次
                    //pdo_update('stonefish_scratch_fans', array('totalnum' => $fans['totalnum'],'todaynum' => $fans['todaynum'],'lasttime'=>$nowtime), array('id' => $fans['id']));
					$status = 2;
				    $consumetime = time();
					$fans['tickettype'] = 1;
					$fans['ticketid'] = 0;
				...
				if($awardinfo['prizetype']!='spaceprize'){
				    ...
				    $temp = pdo_insert('stonefish_scratch_fansaward', $insert);
		...
		//查询此用户是否还有中奖记录并更新状态
		$yes = pdo_fetch("select * FROM ".tablename('stonefish_scratch_fansaward') ." where from_user=:from_user and uniacid=:uniacid and rid=:rid and zhongjiang>=1",array(":from_user"=>$from_user,":uniacid"=>$_W['uniacid'],":rid"=>$rid));
		if(empty($yes)){
			pdo_update('stonefish_scratch_fans', array('zhongjiang' => 0), array('rid' => $rid,'uniacid' => $_W['uniacid'],'from_user' => $from_user));
		}else{
			$yes = pdo_fetch("select * FROM ".tablename('stonefish_scratch_fansaward') ." where from_user=:from_user and uniacid=:uniacid and rid=:rid and zhongjiang>=2",array(":from_user"=>$from_user,":uniacid"=>$_W['uniacid'],":rid"=>$rid));
			if(empty($yes)){
				pdo_update('stonefish_scratch_fans', array('zhongjiang' => 1), array('rid' => $rid,'uniacid' => $_W['uniacid'],'from_user' => $from_user));
			}else{
				pdo_update('stonefish_scratch_fans', array('zhongjiang' => 2), array('rid' => $rid,'uniacid' => $_W['uniacid'],'from_user' => $from_user));
			}
		}
		//查询此用户是否还有中奖记录并更新状态
	...
	public function doMobileRegfans() {
		...
		if(!empty($fans)){
            ...
            pdo_insert('stonefish_scratch_fans', $fansdata);
            $fans['id'] = pdo_insertid();
			//自动读取会员信息存入FANS表中
			$ziduan = array('realname','mobile','qq','email','address','gender','telephone','idcard','company','occupation','position');
			foreach ($ziduan as $ziduans){
				if($reply['is'.$ziduans]){
					if(!empty($_GPC[$ziduans])){
				        pdo_update('stonefish_scratch_fans', array($ziduans => $_GPC[$ziduans]), array('id' => $fans['id']));
				        if($reply['isfans']){				            
                            if($ziduans=='email'){
	...
	public function doMobileEditfans() {
        ...
        $from_user = authcode(base64_decode($_GPC['from_user']), 'DECODE');
		$uniacid = $_W['uniacid'];
		$reply = pdo_fetch("select id,fansnum FROM " . tablename('stonefish_scratch_reply') . " where rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
		$exchange = pdo_fetch("select * FROM ".tablename("stonefish_scratch_exchange")." where rid = :rid", array(':rid' => $rid));
        //判断是否参与过
		$fans = pdo_fetch("select * from ".tablename('stonefish_scratch_fans')." where rid = :rid and uniacid = :uniacid and from_user= :from_user", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
		$uid = pdo_fetchcolumn("select uid FROM ".tablename('mc_mapping_fans') ." where openid=:openid and uniacid=:uniacid",array(":openid"=>$from_user,":uniacid"=>$uniacid));
	...
	public function doMobileShare_confirm() {
		...
		if ($fans == true) {
			//保存分享次数
			pdo_update('stonefish_scratch_fans', array('share_num' => $fans['share_num']+1,'sharetime' => time()), array('id' => $fans['id']));
			$data = array(
                'msg' => '分享次数保存成功！',
                'success' => 1,
            );
	...
	public function doMobileRule() {
		...
		//虚拟人数
		//查询奖品设置
		$prize = pdo_fetchall("select * FROM " . tablename('stonefish_scratch_prize') . " where rid = :rid ORDER BY `break` asc", array(':rid' => $rid));
		if($this->Weixin()){
	...
	public function doMobileFirend() {
		...
		//获取openid
		//助力
		$firend = pdo_fetchall("select * from " . tablename('stonefish_scratch_sharedata') . " where rid = :rid and uniacid = :uniacid and fromuser = :from_user order by `id` desc", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
		foreach ($firend as $mid => $firends) {			    
			if(empty($firends['nickname'])){
				$firend[$mid]['nickname'] = '匿名好友';
			}
			if(empty($firends['avatar'])){
				$firend[$mid]['avatar'] = '../addons/stonefish_scratch/template/images/avatar.jpg';
			}else{
	...
	public function doMobileRank() {
		...
		$rank = pdo_fetchall("select * from " . tablename('stonefish_scratch_fans') . " where rid = :rid and uniacid = :uniacid order by ".$rankorder." desc,`id` asc limit ".$reply['viewranknum'], array(':rid' => $rid,':uniacid' => $uniacid));		
		//排行榜
		if($this->Weixin()){
...
select * from ".tablename('stonefish_scratch_fansaward')." order by zhongjiang asc) as tt  where rid = :rid and uniacid = :uniacid and from_user = :from_user and zhongjiang>=1 GROUP BY prizeid order by `id` desc", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
		foreach ($mylihe as $mid => $mylihes) {
			$mylihe[$mid]['num'] = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid = :uniacid and from_user = :from_user and zhongjiang=1 and prizeid='".$mylihes['prizeid']."'", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
			$mylihe[$mid]['numd'] = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid = :uniacid and from_user = :from_user and zhongjiang=2 and prizeid='".$mylihes['prizeid']."'", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
			$prize = pdo_fetch("select * from " . tablename('stonefish_scratch_prize') . " where id='".$mylihes['prizeid']."'");
	...
	public function doMobileExchange_shangjia() {
		...
		$fansaward = pdo_fetch("select * from " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid = :uniacid and from_user = :from_user and zhongjiang=1", array(':rid' => $rid,':uniacid' => $uniacid,':from_user' => $from_user));
		if(!empty($fansaward)){
		    if($exchange['tickettype']==2){
			...
			if($_GPC['award_id']=='all'){
			    pdo_update('stonefish_scratch_fansaward', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $ticketname), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user, 'zhongjiang' => 1));
			    pdo_update('stonefish_scratch_fans', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $ticketname), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user));
			}else{
				pdo_update('stonefish_scratch_fansaward', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $ticketname), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user, 'zhongjiang' => 1, 'prizeid' =>$_GPC['award_id']));
			    pdo_update('stonefish_scratch_fans', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $ticketname), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user));
			}
			$data = array(                    
			    'msg' => '数据保存成功！请返回兑奖！',
	...
	public function doMobileExchange() {
		...
		if($exchange['tickettype']==4){
			if($exchange['awardingpas']!=$password){
				...
				if($awardid=='all'){
					$prizenum = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and zhongjiang=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
					pdo_update('stonefish_scratch_fansaward', array('tickettype' => $exchange['tickettype'],'zhongjiang' => 2, 'consumetime' => time()), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user, 'zhongjiang' => 1));
					pdo_update('stonefish_scratch_fans', array('tickettype' => $exchange['tickettype'],'zhongjiang' => 2), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user));					
					//减少库存
					if($exchange['inventory']==1){
					    $prize = pdo_fetchall("select * FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and zhongjiang=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
						foreach ($prize as $prizes) {
							pdo_update('stonefish_scratch_prize', array('prizedraw' => $prizes['prizedraw'] + 1), array('id' => $prizes['id']));
						}
				    }
					//减少库存
					//发送消息模板之奖记录
					...
					$prizenum = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and prizeid =:prizeid and zhongjiang=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user, ':prizeid' => $awardid));
					$prizeid = pdo_fetchcolumn("select prizeid FROM " . tablename('stonefish_scratch_fansaward') . " where id='" . $awardid . "'");
					pdo_update('stonefish_scratch_fansaward', array('tickettype' => $exchange['tickettype'],'zhongjiang' => 2, 'consumetime' => time()), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user, 'prizeid' => $awardid, 'zhongjiang' => 1));
					pdo_update('stonefish_scratch_fans', array('tickettype' => $exchange['tickettype'],'zhongjiang' => 2), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user));					
					//减少库存
					if($exchange['inventory']==2){
					    pdo_update('stonefish_scratch_prize', array('prizedraw' => $prize['prizedraw'] + $prizenum), array('id' => $awardid));
				    }
					//减少库存
					//发送消息模板之奖记录
					if($exchange['tmplmsg_exchange']){
			...
			if($duijiangmima==1){
				if($awardid=='all'){
					$prizenum = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and zhongjiang=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
					pdo_update('stonefish_scratch_fansaward', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $shangjia['shangjianame'],'zhongjiang' => 2, 'consumetime' => time()), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user, 'zhongjiang' => 1));
					pdo_update('stonefish_scratch_fans', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $shangjia['shangjianame'],'zhongjiang' => 2), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user));
					//减少库存
					if($exchange['inventory']==1){
					    $prize = pdo_fetchall("select * FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and zhongjiang=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user));
						foreach ($prize as $prizes) {
					...
					$prizenum = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid =:uniacid and from_user =:from_user and prizeid =:prizeid and zhongjiang=1", array(':rid' => $rid, ':uniacid' => $uniacid, ':from_user' => $from_user, ':prizeid' => $awardid));
					$prize = pdo_fetch("select * FROM " . tablename('stonefish_scratch_prize') . " where id='" . $awardid . "'");
					pdo_update('stonefish_scratch_fansaward', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $shangjia['shangjianame'],'zhongjiang' => 2, 'consumetime' => time()), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user, 'prizeid' => $awardid, 'zhongjiang' => 1));
					pdo_update('stonefish_scratch_fans', array('tickettype' => $exchange['tickettype'],'ticketid' => $shangjiaid,'ticketname' => $shangjia['shangjianame'],'zhongjiang' => 2), array('rid' => $rid, 'uniacid' => $uniacid, 'from_user' => $from_user));
					//减少库存
					if($exchange['inventory']==2){
					    pdo_update('stonefish_scratch_prize', array('prizedraw' => $prize['prizedraw'] + $prizenum), array('id' => $awardid));
	...
	public function doWebManage() {
        ...
        $total = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_reply') . "  where uniacid=:uniacid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 20;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * from " . tablename('stonefish_scratch_reply') . " where uniacid=:uniacid " . $where . " order by id desc " . $limit, $params);
	...
	public function doWebTrend() {
		...
		//查询do参数
		if(empty($_GPC['do'])){
			$_GPC['do'] = pdo_fetchcolumn("select do from " . tablename('modules_bindings') . "  where eid = :eid and module=:module", array(':eid' => $_GPC['eid'], ':module' => 'stonefish_scratch'));
		}
		//查询do参数		
        $rid = intval($_GPC['rid']);
		...
		//中奖数量
		$reply['zhongjiangnum'] = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid=:uniacid and zhongjiang>=1", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//领取数量
		$reply['lingqunum'] = pdo_fetchcolumn("select sum(totalnum) FROM " . tablename('stonefish_scratch_fans') . " where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//领取数量
		$reply['helpnum'] = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_scratch_sharedata') . " where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//所有奖品类别
		//今日昨天关键指标
		$fansnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('stonefish_scratch_fans') . ' WHERE rid = :rid AND uniacid = :uniacid AND createtime >= :starttime AND createtime <= :endtime', array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':starttime' => strtotime(date('Y-m-d')) - 86400, ':endtime' => strtotime(date('Y-m-d'))));
		$helpnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('stonefish_scratch_sharedata') . ' WHERE rid = :rid AND uniacid = :uniacid AND visitorstime >= :starttime AND visitorstime <= :endtime', array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':starttime' => strtotime(date('Y-m-d')) - 86400, ':endtime' => strtotime(date('Y-m-d'))));
		$zhongjiangnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('stonefish_scratch_fansaward') . ' WHERE rid = :rid AND uniacid = :uniacid AND createtime >= :starttime AND createtime <= :endtime and zhongjiang>=1', array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':starttime' => strtotime(date('Y-m-d')) - 86400, ':endtime' => strtotime(date('Y-m-d'))));
		
		$today_fansnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('stonefish_scratch_fans') . ' WHERE rid = :rid AND uniacid = :uniacid AND createtime >= :starttime AND createtime <= :endtime', array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':starttime' => strtotime(date('Y-m-d')), ':endtime' => TIMESTAMP));
		$today_helpnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('stonefish_scratch_sharedata') . ' WHERE rid = :rid AND uniacid = :uniacid AND visitorstime >= :starttime AND visitorstime <= :endtime', array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':starttime' => strtotime(date('Y-m-d')), ':endtime' => TIMESTAMP));
		$today_zhongjiangnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('stonefish_scratch_fansaward') . ' WHERE rid = :rid AND uniacid = :uniacid AND createtime >= :starttime AND createtime <= :endtime and zhongjiang>=1', array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':starttime' => strtotime(date('Y-m-d')), ':endtime' => TIMESTAMP));
		//今日昨天关键指标
		...
		if($_W['isajax'] && $_W['ispost']) {
			...
			$data = pdo_fetchall('SELECT createtime FROM ' . tablename('stonefish_scratch_fans') . ' WHERE uniacid = :uniacid AND rid = :rid AND createtime >= :starttime AND createtime <= :endtime', array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':starttime' => $starttime, ':endtime' => $endtime));
		    foreach($data as $da) {
			...
			$data = pdo_fetchall('SELECT visitorstime FROM ' . tablename('stonefish_scratch_sharedata') . ' WHERE uniacid = :uniacid AND rid = :rid AND visitorstime >= :starttime AND visitorstime <= :endtime', array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':starttime' => $starttime, ':endtime' => $endtime));
		    foreach($data as $da) {
			...
			$data = pdo_fetchall('SELECT createtime FROM ' . tablename('stonefish_scratch_fansaward') . ' WHERE uniacid = :uniacid AND rid = :rid AND createtime >= :starttime AND createtime <= :endtime and zhongjiang>=1', array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':starttime' => $starttime, ':endtime' => $endtime));
		    foreach($data as $da) {
			    $key = date('m-d', $da['createtime']);
			    if(in_array($key, array_keys($days))) {
				    $datasets['flow4'][$key]++;
	...
	public function doWebTemplate() {
		...
		//查询是否填写系统参数
		//活动模板
		$template = pdo_fetch("select * FROM " . tablename('stonefish_scratch_template') . " where uniacid = :uniacid ORDER BY `id` asc", array(':uniacid' => $_W['uniacid']));
		if(empty($template)){			
        ...
        $total = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_template') . "  where uniacid=:uniacid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 20;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * from " . tablename('stonefish_scratch_template') . " where uniacid=:uniacid " . $where . " order by id desc " . $limit, $params);
	...
	public function doWebTemplatepost() {
		...
		load()->func('tpl');
		if(!empty($id)) {
			$item = pdo_fetch("select * FROM ".tablename('stonefish_scratch_template')." where id = :id", array(':id' => $id));				
		}else{
		...
		if(checksubmit('submit')) {
			...
			if(!empty($id)) {
				pdo_update('stonefish_scratch_template', $data, array('id' => $id));
				message('模板修改成功！', url('site/entry/template', array('m' => 'stonefish_scratch')), 'success');
			}else{
				pdo_insert('stonefish_scratch_template', $data);
				message('模板添加成功！', url('site/entry/template', array('m' => 'stonefish_scratch')), 'success');
			}			
		}
	...
	public function doWebTemplatedel() {
		...
		if(!empty($id)) {
			$item = pdo_fetch("select * FROM ".tablename('stonefish_scratch_template')." where id = :id", array(':id' => $id));
			if(!empty($item)){
				pdo_delete('stonefish_scratch_template', array('id' => $id));
	...
	public function doWebTmplmsg() {
        ...
        $total = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_tmplmsg') . "  where uniacid=:uniacid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 20;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * from " . tablename('stonefish_scratch_tmplmsg') . " where uniacid=:uniacid " . $where . " order by id desc " . $limit, $params);
	...
	public function doWebTmplmsgpost() {
		...
		load()->func('tpl');
		if(!empty($id)) {
			$item = pdo_fetch("select * FROM ".tablename('stonefish_scratch_tmplmsg')." where id = :id", array(':id' => $id));				
		}else{
			$item['uniacid'] = $_W['uniacid'];
		}
		if(checksubmit('submit')) {
			...
			if(!empty($id)) {
				pdo_update('stonefish_scratch_tmplmsg', $data, array('id' => $id));
				message('消息模板修改成功！', url('site/entry/tmplmsg', array('m' => 'stonefish_scratch')), 'success');
			}else{
				pdo_insert('stonefish_scratch_tmplmsg', $data);
				message('消息模板添加成功！', url('site/entry/tmplmsg', array('m' => 'stonefish_scratch')), 'success');
			}			
		}
	...
	public function doWebTmplmsgdel() {
		...
		if(!empty($id)) {
			$item = pdo_fetch("select * FROM ".tablename('stonefish_scratch_tmplmsg')." where id = :id", array(':id' => $id));
			if(!empty($item)){
				pdo_delete('stonefish_scratch_tmplmsg', array('id' => $id));
...
//消息通知记录
	public function doWebPosttmplmsg() {
		...
		$rid = $_GPC['rid'];
		$rid = empty($rid) ? $_GPC['id'] : $rid;
		$reply = pdo_fetch("select poweravatar from ".tablename('stonefish_scratch_reply')." where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//查询do参数
		if(empty($_GPC['do'])){
			$_GPC['do'] = pdo_fetchcolumn("select do from " . tablename('modules_bindings') . "  where eid = :eid and module=:module", array(':eid' => $_GPC['eid'], ':module' => 'stonefish_scratch'));
		}
		...
		$total = pdo_fetchcolumn("select count(a.id) from " . tablename('stonefish_scratch_fanstmplmsg') . " as a," . tablename('stonefish_scratch_fans') . " as b where a.rid = :rid and a.uniacid=:uniacid and a.from_user=b.from_user" . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 20;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select a.tmplmsg,a.createtime,b.avatar,b.realname,b.nickname,b.mobile,c.template_name from " . tablename('stonefish_scratch_fanstmplmsg') . " as a," . tablename('stonefish_scratch_fans') . " as b," . tablename('stonefish_scratch_tmplmsg') . " as c where a.rid = :rid and a.uniacid=:uniacid and a.from_user=b.from_user and c.id=a.tmplmsgid" . $where . " order by a.id desc " . $limit, $params);
	...
	public function doWebFansdata() {
		...
		foreach ($list as &$lists) {
			$lists['awardinfo'] = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fansaward') . "  where rid = :rid and from_user=:from_user", array(':rid' => $rid,':from_user' => $lists['from_user']));
			$lists['share_num'] = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_sharedata') . "  where rid = :rid and fromuser=:from_user", array(':rid' => $rid,':from_user' => $lists['from_user']));
		...
		//中奖情况以及是否为关注会员并发送消息
		//一些参数的显示
        $num1 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fans') . "  where rid = :rid and uniacid=:uniacid and zhongjiang=0", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num2 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fans') . "  where rid = :rid and uniacid=:uniacid and zhongjiang>=1", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num3 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fans') . "  where rid = :rid and uniacid=:uniacid and zhongjiang>=1 and xuni=1", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//一些参数的显示
	...
	public function doWebSetfansstatus() {
		...
		if ($id) {
			$data = ($data==1?'0':'1');
			pdo_update("stonefish_scratch_fans", array('status' => $data), array("id" => $id));
	...
	public function doWebDeletefans() {
        global $_GPC, $_W;
		$rid = intval($_GPC['rid']);
		$reply = pdo_fetch("select * from ".tablename('stonefish_scratch_reply')." where rid = :rid and uniacid=:uniacid", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $exchange = pdo_fetch("select inventory FROM ".tablename("stonefish_scratch_exchange")." where rid = :rid", array(':rid' => $rid));
		if(empty($reply)){
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
            ...
            if($id == 0)
                continue;
			$fans = pdo_fetch("select * from ".tablename('stonefish_scratch_fans')." where id = :id", array(':id' => $id));
            if(empty($fans)){
            ...
            //删除粉丝中奖记录
			load()->model('mc');
			$fansaward = pdo_fetchall("select id,prizeid,zhongjiang from " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid=:uniacid and from_user=:from_user", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':from_user' => $fans['from_user']));
			foreach ($fansaward as $fansawards) {
				$prize = pdo_fetch("select prizedraw,prizetype,prizevalue from " . tablename('stonefish_scratch_prize') . " where id = :id", array(':id' => $fansawards['prizeid']));
				if($exchange['inventory']==1){
					pdo_update('stonefish_scratch_prize', array('prizedraw' => $prize['prizedraw']-1), array('id' => $fansawards['prizeid']));
				}else{
				...
				if($prize['prizetype']!='physical' && $prize['prizetype']!='virtual' && $prize['prizetype']!='againprize'){
					...
					//扣除积分到粉丝数据库					
			        mc_credit_update($uid, $prize['prizetype'], -$prize['prizevalue'], array($_W['uid'], '刮刮卡删除奖品扣除'.$prize['prizevalue'].'个'.$credit_names));
			        //扣除积分到粉丝数据库
				}
			    //查询奖品是否为虚拟积分，如果是则扣除相应的积分
			}
			//删除粉丝中奖记录			
			//删除粉丝中奖详细记录
			pdo_delete('stonefish_scratch_fansaward', array('from_user' => $fans['from_user']));
			//删除粉丝中奖详细记录
			//删除粉丝中奖详细记录
			pdo_delete('stonefish_scratch_sharedata', array('fromuser' => $fans['from_user']));
			//删除粉丝中奖详细记录
			//删除粉丝参与记录
		...
		//减少参与记录
		pdo_update('stonefish_scratch_reply', array('fansnum' => $reply['fansnum']-$i), array('id' => $reply['id']));
		//减少参与记录
	...
	public function doWebUserinfo() {
		...
		if($_W['isajax']) {
			...
			//粉丝数据
			if($uid){
				$data = pdo_fetch("select * FROM ".tablename('stonefish_scratch_fans')." where id = :id", array(':id' => $uid));
			}else{
				echo '未找到指定粉丝资料';
				exit;
			}
	...
	public function doWebPrizeinfo() {
		...
		if($_W['isajax']) {
			...
			//中奖记录
			if($uid){
				$data = pdo_fetch("select id, from_user from " . tablename('stonefish_scratch_fans') . ' where id = :id', array(':id' => $uid));
				$list = pdo_fetchall("select a.*,b.* from " . tablename('stonefish_scratch_fansaward') . " a," . tablename('stonefish_scratch_prize') . " AS b where a.prizeid = b.id and a.rid = :rid and a.uniacid=:uniacid and a.from_user=:from_user order by a.id desc ", array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':from_user' => $data['from_user']));
			}else{
				echo '未找到指定粉丝中奖记录';
				exit;
			}
	...
	public function doWebSharelist() {
		...
		if($_W['isajax']) {
			...
			$rid = intval($_GPC['rid']);
			//规则
			$reply = pdo_fetch("select poweravatar FROM " . tablename('stonefish_scratch_reply') . " where rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
			//粉丝数据
			$data = pdo_fetch("select id, from_user  FROM " . tablename('stonefish_scratch_fans') . ' where id = :id', array(':id' => $uid));
			$share = pdo_fetchall("select * FROM " . tablename('stonefish_scratch_sharedata') . "  where rid = :rid and uniacid=:uniacid and fromuser=:fromuser ORDER BY id DESC ", array(':uniacid' => $_W['uniacid'], ':rid' => $rid, ':fromuser' => $data['from_user']));
	...
	public function doWebSavexunishare() {
		...
		if(!$point){
		    message('助力额必需填写', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')), 'error');
		}
		if(!$rid){
		    message('系统出错', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')), 'error');
		}
		if($uid) {
		    //规则
			$reply = pdo_fetch("select * FROM " . tablename('stonefish_scratch_reply') . " where rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
            ...
            pdo_insert('stonefish_scratch_sharedata', $insert);
			//添加中奖记录
            //设置此粉丝为虚拟中奖者
            pdo_update('stonefish_scratch_fans', array('sharepoint' => $data['sharepoint'] + $point,'xuni' => 1), array('id' => $data['id']));
			//设置此粉丝为虚拟中奖者
			message('添加虚拟助力量成功', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')));
		}else{
			message('未找到指定用户', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')), 'error');
		}       
    }
	//虚拟助力
	//虚拟奖品
	...
	public function doWebSavexuniaward() {
		...
		if(!$awardid){
		    message('必需选择奖品才能生效', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')), 'error');
		}
		if(!$rid){
		    message('系统出错', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')), 'error');
		}
		if($uid) {
			...
			$reply = pdo_fetch("select * from " . tablename('stonefish_scratch_reply') . " where rid = :rid order by `id` desc", array(':rid' => $rid));
			//粉丝数据
			$data = pdo_fetch("select id, from_user, awardnum  from " . tablename('stonefish_scratch_fans') . ' where id = :id', array(':id' => $uid));
			//添加中奖记录
			$prize = pdo_fetch("select * from " . tablename('stonefish_scratch_prize') . "  where id=:id", array(':id' => $awardid));
			pdo_update('stonefish_scratch_prize', array('prizedraw' => $prize['prizedraw'] + 1), array('id' => $awardid));
            //保存award中
			...
			message('添加虚拟奖品成功', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')));
		} else {
			message('未找到指定用户', url('site/entry/fansdata',array('rid' => $rid, 'm' => 'stonefish_scratch')), 'error');
		}      
    }
	...
	public function doWebSharedata() {
		...
		$total = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_sharedata') . "  where rid = :rid and uniacid=:uniacid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 20;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * from " . tablename('stonefish_scratch_sharedata') . " where rid = :rid and uniacid=:uniacid " . $where . " order by id desc " . $limit, $params);
		//分享人
		foreach ($list as &$lists) {
			$fans = pdo_fetch("select avatar,nickname,realname from " . tablename('stonefish_scratch_fans') . "  where rid = :rid and from_user=:from_user", array(':rid' => $rid,':from_user' => $lists['fromuser']));
	...
	public function doWebDeletesharedata() {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
            ...
            if($id == 0)
                continue;
			$sharedata = pdo_fetch("select id,point,fromuser from ".tablename('stonefish_scratch_sharedata')." where id = :id", array(':id' => $id));
            if(empty($sharedata)){
				echo json_encode(array('errno' => 1,'error' => '抱歉，选中的数据不存在！'));
				exit;
            }
			$fans = pdo_fetch("select * from " . tablename('stonefish_scratch_fans') . " where rid = :rid and uniacid=:uniacid and from_user=:from_user", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':from_user' => $sharedata['fromuser']));
			//减少参与粉丝分享助力
			pdo_update('stonefish_scratch_fans', array('sharenum' => $fans['sharenum']-1,'sharepoint' => $fans['sharepoint']-$sharedata['point']), array('id' => $fans['id']));
			//减少参与粉丝分享助力			
			//删除粉丝分享记录
			pdo_delete('stonefish_scratch_sharedata', array('id' => $sharedata['id']));
			//删除粉丝分享记录
        }
	...
	public function doWebPrizedata() {
		...
		foreach ($award as $k =>$awards) {
			$award[$k]['num'] = pdo_fetchcolumn("select count(id) FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid=:uniacid and prizeid=:prizeid", array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':prizeid' => $awards['id']));
		}
		//所有奖品类别
		$params = array(':rid' => $rid, ':uniacid' => $_W['uniacid']);
		if (!empty($_GPC['nickname'])) {
		...
		if (!empty($_GPC['prizeid'])) {
            $statustitle .= pdo_fetchcolumn("select prizerating FROM ".tablename('stonefish_scratch_prize')." where id=:prizeid", array(':prizeid' => $_GPC['prizeid']));;
			$where.=' and a.prizeid=:prizeid';
            $params[':prizeid'] = $_GPC['prizeid'];
        }
		//导出标题以及参数设置				
		$total = pdo_fetchcolumn("select count(a.id) from " . tablename('stonefish_scratch_fansaward') . " as a," . tablename('stonefish_scratch_fans') . " as b where a.from_user = b.from_user and a.rid = b.rid and a.uniacid = b.uniacid and a.rid = :rid and a.uniacid=:uniacid" . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 20;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select a.*,b.id as fid,b.avatar,b.nickname,b.realname,b.mobile from " . tablename('stonefish_scratch_fansaward') . " as a," . tablename('stonefish_scratch_fans') . " as b where a.from_user = b.from_user and a.rid = b.rid and a.uniacid = b.uniacid and a.rid = :rid and a.uniacid=:uniacid" . $where . " order by a.id desc " . $limit, $params);
		//奖品名称
		foreach ($list as &$lists) {
			$prize = pdo_fetch("select prizetype,prizerating,prizename from " . tablename('stonefish_scratch_prize') . "  where id = :id", array(':id' =>$lists['prizeid']));
			$lists['prizetype'] =$prize['prizetype'];
		...
		//奖品名称
		//一些参数的显示
        $num1 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fansaward') . "  where rid = :rid and uniacid=:uniacid and zhongjiang>=1 and tickettype=1", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num2 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fansaward') . "  where rid = :rid and uniacid=:uniacid and zhongjiang>=1 and tickettype=2", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
        $num3 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fansaward') . "  where rid = :rid and uniacid=:uniacid and zhongjiang>=1 and tickettype=3", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		$num4 = pdo_fetchcolumn("select count(id) from " . tablename('stonefish_scratch_fansaward') . "  where rid = :rid and uniacid=:uniacid and zhongjiang>=1 and tickettype=4", array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
		//一些参数的显示
	...
	public function doWebSetprizestatus() {
		...
		$exchange = pdo_fetch("select inventory,tickettype FROM ".tablename("stonefish_scratch_exchange")." where rid = :rid", array(':rid' => $rid));
		if (empty($id)) {
            message('抱歉，传递的参数错误！', '', 'warning');
        }
		//查询奖品数量
		$prize = pdo_fetch("select * FROM " . tablename('stonefish_scratch_prize') . " where id = :id ORDER BY `id` DESC", array(':id' => $pid));
        ...
        $temp = pdo_update('stonefish_scratch_fansaward', $p, array('id' => $id));
		if($exchange['inventory']==2 && $zhongjiang == 2){
			pdo_update('stonefish_scratch_prize', array('prizedraw' => $prize['prizedraw']+1), array('id' => $pid));
		}
        ...
        if ($temp == false) {
		    ...
		    //修改用户状态
			$from_user = pdo_fetchcolumn("select from_user FROM " . tablename('stonefish_scratch_fansaward') . " where id = :id ORDER BY `id` DESC", array(':id' => $id));
			pdo_update('stonefish_scratch_fans', array('zhongjiang' => $zhongjiang), array('rid' => $rid,'uniacid' => $_W['uniacid'],'from_user' => $from_user));
	...
	public function doWebDeleteprizedata() {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
            ...
            if($id == 0)
                continue;
			$fansaward = pdo_fetch("select prizeid,from_user,zhongjiang from ".tablename('stonefish_scratch_fansaward')." where id = :id", array(':id' => $id));
			$from_user = $fansaward['from_user'];
			$fans = pdo_fetch("select * from ".tablename('stonefish_scratch_fans')." where rid = :rid and  uniacid = :uniacid and  from_user = :from_user", array(':rid' => $rid,':uniacid'=>$_W['uniacid'],':from_user'=>$from_user));
            if(empty($fansaward)){
				echo json_encode(array('errno' => 1,'error' => '抱歉，选中的中奖数据不存在！'));
				exit;
            }
			$prize = pdo_fetch("select prizetype,prizevalue,prizedraw FROM " . tablename('stonefish_scratch_prize') . " where id = :id ORDER BY `id` DESC", array(':id' => $fansaward['prizeid']));
			//修改积分到粉丝数据库
			if($prize['prizetype']!='physical' && $prize['prizetype']!='virtual' && $prize['prizetype']!='againprize' && $prize['prizetype']!='spaceprize'){
			    ...
			    mc_credit_update($uid, $prize['prizetype'], -$prize['prizevalue'], array($_W['uid'], '刮刮卡取消中奖扣除'.$prize['prizevalue'].'个'.$credit_names));
			}
			//修改积分到粉丝数据库
			if($exchange['inventory']==1){
				pdo_update('stonefish_scratch_prize', array('prizedraw' => $prize['prizedraw']-1), array('id' => $fansaward['prizeid']));
			}else{
			...
			//删除粉丝中奖记录
			pdo_update('stonefish_scratch_fansaward', array('zhongjiang' => 0, 'zhongjiangtime' => ''), array('id' => $id));
			//再来一次删除记录并减少次数
			if($prize['prizetype']=='againprize'){
				pdo_delete('stonefish_scratch_fansaward', array('id' => $id));
				pdo_update('stonefish_scratch_fans', array('totalnum' => $fans['totalnum']-1,'todaynum' => $fans['todaynum']-1), array('id' => $fans['id']));
			}
			//再来一次删除记录并减少次数
			//删除粉丝中奖记录
			//查询此用户是否还有中奖记录并更新状态
	...
	public function doWebRankdata() {
        ...
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("select * from " . tablename('stonefish_scratch_fans') . " where rid = :rid and uniacid=:uniacid " . $where . " order by ".$order." desc,id asc " . $limit, $params);
	...
	public function doWebImportingsave() {
        ...
        message('导入增送次数成功', url('site/entry/branch',array('rid' => $rid, 'm' => 'stonefish_scratch')));
		exit;    
    }
	//导入商家网点增送记录
	//修改商家网店增送记录
	...
	public function doWebEditbranchsave() {
		...
		if($uid) {
			...
			message('修改增送次数成功', url('site/entry/branch',array('rid' => $rid, 'm' => 'stonefish_scratch')));
		} else {
	...
	public function doWebDeletebranch() {
        global $_GPC, $_W;
		$rid = intval($_GPC['rid']);
		$reply = pdo_fetch("select * FROM ".tablename('stonefish_scratch_reply')." where rid = :rid", array(':rid' => $rid));
        if (empty($reply)) {
        ...
        foreach ($_GPC['idArr'] as $k => $id) {
			...
			foreach ($doingslist as $doingslists) {
				//删除中奖记录
				//删除奖品第一步先恢复到奖池中
				$award = pdo_fetch("select id,prizeid,from_user FROM " . tablename('stonefish_scratch_fansaward') . " where id = :id", array(':id' => $doingslists['prizeid']));
				$prize = pdo_fetch("select id,prizedraw FROM " . tablename('stonefish_scratch_prize') . " where id = :id", array(':id' => $award['prizeid']));
				pdo_update('stonefish_scratch_prize', array('prizedraw' => $prize['prizedraw']-1), array('id' => $award['prizeid']));			
				//删除奖品第一步先恢复到奖池中
				//查询粉丝是否还有中奖记录，没有则需要改变粉丝状态
				$fansaward = pdo_fetch("select id FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid=:uniacid and from_user=:from_user and id!=:id", array(':rid' => $rid, ':from_user' => $award['from_user'], ':uniacid' => $_W['uniacid'], ':id' => $doingslists['prizeid']));
				if(empty($fansaward)){
					pdo_update('stonefish_scratch_fans', array('zhongjiang' => 0), array('from_user' => $award['from_user'],'uniacid' => $_W['uniacid'],'rid' => $rid));
				}else{
					$awardnum = pdo_fetchcolumn("select count(*) FROM " . tablename('stonefish_scratch_fansaward') . " where rid = :rid and uniacid=:uniacid and from_user=:from_user and id!=:id", array(':rid' => $rid,':uniacid' => $_W['uniacid'],':from_user' => $award['from_user'],':id' => $doingslists['prizeid']));
					pdo_update('stonefish_scratch_fans', array('awardnum' => $awardnum,'zhongjiang' => 1), array('from_user' => $award['from_user'],'uniacid' => $_W['uniacid'],'rid' => $rid));
				}
				//查询粉丝是否还有中奖记录，没有则需要改变粉丝状态
				//查询奖品是否为虚拟积分，如果是则扣除相应的积分			
				//查询奖品是否为虚拟积分，如果是则扣除相应的积分
    ...
    function getJsApiTicket($uniacid,$appId,$appSecret) {
        load()->func('cache');
        $api = cache_load("stonefish_scratch.api_share.json::".$uniacid, true);
        $new = false;
        ...
        $data = cache_load("stonefish_scratch.jsapi_ticket.json::".$uniacid, true);
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($ticket) {
                ...
                cache_write("stonefish_scratch.jsapi_ticket.json::".$uniacid, iserializer($data));
                cache_write("stonefish_scratch.api_share.json::".$uniacid, iserializer(array("appid"=>$appId,"appsecret"=>$appSecret)));
            }
        } else {
    ...
    function getAccessToken($uniacid,$appId,$appSecret) {
        ...
        $data = cache_load("stonefish_scratch.access_token.json::".$uniacid, true);     
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($access_token) {
                ...
                cache_write("stonefish_scratch.access_token.json::".$uniacid, iserializer($data));
                cache_write("stonefish_scratch.api_share.json::".$uniacid, iserializer(array("appid"=>$appId,"appsecret"=>$appSecret)));
            }
        } else {
