...
class stonefish_scratchModuleProcessor extends WeModuleProcessor {
	...
	public function respond() {
		...
		//查询活动
		$sql = "SELECT title,description,start_picurl,isshow,starttime,endtime,end_title,end_description,end_picurl FROM " . tablename('stonefish_scratch_reply') . " WHERE `uniacid` = :uniacid and `rid` = :rid LIMIT 1";
		...
		//查询活动
		//查询是否被屏蔽
		$lists = pdo_fetch("SELECT status,zhongjiang FROM ".tablename('stonefish_scratch_fans')." WHERE uniacid = :uniacid and rid= :rid and from_user = :from_user", array(':uniacid' => $uniacid, ':rid' => $rid, ':from_user' => $from_user));
