...
class Welian_Indiana_Credit {
	...
	public function updateCredit1($openid = '',$uniacid=0,$credit1=0) {
		global $_W;
		if(strpos($openid, "machine") == FALSE){
			load()->model('mc');
			$uid = self::mc_openidTouid($openid,$uniacid);
	
			mc_credit_update($uid, 'credit1', $credit1, array($uid, '一元夺宝商业版积分操作','weliam_indiana'));
		}
		
	}
	
	public function updateCredit2($openid = '',$uniacid=0,$credit2=0) {
		global $_W;
		if(strpos($openid, "machine") == FALSE){
			load()->model('mc');
			$uid = self::mc_openidTouid($openid,$uniacid);
	
			mc_credit_update($uid, 'credit2', $credit2, array($uid, '一元夺宝商业版余额操作'));
		}
