<?php
	global $_W,$_GPC;
	load() -> func('tpl');
	$ops = array('display', 'edit', 'delete','wdisplay','recover','regain','shiftdelete','machine','persondownload'); // 只支持此 3 种操作.
	...
	if ($op == 'edit') {
		...
		if (checksubmit()) {
			...
			if(empty($goods)){
				...
				//第一期夺宝码计算
				if(!empty($ret)){
					$CountNum=intval($data['price'])/$data['init_money'];
					$codes=array();
...
//			message("xxx");exit;
	...
	/*if($op == 'persondownload'){
		$result = pdo_fetchall("select nickname from".tablename('mc_members')."where nickname !=''");
		$nicknames = array();
		for($i = 0 ;$i < sizeof($result);$i++){
			$nicknames[$i] = $result[$i]['nickname'];
		}
		$nicknamese = serialize($nicknames);
		file_put_contents(WELIAM_INDIANA."/static/name.text", var_export($nicknamese, true).PHP_EOL, FILE_APPEND);
	}*/
?>