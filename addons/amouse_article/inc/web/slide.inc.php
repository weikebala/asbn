<?php
    //shizhongying qq:45300551
    global $_GPC, $_W;
    $op= $_GPC['op'] ? $_GPC['op'] : 'display';
    $weid= $_W['uniacid'];
    $category = pdo_fetchall("SELECT * FROM ".tablename('fineness_article_category')." WHERE uniacid =$weid and parentid=0 ORDER BY parentid ASC, displayorder ASC, id ASC ", array(), 'id');
    if($op == 'display') {
        ...
        if($id > 0) {
            $slide= pdo_fetch('SELECT * FROM '.tablename('fineness_adv')." WHERE weid=:weid AND id=:id", array(':weid' => $_W['uniacid'], ':id' => $id));
        }
        $pcate = $slide['pid'];
        ...
        if(checksubmit('submit')) {
            ...
            $pid= trim($_GPC['pid']) ? trim($_GPC['pid']) : message('请选择分类！');
            $insert= array('title' => $title,
                'link' => $_GPC['link'],'pid'=>$_GPC['pid'],
                'thumb' => $_GPC['thumb'],
                'weid' => $_W['uniacid']);
            ...
            message('更新幻灯片数据成功！', $this->createWebUrl('slide', array('op' => 'display', 'name' => 'zqwyx_heixiu')), 'success');
        }
