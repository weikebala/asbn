...
if (!empty($_POST) && verify($_POST) && $_POST['respMsg'] == 'success') {
	if ($type == '0') {
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('sea_member_log') . ' WHERE `uniacid`=:uniacid and `id`=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
		if (!empty($log) && empty($log['status'])) {
			pdo_update('sea_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'logno' => $log['openid']), array('id' => $logid));
