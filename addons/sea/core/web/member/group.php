...
if ($operation == 'display') {
    ...
    $list    = array(
        array(
            'groupname' => '无分组',
            'membercount' => pdo_fetchcolumn('select count(*) from ' . tablename('sea_member') . ' where uniacid=:uniacid and groupid=0 limit 1', array(
                ':uniacid' => $_W['uniacid']
            ))
        )
    );
    $alllist = pdo_fetchall("SELECT * FROM " . tablename('sea_member_group') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY id asc");
    foreach ($alllist as &$row) {
        $row['membercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_member') . ' where uniacid=:uniacid and groupid=:groupid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':groupid' => $row['id']
        ));
    }
    ...
    $group = pdo_fetch("SELECT * FROM " . tablename('sea_member_group') . " WHERE id = '$id'");
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_member_group', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
            plog('member.group.edit', "修改会员分组 ID: {$id}");
        } else {
            pdo_insert('sea_member_group', $data);
            $id = pdo_insertid();
    ...
    $id    = intval($_GPC['id']);
    $group = pdo_fetch("SELECT id,groupname FROM " . tablename('sea_member_group') . " WHERE id = '$id'");
    if (empty($group)) {
        message('抱歉，分组不存在或是已经被删除！', $this->createWebUrl('member/group', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_member_group', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
