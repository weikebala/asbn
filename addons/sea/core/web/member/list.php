...
if ($op == 'display') {
    ...
    if ($_GPC['followed'] != '') {
        ...
        } elseif($_GPC['followed'] == 1) {
            $condition .= ' and f.follow=' . intval($_GPC['followed']);
        } elseif ($_GPC['followed'] == 0) {
            $condition .= ' and dm.uid=0';
            $no_join_mapping_fans = 1;
        }
    }
    if ($_GPC['isblack'] != '') {
    ...
    if ($no_join_mapping_fans != 1) {
        $mc_mapping_fans = " left join " . "(select * from " . tablename('mc_mapping_fans') . " group by openid )" . " f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}";
        $sql .= $mc_mapping_fans;
    }
   ...
   /* $sql = "select dm.*,l.levelname,g.groupname,a.nickname as agentnickname,a.avatar as agentavatar from " . tablename('sea_member') . " dm " . " left join " . tablename('sea_member_group') . " g on dm.groupid=g.id" . " left join " . tablename('sea_member') . " a on a.id=dm.agentid" . " left join " . tablename('sea_member_level') . " l on dm.level =l.id"  . " where 1 {$condition}  ORDER BY dm.id DESC";*/
    ...
    if ($no_join_mapping_fans != 1) {
        $mc_mapping_fans = " left join " . "(select * from " . tablename('mc_mapping_fans') . " group by openid )". " f on f.openid=dm.openid" ;
        $total_sql .= $mc_mapping_fans;
    }
   ...
   //$total           = pdo_fetchcolumn("select count(*) from" . tablename('sea_member') . " dm " . " left join " . tablename('sea_member_group') . " g on dm.groupid=g.id" . " left join " . tablename('sea_member_level') . " l on dm.level =l.id" . " left join " . "(select openid,uniacid,follow,count(distinct openid) from " . tablename('mc_mapping_fans') . " where uniacid ={$_W['uniacid']} group by openid) f on f.openid=dm.openid" . " where 1 {$condition} ", $params);
        ...
        if ($hascommission) {
            if (cv('commission.agent.changeagent')) {
                ...
                if (!empty($adata)) {
                    ...
                    pdo_update('sea_member', $adata, array(
                        'id' => $id,
                        'uniacid' => $_W['uniacid']
                    ));
                    if (empty($_GPC['oldstatus']) && $adata['status'] == 1) {
    ...
    $member = m('member')->getMember($id);
	if(!empty($member['sfzimages'])){
		//查询有几个逗号
		$imgnum=substr_count($member['sfzimages'],',');
		if($imgnum>0){
			$sfzimages = explode(',',$member['sfzimages']); 
			
			foreach($sfzimages as $key=>$var){
				$dtimgs[$key]['imgs']=$var;
			}
			
		}else{
			$dtimgs['0']['imgs']=$member['sfzimages'];
		}
	}
    if ($hascommission) {
    ...
    $member['self_ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(
        ':uniacid' => $_W['uniacid'],
        ':openid' => $member['openid']
    ));
    $member['self_ordermoney'] = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('sea_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(
        ':uniacid' => $_W['uniacid'],
        ':openid' => $member['openid']
    ));
    if (!empty($member['agentid'])) {
