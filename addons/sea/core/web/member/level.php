...
if ($operation == 'display') {
    ...
    $level = pdo_fetch("SELECT * FROM " . tablename('sea_member_level') . " WHERE id = '$id'");
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_member_level', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
            plog('member.level.edit', "修改会员等级 ID: {$id}");
        } else {
            pdo_insert('sea_member_level', $data);
            $id = pdo_insertid();
    ...
    $id    = intval($_GPC['id']);
    $level = pdo_fetch("SELECT id,levelname FROM " . tablename('sea_member_level') . " WHERE id = '$id'");
    if (empty($level)) {
        message('抱歉，等级不存在或是已经被删除！', $this->createWebUrl('member/level', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_member_level', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
