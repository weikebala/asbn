...
if ($op == 'display') {
	...
	if ($op == 'check') {
		...
		$resp = ihttp_post(sea_AUTH_URL, array('type' => 'check', 'ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => $auth['domain'], 'version' => $version, 'files' => $files));
		...
		if ($op == 'download') {
			...
			if (!empty($path)) {
				$resp = ihttp_post(sea_AUTH_URL, array('type' => 'download', 'ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => $auth['domain'], 'path' => $path));
				$ret = @json_decode($resp['content'], true);
				if (is_array($ret)) {
					$path = $ret['path'];
				...
				if (!empty($upgrade['upgrade'])) {
					$updatefile = IA_ROOT . '/addons/sea/upgrade.php';
