...
if (!empty($year) && !empty($month) && !empty($day)) {
    for ($hour = 0; $hour < 24; $hour++) {
        ...
        $dr       = array(
            ...
            'count' => pdo_fetchcolumn("SELECT ifnull({$countfield},0) as cnt FROM " . tablename('sea_order') . " WHERE uniacid=:uniacid and status>=1 and createtime >=:starttime and createtime <=:endtime", array(
                ':uniacid' => $_W['uniacid'],
