...
$sql = 'SELECT g.id,g.title,g.thumb,' . '(select ifnull(sum(og.price),0) from  ' . tablename('sea_order_goods') . ' og left join ' . tablename('sea_order') . " o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition})  as money," . '(select ifnull(sum(og.total),0) from  ' . tablename('sea_order_goods') . ' og left join ' . tablename('sea_order') . " o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition}) as count  " . 'from ' . tablename('sea_goods') . ' g  ' . "where 1 {$condition1}  order by {$orderby} desc ";
if (empty($_GPC['export'])) {
    $sql .= 'LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
}
$list = pdo_fetchall($sql, $params1);
$total = pdo_fetchcolumn('select  count(*) from ' . tablename('sea_goods') . ' g ' . " where 1 {$condition1} ", $params1);
