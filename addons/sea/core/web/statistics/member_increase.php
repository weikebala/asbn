...
if (!empty($days)) {
	...
	for ($i = $days; $i >= 0; $i--) {
		...
		$datas[] = array('date' => $time, 'mcount' => pdo_fetchcolumn("select count(*) from " . tablename('sea_member') . " where isagent=0  {$condition}", $params), 'acount' => pdo_fetchcolumn("select count(*) from " . tablename('sea_member') . " where isagent=1  {$condition}", $params));
	}
	...
	for ($d = 1; $d <= $lastday; $d++) {
		...
		$datas[] = array('date' => "{$d}日", 'mcount' => pdo_fetchcolumn("select count(*) from " . tablename('sea_member') . " where isagent=0  {$condition}", $params), 'acount' => pdo_fetchcolumn("select count(*) from " . tablename('sea_member') . " where isagent=1  {$condition}", $params));
	}
} else if (!empty($year)) {
	...
	foreach ($months as $m) {
		...
		$datas[] = array('date' => $m['data'] . "月", 'mcount' => pdo_fetchcolumn("select count(*) from " . tablename('sea_member') . " where isagent=0  {$condition}", $params), 'acount' => pdo_fetchcolumn("select count(*) from " . tablename('sea_member') . " where isagent=1  {$condition}", $params));
	}
}
load()->func('tpl');
