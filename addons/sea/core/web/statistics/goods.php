...
$sql = "select og.price,og.total,o.createtime,o.ordersn,g.title,g.thumb,g.goodssn,op.goodssn as optiongoodssn from " . tablename('sea_order_goods') . ' og ' . " left join " . tablename('sea_order') . " o on o.id = og.orderid " . " left join " . tablename('sea_goods') . " g on g.id = og.goodsid " . " left join " . tablename('sea_goods_option') . " op on op.id = og.optionid " . " where 1 {$condition} order by {$orderby} desc ";
...
unset($row);
$total = pdo_fetchcolumn("select  count(*) from " . tablename('sea_order_goods') . ' og ' . " left join " . tablename('sea_order') . " o on o.id = og.orderid " . " left join " . tablename('sea_goods') . " g on g.id = og.goodsid " . " where 1 {$condition}", $params);
