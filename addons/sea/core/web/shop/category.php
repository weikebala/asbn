...
$children  = array();
$category  = pdo_fetchall("SELECT * FROM " . tablename('sea_category') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY parentid ASC, displayorder DESC");
...
if ($operation == 'display') {
    ...
    if (!empty($id)) {
        ...
        $item     = pdo_fetch("SELECT * FROM " . tablename('sea_category') . " WHERE id = '$id' limit 1");
    ...
    if (!empty($parentid)) {
        $parent = pdo_fetch("SELECT id, parentid, name FROM " . tablename('sea_category') . " WHERE id = '$parentid' limit 1");
        if (empty($parent)) {
            message('抱歉，上级分类不存在或是已经被删除！', $this->createWebUrl('post'), 'error');
        }
        if (!empty($parent['parentid'])) {
            $parent1 = pdo_fetch("SELECT id, name FROM " . tablename('sea_category') . " WHERE id = '{$parent['parentid']}' limit 1");
        }
    }
    if (empty($parent)) {
        $level = 1;
    ...
    if (checksubmit('submit')) {
        ...
        $activity_url = $level==1?trim($_GPC['activity_url']):"";
        $data = array(
            'uniacid' => $_W['uniacid'],
        ...
        if (!empty($id)) {
            unset($data['parentid']);
            pdo_update('sea_category', $data, array(
                'id' => $id
            ));
            load()->func('file');
    ...
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id, name, parentid FROM " . tablename('sea_category') . " WHERE id = '$id'");
    if (empty($item)) {
        message('抱歉，分类不存在或是已经被删除！', $this->createWebUrl('shop/category', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_category', array(
        'id' => $id,
        'parentid' => $id
    ), 'OR');
