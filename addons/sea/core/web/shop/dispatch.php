...
if ($operation == "display") { 
    ...
    if (!empty($_GPC["displayorder"])) { ca("shop.dispatch.edit"); foreach ($_GPC["displayorder"] as $id => $displayorder) { pdo_update("sea_dispatch", array("displayorder" => $displayorder), array("id" => $id)); } plog("shop.dispatch.edit","批量修改配送方式排序"); message("分类排序更新成功！", $this->createWebUrl("shop/dispatch", array("op" => "display")), "success"); 
    } 
    $list = pdo_fetchall("SELECT * FROM " . tablename("sea_dispatch") . " WHERE uniacid = '{$_W["uniacid"]}' and dispatchtype=0 ORDER BY id asc"); 
    ...
    if (checksubmit("submit")) { 
        ...
        if ($data["isdefault"]) { 
            pdo_update("sea_dispatch", array("isdefault" => 0), array("uniacid" => $_W["uniacid"])); 
        } 
        if (!empty($id)) { 
            plog("shop.dispatch.edit","修改配送方式 ID: {$id}"); pdo_update("sea_dispatch", $data, array("id" => $id)); 
        } 
        else { 
            pdo_insert("sea_dispatch", $data); $id = pdo_insertid(); plog("shop.dispatch.add","添加配送方式 ID: {$id}"); 
        } 
        message("更新配送方式成功！", $this->createWebUrl("shop/dispatch", array("op" => "display")), "success"); 
    } 
    $dispatch = pdo_fetch("SELECT * FROM " . tablename("sea_dispatch") . " WHERE id = '$id' and uniacid = '{$_W["uniacid"]}'"); 
    if(!empty($dispatch)){ 
    ...
    if(!is_array($areas)){ 
        require_once sea_INC."json/xml2json.php"; $file = IA_ROOT."/addons/sea/static/js/dist/area/Area.xml"; $content = file_get_contents($file); $json =xml2json::transformXmlStringToJson($content); $areas =json_decode( $json,true); m("cache")->set("areas",$areas,"global"); 
    } 
} elseif ($operation == "delete") { ca("shop.dispatch.delete"); $id = intval($_GPC["id"]); $dispatch = pdo_fetch("SELECT id,dispatchname FROM " . tablename("sea_dispatch") . " WHERE id = '$id' AND uniacid=" . $_W["uniacid"] . ""); if (empty($dispatch)) { message("抱歉，配送方式不存在或是已经被删除！", $this->createWebUrl("shop/dispatch", array("op" => "display")), "error"); } pdo_delete("sea_dispatch", array("id" => $id)); plog("shop.dispatch.delete","删除配送方式 ID: {$id} 名称: {$dispatch["dispatchname"]} "); message("配送方式删除成功！", $this->createWebUrl("shop/dispatch", array("op" => "display")), "success"); 
