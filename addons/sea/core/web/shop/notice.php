...
if ($operation == 'display') {
    ca('shop.notice.view');
    $list = pdo_fetchall("SELECT * FROM " . tablename('sea_notice') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_notice', $data, array(
                'id' => $id
            ));
            plog('shop.notice.edit', "修改公告 ID: {$id}");
        } else {
            pdo_insert('sea_notice', $data);
            $id = pdo_insertid();
    ...
    $notice = pdo_fetch("SELECT * FROM " . tablename('sea_notice') . " WHERE id = '$id' and uniacid = '{$_W['uniacid']}'");
} elseif ($operation == 'delete') {
    ca('shop.notice.delete');
    $id     = intval($_GPC['id']);
    $notice = pdo_fetch("SELECT id,title  FROM " . tablename('sea_notice') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
    if (empty($notice)) {
        message('抱歉，店铺公告不存在或是已经被删除！', $this->createWebUrl('shop/notice', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_notice', array(
        'id' => $id
    ));
