...
if ($operation == 'display') {
    ...
    $list  = pdo_fetchall("SELECT  c.*, o.ordersn,g.title,g.thumb FROM " . tablename('sea_order_comment') . " c  " . " left join " . tablename('sea_goods') . " g on c.goodsid = g.id  " . " left join " . tablename('sea_order') . " o on c.orderid = o.id  " . " WHERE 1 {$condition} ORDER BY createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
    $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('sea_order_comment') . " c  " . " left join " . tablename('sea_goods') . " g on c.goodsid = g.id  " . " left join " . tablename('sea_order') . " o on c.orderid = o.id  " . " WHERE 1 {$condition} ", $params);
    $pager = pagination($total, $pindex, $psize);
} elseif ($operation == 'delete') {
    ca('shop.comment.delete');
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id,goodsid FROM " . tablename('sea_order_comment') . " WHERE id ='$id'");
    if (empty($item)) {
        message('抱歉，评价不存在或是已经被删除！', $this->createWebUrl('shop/comment', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_update('sea_order_comment', array(
        'deleted' => 1
    ), array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    $goods = pdo_fetch('select id,thumb,title from ' . tablename('sea_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':id' => $item['goodsid'],
        ':uniacid' => $_W['uniacid']
    ));
    ...
    ca('shop.comment.add');
    $id      = intval($_GPC['id']);
    $item    = pdo_fetch("SELECT * FROM " . tablename('sea_order_comment') . " WHERE id =:id and uniacid=:uniacid limit 1 ", array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    ...
    if (checksubmit()) {
        ...
        if (!empty($id)) {
            pdo_update('sea_order_comment', $data, array(
                'id' => $id
            ));
            plog('shop.comment.edit', "编辑商品虚拟评价 ID: {$id} 商品ID: {$goods['id']} 商品标题: {$goods['title']}");
        } else {
            pdo_insert('sea_order_comment', $data);
            $id = pdo_insertid();
    ...
    $order = pdo_fetch('select id,ordersn from ' . tablename('sea_order') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':id' => $item['orderid'],
        ':uniacid' => $_W['uniacid']
    ));
    if (checksubmit()) {
