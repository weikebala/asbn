...
if ($operation == 'display') {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('sea_adpc') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_adpc', $data, array(
                'id' => $id
            ));
            plog('shop.adpc.edit', "修改广告 ID: {$id}");
        } else {
            pdo_insert('sea_adpc', $data);
            $id = pdo_insertid();
    ...
    $item = pdo_fetch("select * from " . tablename('sea_adpc') . " where id=:id and uniacid=:uniacid limit 1", array(
        ":id" => $id,
        ":uniacid" => $_W['uniacid']
    ));
    ...
    ca('shop.adpc.delete');
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id,advname FROM " . tablename('sea_adpc') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
    if (empty($item)) {
        message('抱歉，广告不存在或是已经被删除！', $this->createWebUrl('shop/adpc', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_adpc', array(
        'id' => $id
    ));
