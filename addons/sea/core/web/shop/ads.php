...
if ($operation == 'display') {
    $ads = pdo_fetch("SELECT * FROM " . tablename('sea_ads') . " WHERE uniacid = :uniacid",
    ...
    if(empty($ads)){
        ...
        pdo_insert('sea_ads', $data_center);
        pdo_insert('sea_ads', $data_bottom);
    }
    ca('shop.ads.view');
    if (!empty($_GPC['displayorder'])) {
        ca('shop.ads.edit');
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('sea_ads', array(
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('sea_ads') . " WHERE uniacid = '{$_W['uniacid']}'");
    //print_r($list);exit;
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_ads', $data, array(
                'id' => $id
            ));
            plog('shop.ads.edit', "修改幻灯片 ID: {$id}");
        } else {
            pdo_insert('sea_ads', $data);
            $id = pdo_insertid();
    ...
    $item = pdo_fetch("select * from " . tablename('sea_ads') . " where id=:id and uniacid=:uniacid limit 1", array(
        ":id" => $id,
        ":uniacid" => $_W['uniacid']
    ));
}
...
//     ca('shop.ads.delete');
//     $id   = intval($_GPC['id']);
//     $item = pdo_fetch("SELECT id,adsname FROM " . tablename('sea_ads') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
//     if (empty($item)) {
//         message('抱歉，幻灯片不存在或是已经被删除！', $this->createWebUrl('shop/ads', array(
//             'op' => 'display'
//         )), 'error');
//     }
//     pdo_delete('sea_ads', array(
//         'id' => $id
//     ));
