...
if ($operation == "save") {
    ...
    if (empty($setdata)) {
        ...
        pdo_update("sea_sysset", $data, array(
            "uniacid" => $_W["uniacid"]
        ));
    }
    $setdata = pdo_fetch("select * from " . tablename("sea_sysset") . " where uniacid=:uniacid limit 1", array(
...
";
        ...
        if (!empty($value["verifystoreid"])) {
            $value["storeinfo"] = pdo_fetchcolumn("select storename from " . tablename("sea_store") . " where id=:storeid limit 1 ", array(
...
$stores = pdo_fetchall("select id,storename from " . tablename("sea_store") . " where uniacid=:uniacid ", array(
    ":uniacid" => $_W["uniacid"]
));
$paras = array(
    ":uniacid" => $_W["uniacid"]
