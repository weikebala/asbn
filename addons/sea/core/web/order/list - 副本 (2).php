...
if ($operation == 'display') {
    ...
    if ($_GPC['export'] == 1) {
        ...
        if (!empty($agentid) && $level > 0) {
            $columns[] = array('title' => '加盟级别', 'field' => 'level', 'width' => 24);
            $columns[] = array('title' => '加盟佣金', 'field' => 'commission', 'width' => 24);
        }
        foreach ($list as &$row) {
...
//print_r($list);
	...
	//print_r($supplier_uid);
	if(!empty($supplier_uid)){
		//查询地址内是否有该供应商信息
		$supplier_address=pdo_fetch('select * from ' . tablename('sea_refund_address') .' where supplier_uid='.$supplier_uid );
...
function zymfunc_3($_var_21)
{
    ...
    pdo_update('sea_order', array('status' => 2, 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['expresssn']), 'sendtime' => time()), array('id' => $_var_21['id'], 'uniacid' => $_W['uniacid']));
            ...
            if ($_var_30 == -1) {
				...
				if ($_var_30 == 5) {//换货 确认发货 sunkang
					...
					if(!empty($ishhh['rexpress'])){
						$hhinfo=$ishhh['hhinfo'].','.$rexpresscom.$rexpress.$rexpresssn.$ishhh['returntime'];
