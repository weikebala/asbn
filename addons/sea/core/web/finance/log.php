...
if ($op == 'display') {
    ...
    $sql = "select log.id,m.id as mid, m.realname,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname from " . tablename('sea_member_log') . " log " . " left join " . tablename('sea_member') . " m on m.openid=log.openid" . " left join " . tablename('sea_member_group') . " g on m.groupid=g.id" . " left join " . tablename('sea_member_level') . " l on m.level =l.id" . " where 1 {$condition} ORDER BY log.createtime DESC ";
    ...
    $total = pdo_fetchcolumn("select count(*) from " . tablename('sea_member_log') . " log " . " left join " . tablename('sea_member') . " m on m.openid=log.openid and m.uniacid= log.uniacid" . " left join " . tablename('sea_member_group') . " g on m.groupid=g.id" . " left join " . tablename('sea_member_level') . " l on m.level =l.id" . " where 1 {$condition} ", $params);
    $pager = pagination($total, $pindex, $psize);
} else if ($op == 'pay') {
    $id      = intval($_GPC['id']);
    $paytype = $_GPC['paytype'];
    $set     = m('common')->getSysset('shop');
    $log     = pdo_fetch('select * from ' . tablename('sea_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
        ':id' => $id,
        ':uniacid' => $uniacid
    ));
    if (empty($log)) {
