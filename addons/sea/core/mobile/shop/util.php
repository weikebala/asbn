...
if ($operation == 'category') {
	...
	$sql = 'SELECT id,nickname,headimgurl,level,content,createtime, images,append_images,append_content,reply_images,reply_content,append_reply_images,append_reply_content ' . ' FROM ' . tablename('sea_order_comment') . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
...
} else if ($operation == 'recommand') {//商品详情页推荐四个产品
    $goodsid = $_GPC['goodsid'];
    $recom_good = m('goods')->getRecomList(array('pagesize' => 4,"good_id"=>$goodsid));
    if(!empty($recom_good)){
        $goods = array_slice($recom_good,0,4);
    }
	show_json(1, array('list' => $goods));
}
