...
if ($operation == 'display') {
	...
	$list = pdo_fetchall("SELECT * FROM " . tablename('sea_order') . " " . " where 1" . $condition . " ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_order') . " WHERE 1 {$condition}");
	$pager = pagination($total, $pindex, $psize);
	if (!empty($list)) {
		foreach ($list as &$row) {
			$goodsid = pdo_fetchall("SELECT goodsid,total FROM " . tablename('sea_order_goods') . " WHERE orderid = '{$row['id']}'", array(), 'goodsid');
			$goods = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice,o.total,o.optionid FROM " . tablename('sea_order_goods') . " o left join " . tablename('shopping_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$row['id']}'");
	...
	$item = pdo_fetch("SELECT * FROM " . tablename('sea_order') . " WHERE uniacid = '{$_W['uniacid']}' AND from_user = '{$_W['fans']['from_user']}' and id='{$orderid}' limit 1");
	if (empty($item)) {
		message('抱歉，您的订单不存或是已经被取消！', $this->createMobileUrl('myorder'), 'error');
	}
	$goodsid = pdo_fetch("SELECT goodsid,total FROM " . tablename('sea_order_goods') . " WHERE orderid = '{$orderid}'", array(), 'goodsid');
	$goods = pdo_fetchall("SELECT g.id, g.title, g.thumb, g.unit, g.marketprice, o.total,o.optionid FROM " . tablename('sea_order_goods') . " o left join " . tablename('shopping_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$orderid}'");
	$goods = set_medias($goods, 'thumb');
	foreach ($goods as &$g) {
	...
	$orderid = intval($_GPC['orderid']);
	$order = pdo_fetch("SELECT status FROM " . tablename('sea_order') . " WHERE id = :id AND from_user = :from_user", array(':id' => $orderid, ':from_user' => $_W['fans']['from_user']));
	if (empty($order)) {
	...
	pdo_update('sea_order', array('status' => 3), array('uniacid' => $_W['uniacid'], 'id' => $orderid, 'openid' => $openid));
	...
	pdo_update('sea_order', array('status' => -1), array('uniacid' => $_W['uniacid'], 'id' => $orderid, 'openid' => $openid));
