...
$member_shop['total'] = $member['total'];
$uniacid        = $_W['uniacid'];
$goodsid        = intval($_GPC['id']);
$goods          = pdo_fetch("SELECT * FROM " . tablename('sea_goods') . " WHERE id = :id limit 1", array(
    ':id' => $goodsid
));
...
if (p('commission')) {
    if (empty($member['agentblack'])) {
        ...
        if ($opencommission) {
            ...
            $commission_text = empty($cset['buttontext']) ? '我要加盟' : $cset['buttontext'];
        }
    }
}

if (p('supplier')){
   $supplier_uid = pdo_fetchcolumn("select supplier_uid from " . tablename('sea_goods') . " where id=:id ", array(':id' => $goodsid));
   $supplier_openid = pdo_fetchcolumn("select openid from " . tablename('sea_perm_user') . " where uid=:id ", array(':id' => $supplier_uid ));
   $supplier_info = m('member')->getInfo($supplier_openid);
   $supplier_info['supplier_uid'] = $supplier_uid;
   //print_r($supplier_info['supplier_uid']);
...
if ($diyform_plugin) {
    ...
    if ($_W['isajax'] && $_GPC['op'] == 'create') {
        ...
        if (empty($goods_temp)) {
            pdo_insert('sea_diyform_temp', $insert);
            $gdid = pdo_insertid();
        } else {
            pdo_update('sea_diyform_temp', $insert, array(
                'id' => $goods_temp['id']
            ));
            $gdid = $goods_temp['id'];
        }
...
if ($_W['isajax']) {
    ...
    if ($goods['usermaxbuy'] > 0) {
        $order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('sea_order_goods') . ' og ' . ' left join ' . tablename('sea_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(
            ':goodsid' => $goodsid,
            ':uniacid' => $uniacid,
            ':openid' => $openid
        ));
    ...
    if (!empty($goods['hasoption'])) {
        $allspecs = pdo_fetchall("select * from " . tablename('sea_goods_spec') . " where goodsid=:id order by displayorder asc", array(
            ':id' => $goodsid
        ));
        foreach ($allspecs as &$s) {
            $items      = pdo_fetchall("select * from " . tablename('sea_goods_spec_item') . " where  `show`=1 and specid=:specid order by displayorder asc", array(
                ":specid" => $s['id']
            ));
    ...
    $options = array();
    if (!empty($goods['hasoption'])) {
        $options = pdo_fetchall("select id,title,thumb,marketprice,productprice,costprice, stock,weight,specs from " . tablename('sea_goods_option') . " where goodsid=:id order by id asc", array(
            ':id' => $goodsid
        ));
    ...
    $specs  = $allspecs;
    $params = pdo_fetchall("SELECT * FROM " . tablename('sea_goods_param') . " WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc", array(
        ':uniacid' => $uniacid,
        ":goodsid" => $goods['id']
    ));
    ...
    $fcount = pdo_fetchcolumn('select count(*) from ' . tablename('sea_member_favorite') . ' where uniacid=:uniacid and openid=:openid and goodsid=:goodsid and deleted=0 ', array(
        ':uniacid' => $uniacid,
        ':openid' => $openid,
        ':goodsid' => $goods['id']
    ));
    pdo_query('update ' . tablename('sea_goods') . " set viewcount=viewcount+1 where id=:id and uniacid='{$uniacid}' ", array(
        ":id" => $goodsid
    ));
    $history = pdo_fetchcolumn('select count(*) from ' . tablename('sea_member_history') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid and deleted=0 limit 1', array(
        ':goodsid' => $goodsid,
        ':uniacid' => $uniacid,
        ':openid' => $openid
    ));
    if ($history <= 0) {
        ...
        pdo_insert('sea_member_history', $history);
    }
    ...
    if ($goods['isverify'] == 2) {
        ...
        if (empty($storeids)) {
            $stores = pdo_fetchall('select * from ' . tablename('sea_store') . ' where  uniacid=:uniacid and status=1', array(
                ':uniacid' => $_W['uniacid']
            ));
        } else {
            $stores = pdo_fetchall('select * from ' . tablename('sea_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1', array(
                ':uniacid' => $_W['uniacid']
            ));
        }
    }
    ...
    if ($sale_plugin) {
		...
		if(!empty($saleset['enoughszk'])){
                $arrSort = array();
                foreach($saleset['enoughszk'] as $key=>$value){
                    foreach($value as $k=>$v){
                        $arrSort[$k][$key] = $v;
                    }
                }
                array_multisort($arrSort['enoughzk'],constant("SORT_ASC"),$saleset['enoughszk']);
        }
    }
    $ret        = array(
        'goods' => $goods,
        ...
        'level' => $level,
        'shop' => $shop,
        'goodscount' => pdo_fetchcolumn('select count(*) from ' . tablename('sea_goods') . ' where uniacid=:uniacid and status=1 and deleted=0 ', array(
            ':uniacid' => $uniacid
        )),
        'cartcount' => pdo_fetchcolumn('select sum(total) from ' . tablename('sea_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 ', array(
            ':uniacid' => $uniacid,
            ':openid' => $openid
        )),
        'isfavorite' => $fcount > 0,
