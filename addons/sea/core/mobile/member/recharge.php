...
if ($operation == 'display' && $_W['isajax']) {
    ...
    pdo_delete('sea_member_log', array(
        'openid' => $openid,
        'status' => 0,
        'type' => 0,
        'uniacid' => $_W['uniacid']
	...
	if($log['money'] <= 0){
        pdo_update('sea_member_log', array('money' => $money, 'couponid' => $couponid), array('id' => $log['id']));
    }
    ...
    $sign      = trim($_GPC['sign']);
    if (empty($logno)) {
        $url = $this->createMobileUrl('order/payresult',array("ordersn"=>$logno));
    ...
    $log = pdo_fetch('SELECT * FROM ' . tablename('sea_member_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(
        ':uniacid' => $uniacid,
        ':logno' => $logno
    ));
    if (!empty($log) && empty($log['status'])) {
