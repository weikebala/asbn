...
if ($operation == 'display' && $_W['isajax']) {
    ...
    $plid = $log['plid'];
    if (empty($log)) {
        $log = array('uniacid' => $uniacid, 'openid' => $member['uid'], 'module' => 'sea', 'tid' => $order['ordersn'], 'fee' => $order['price'], 'status' => 0);
    ...
    $abcchinapay = array('success' => false);
    if (isset($set['pay']) && $set['pay']['abcchinapay'] == 1) {
        if (is_array($setting['payment']['abcchinapay']) && $setting['payment']['abcchinapay']['switch']) {
            $abcchinapay['success'] = true;
        }
    }
    ...
    $order_goods = pdo_fetchall('select og.id,g.title, og.goodsid,og.optionid,g.thumb, g.total as stock,og.total as buycount,g.status,g.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups from  ' . tablename('sea_order_goods') . ' og ' . ' left join ' . tablename('sea_goods') . ' g on og.goodsid = g.id ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
    foreach ($order_goods as $key => &$value) {
        if (!empty($value['optionid'])) {
            $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,virtual,weight from ' . tablename('sea_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':goodsid' => $value['goodsid'], ':id' => $value['optionid']));
            if (!empty($option)) {
    ...
    show_json(1, array('order' => $order, 'set' => $set, 'credit' => $credit, 'wechat' => $wechat, 'alipay' => $alipay, 'unionpay' => $unionpay, 'abcchinapay' => $abcchinapay, 'yunpay' => $yunpay, 'cash' => $cash, 'isweixin' => is_weixin(), 'currentcredit' => $currentcredit, 'returnurl' => $returnurl, 'goods' => $order_goods));
} else {
    if ($operation == 'pay' && $_W['ispost']) {
        ...
        $type = $_GPC['type'];
        if (!in_array($type, array('weixin', 'alipay', 'unionpay', 'yunpay','abcchinapay'))) {
            show_json(0, '未找到支付方式');
        }
        ...
        foreach ($order_goods as $data) {
            ...
            if (!empty($data['optionid'])) {
                $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,virtual from ' . tablename('sea_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':id' => $data['optionid']));
                if (!empty($option)) {
                    if ($option['stock'] != -1) {
        ...
        if ($type == 'weixin') {
            ...
            if (is_weixin()) {
                ...
                pdo_update('sea_order', array('paytype' => 21), array('id' => $order['id']));
            ...
            if ($type == 'alipay') {
                pdo_update('sea_order', array('paytype' => 22), array('id' => $order['id']));
                show_json(1);
            } else {
                if ($type == 'yunpay') {
                    pdo_update('sea_order', array('paytype' => 24), array('id' => $order['id']));
                    show_json(1);
                }
            }
        }
        if ($type == 'abcchinapay') {
            pdo_update('sea_order', array('paytype' => 29), array('id' => $order['id']));
            show_json(1);
        }
    } else {
        if ($operation == 'complete' && $_W['ispost']) {
            ...
            $type = $_GPC['type'];
            if (!in_array($type, array('weixin', 'alipay', 'credit', 'cash', 'abcchinapay'))) {
                show_json(0, '未找到支付方式');
            }
            if ($member['credit2'] < $order['deductcredit2'] && $order['deductcredit2'] > 0) {
                show_json(0, '余额不足，请充值后在试！');
            ...
            $plid = $log['plid'];
            if ($type == 'cash') {
                pdo_update('sea_order', array('paytype' => 3), array('id' => $order['id']));
                $ret = array();
                $ret['result'] = 'success';
                $ret['type'] = 'cash';
                $ret['from'] = 'return';
            ...
            if ($type == 'credit') {
                ...
                pdo_update('sea_order', array('paytype' => 1), array('id' => $order['id']));
                $ret = array();
                $ret['result'] = 'success';
                $ret['type'] = $log['type'];
                $ret['from'] = 'return';
...
//                    die('支付出现错误，请重试!');
//                }
                $log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'sea', ':tid' => $tid));
                if (empty($log)) {
                    die('支付出现错误，请重试!');
                }
                if ($log['status'] != 1) {
                ...
                $orderid = pdo_fetchcolumn('select id from ' . tablename('sea_order') . ' where ordersn=:ordersn and uniacid=:uniacid', array(':ordersn' => $log['tid'], ':uniacid' => $_W['uniacid']));
				$openid = m('user')->isLogin();
				if(!$openid){
					$url = $_W['siteroot'] . 'addons/sea/payment/alipay/return.php?ordersn='.$log['tid'];
				}else{
					$url = $this->createMobileUrl('order/payresult', array('ordersn' => $log['tid']));	
				}
                ...
                if ($operation == 'abcchinapay') {
                    ...
                    if ($log['status'] != 1) {
                        $record = array();
                        $record['status'] = '1';
                        $record['type'] = 'abcchinapay';
                        pdo_update('core_paylog', $record, array('plid' => $log['plid']));
                        $ret = array();
                        $ret['result'] = 'success';
                        $ret['type'] = 'abcchinapay';
                        $ret['from'] = 'return';
                        $ret['tid'] = $log['tid'];
                        $ret['user'] = $log['openid'];
                        $ret['fee'] = $log['fee'];
