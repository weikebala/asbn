...
if ($operation == 'index') {
	...
	$adss = pdo_fetchall('select * from ' . tablename('sea_ads') . ' where uniacid=:uniacid', array(':uniacid' => $uniacid));
	$adss = set_medias($adss, 'thumb_1,thumb_2,thumb_3,thumb_4');
	$category = pdo_fetchall('select * from ' . tablename('sea_category'));
	$category = set_medias($category, 'thumb');
	$goods = pdo_fetchall('select * from ' . tablename('sea_goods'));
