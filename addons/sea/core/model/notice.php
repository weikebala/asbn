...
class Sz_DYi_Notice
{
    public function sendOrderMessage($orderid = '0', $delRefund = false)
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('sea_order') . ' where id=:id limit 1', array(
            ':id' => $orderid
        ));
        if (empty($order)) {
            return;
        }
        $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=order&p=detail&id=' . $orderid;
        ...
        $openid      = $order['openid'];
        $order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('sea_order_goods') . ' og ' . ' left join ' . tablename('sea_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        $goods       = '';
        ...
        if ($delRefund) {
            if (!empty($order['refundid'])) {
                $refund = pdo_fetch('select * from ' . tablename('sea_order_refund') . ' where id=:id limit 1', array(
                    ':id' => $order['refundid']
                ));
                if (empty($refund)) {
                    return;
        ...
        if ($order['status'] == -1) {
            if (empty($order['dispatchtype'])) {
                $address      = pdo_fetch('select * from ' . tablename('sea_member_address') . ' where id=:id and uniacid=:uniacid limit 1 ', array(
                    ":uniacid" => $_W['uniacid'],
                    ":id" => $order['addressid']
                ));
                $orderAddress = array(
            ...
            if (strexists($pay_detailurl, '/addons/sea/')) {
                $pay_detailurl = str_replace("/addons/sea/", '/', $pay_detailurl);
            }
            ...
            if (empty($order['dispatchtype'])) {
                ...
                $order_address_deliver_data = iunserializer($order['address']);
                $deliver_name = pdo_fetchcolumn('select name from ' . tablename('tradearea_deliver') . ' where uniacid = :uniacid and id = :id',array(':uniacid'=>$_W['uniacid'],':id'=>$order_address_deliver_data['tradearea_deliver']));
    ...
    public function sendMemberUpgradeMessage($openid = '', $oldlevel = null, $level = null)
    {
        ...
        $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=member';
        if (strexists($detailurl, '/addons/sea/')) {
            $detailurl = str_replace("/addons/sea/", '/', $detailurl);
        }
        if (strexists($detailurl, '/core/mobile/order/')) {
    ...
    {
        global $_W, $_GPC;
        $log_info   = pdo_fetch('select * from ' . tablename('sea_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
            ':id' => $log_id,
            ':uniacid' => $_W['uniacid']
        ));
        ...
        if ($log_info['type'] == 0) {
            ...
            $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=member&p=log&type=1';
            if (strexists($detailurl, '/addons/sea/')) {
                $detailurl = str_replace("/addons/sea/", '/', $detailurl);
            }
