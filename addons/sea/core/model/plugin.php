...
class Sz_DYi_Plugin
{
    ...
    {
        $dbplugin = pdo_fetchall('select * from ' . tablename('sea_plugin') . ' where identity=:identyty limit  1', array(
            ':identity' => $pluginName
        ));
        if (empty($dbplugin)) {
            return false;
    ...
    {
        global $_W;
		$path = IA_ROOT . "/addons/sea/data/perm";
		if (!is_dir($path)) {
			load()->func('file');
			@mkdirs($path);
		}
		...
		if (!is_array($plugins)) {
			$plugins = pdo_fetchall('select * from ' . tablename('sea_plugin') . ' order by displayorder asc');
