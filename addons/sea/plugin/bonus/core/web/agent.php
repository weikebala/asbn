...
if ($operation == 'display') {
    ...
    $agentSql='SELECT id,realname  FROM `fxs_sea_member` WHERE agentlevels=1';
...
elseif($operation=='changeAgent'){
    ...
    $childCheckSql='SELECT COUNT(1) AS s FROM `fxs_sea_member` WHERE id=:id AND agentid=0 limit 1';
    $childCheckResult=pdo_fetchcolumn($childCheckSql,array(':id'=>$id));
    if($childCheckResult==0){
        message('此粉丝已经有推荐人', '', 'error');
    }
    ...
    message('更改成功!', $this->createPluginWebUrl('bonus/agent'), 'success');
}
elseif($operation=='batchChangeAgent'){
    ...
    $childCheckSql='SELECT GROUP_CONCAT(nickname) AS str  FROM `fxs_sea_member` WHERE id IN ('.$id.') AND agentid!=0  GROUP BY agentid!=0 limit 1 ';
    $childCheckResult=pdo_fetchall($childCheckSql);
    if(!empty($childCheckResult)){
        message('粉丝【'.$childCheckResult[0]['str'].'】已经有推荐人', '', 'error');
    }
    $idArray=explode(",",$id);
    ...
    if ($operation == 'detail') {
        ...
        if (checksubmit('submit')) {
            ...
            if (empty($_GPC['oldstatus']) && $data['status'] == 1) {
                ...
                plog('bonus.agent.check', "审核加盟商 <br/>加盟商信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
            }
            ...
            plog('bonus.agent.edit', "修改加盟商 <br/>加盟商信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
            if (!empty($data['bonuslevel'])) {
                $data['bonus_status'] = 1;
            }
            pdo_update('sea_member', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
            if (empty($_GPC['oldstatus']) && $data['status'] == 1) {
        ...
        if ($operation == 'delete') {
            ...
            $id = intval($_GPC['id']);
            $member = pdo_fetch('select * from ' . tablename('sea_member') . ' where uniacid=:uniacid and id=:id limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $id));
            if (empty($member)) {
                message('会员不存在，无法取消加盟商资格!', $this->createPluginWebUrl('bonus/agent'), 'error');
            }
            $agentcount = pdo_fetchcolumn('select count(*) from ' . tablename('sea_member') . ' where  uniacid=:uniacid and agentid=:agentid limit 1 ', array(':uniacid' => $_W['uniacid'], ':agentid' => $id));
            if ($agentcount > 0) {
                message('此会员有下线存在，无法取消加盟商资格!', '', 'error');
            }
            pdo_update('sea_member', array('isagent' => 0, 'status' => 0), array('id' => $_GPC['id']));
            plog('bonus.agent.delete', "取消加盟商资格 <br/>加盟商信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
            ...
            if ($operation == 'agentblack') {
                ...
                if (!empty($black)) {
                    pdo_update('sea_member', array('isagent' => 1, 'status' => 0, 'agentblack' => 1), array('id' => $_GPC['id']));
                    plog('bonus.agent.agentblack', "设置黑名单 <br/>加盟商信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
                    message('设置黑名单成功！', $this->createPluginWebUrl('bonus/agent'), 'success');
                } else {
                    pdo_update('sea_member', array('isagent' => 1, 'status' => 1, 'agentblack' => 0), array('id' => $_GPC['id']));
                    plog('bonus.agent.agentblack', "取消黑名单 <br/>加盟商信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
                ...
                if ($operation == 'user') {
                    ...
                    if ($operation == 'query') {
                        ...
                        if ($operation == 'check') {
                            ...
                            $time = time();
                            pdo_update('sea_member', array('status' => 1, 'agenttime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
                            $this->model->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'agenttime' => $time), TM_COMMISSION_BECOME);
                            if (!empty($member['agentid'])) {
                                $this->model->upgradeLevelByAgent($member['agentid']);
                            }
                            plog('bonus.agent.check', "审核加盟商 <br/>加盟商信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
                            message('审核加盟商成功!', $this->createPluginWebUrl('bonus/agent'), 'success');
                        }
                    }
                }
            }
