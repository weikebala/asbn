...
$sql = 'select distinct cg.mid from ' . tablename('sea_bonus_goods') . ' cg left join  ' . tablename('sea_order') . '  o on o.id=cg.orderid and cg.status=0 left join ' . tablename('sea_order_refund') . " r on r.orderid=o.id and ifnull(r.status,-1)<>-1 where 1 and o.status>=3 and o.uniacid={$_W['uniacid']} and ({$time} - o.finishtime > {$day_times})  ORDER BY o.finishtime DESC,o.status DESC";
    ...
    if ($member['myordermoney'] < $set['consume_withdraw'] || empty($member)) {
        ...
        if ($member['commission_ok'] <= 0) {
            ...
            if (!empty($member['bonuslevel'])) {
                $row['realname'] = pdo_fetchcolumn('select levelname from ' . tablename('sea_bonus_level') . ' where id=' . $member['bonuslevel']);
            } else {
...
if (!empty($_POST)) {
    ...
    foreach ($count as $key => $value) {
        ...
        pdo_insert('sea_bonus_log', array('openid' => $member['openid'], 'uid' => $member['uid'], 'money' => $send_money, 'uniacid' => $_W['uniacid'], 'paymethod' => $set['paymethod'], 'sendpay' => $sendpay, 'status' => 1, 'ctime' => time(), 'send_bonus_sn' => $send_bonus_sn));
        if ($sendpay == 1) {
        ...
        pdo_update('sea_bonus_goods', $bonus_goods, array('mid' => $member['id'], 'uniacid' => $_W['uniacid']));
    }
    if ($islog) {
