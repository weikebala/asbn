...
if (empty($result)) {
    ...
    $sql = 'INSERT INTO ' . tablename('sea_plugin') . ' (`displayorder`,`identity`,`name`,`version`,`author`,`status`) VALUES(' . $displayorder . ',\'bonus\',\'海软分红\',\'1.0\',\'官方\',\'1\');';
    pdo_fetchall($sql);
}
$sql = 'CREATE TABLE IF NOT EXISTS ' . tablename('sea_bonus_goods') . ' (
...
';
pdo_fetchall($sql);
if (!pdo_fieldexists('sea_member', 'bonuslevel')) {
    pdo_fetchall('ALTER TABLE ' . tablename('sea_member') . ' ADD `bonuslevel` INT DEFAULT \'0\' AFTER `agentlevel`, ADD `bonus_status` TINYINT(1) DEFAULT \'0\' AFTER `bonuslevel`;');
}
if (!pdo_fieldexists('sea_member', 'bonus_area')) {
    pdo_fetchall('ALTER TABLE ' . tablename('sea_member') . ' ADD `bonus_area` TINYINT(1) DEFAULT \'0\' AFTER `bonuslevel`, ADD `bonus_province` varchar(50) DEFAULT \'\' AFTER `bonus_area`, ADD `bonus_city` varchar(50) DEFAULT \'\' AFTER `bonus_province`, ADD `bonus_district` varchar(50) DEFAULT \'\' AFTER `bonus_city`, ADD `bonus_area_commission` decimal(10,2) DEFAULT \'0.00\' AFTER `bonus_district`;');
}
if (!pdo_fieldexists('sea_goods', 'bonusmoney')) {
    pdo_fetchall('ALTER TABLE ' . tablename('sea_goods') . ' ADD `bonusmoney` DECIMAL(10,2) DEFAULT \'0.00\' AFTER `costprice`;');
}
if (!pdo_fieldexists('sea_bonus_goods', 'bonus_area')) {
    pdo_fetchall('ALTER TABLE ' . tablename('sea_bonus_goods') . ' ADD `bonus_area` TINYINT(1) DEFAULT \'0\' AFTER `levelid`;');
}
if (!pdo_fieldexists('sea_bonus_level', 'msgtitle')) {
    pdo_query('ALTER TABLE ' . tablename('sea_bonus_level') . ' ADD `msgtitle` varchar(100) DEFAULT \'\';');
}
if (!pdo_fieldexists('sea_bonus_level', 'msgcontent')) {
    pdo_query('ALTER TABLE ' . tablename('sea_bonus_level') . ' ADD `msgcontent` varchar(255) DEFAULT \'\';');
}
message('海软分红插件安装成功', $this->createPluginWebUrl('bonus/agent'), 'success');