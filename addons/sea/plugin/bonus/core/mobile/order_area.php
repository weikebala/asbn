...
if ($_W['isajax']) {
    ...
    $psize = 20;
    $list = array();
    $sql = 'select o.id,o.ordersn,o.openid,o.status,o.createtime,cg.money from ' . tablename('sea_order') . ' o' . ' left join ' . tablename('sea_bonus_goods') . " cg on cg.orderid=o.id where 1 {$conditionq} and cg.bonus_area!=0 and o.uniacid=" . $_W['uniacid'] . " and cg.mid ={$member['id']} ORDER BY o.createtime DESC,o.status DESC  ";
    ...
    if (!empty($list)) {
        foreach ($list as &$row) {
            ...
            if (!empty($this->set['openorderdetail'])) {
                $goods = pdo_fetchall('SELECT og.id,og.ordergoodid,og.money,g.thumb,g.title,og.total,og.optionname from ' . tablename('sea_bonus_goods') . ' og' . ' left join ' . tablename('sea_goods') . ' g on g.id=og.ordergoodid  ' . ' where og.orderid=:orderid and og.uniacid = :uniacid order by og.createtime  desc ', array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
