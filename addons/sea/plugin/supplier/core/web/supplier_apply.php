...
if ($operation == 'display') {
    ...
    $list = pdo_fetchall('select a.*,p.accountname, mobile as telephone, accountbank, banknumber   from ' . tablename('sea_supplier_apply') . ' a left join ' . tablename('sea_perm_user') . ' p on p.uid=a.uid where a.status=0 and p.uniacid=' . $_W['uniacid'] . $where);
    $total = count($list);
} else {
    if ($operation == 'detail') {
        $id = intval($_GPC['id']);
        if (!empty($id)) {
            ...
            $apply = pdo_fetch('select * from ' . tablename('sea_supplier_apply') . ' where id = ' . $id);
            $openid = pdo_fetchcolumn('select openid from ' . tablename('sea_perm_user') . ' where uid=:uid and uniacid=:uniacid', array(':uid' => $apply['uid'], ':uniacid' => $_W['uniacid']));
            if ($apply['type'] == 2) {
            ...
            pdo_update('sea_supplier_apply', $data, array('id' => $id));
