...
global $_W, $_GPC;
$perms = pdo_fetch('select * from ' . tablename('sea_perm_role') . ' where status1 = 1');
...
if ($operation == 'post') {
    ...
    $su_info = pdo_fetch('select * from ' . tablename('sea_perm_user') . ' where uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
    if (!empty($su_info)) {
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        ...
        if (!empty($su_info['id'])) {
            ...
            pdo_update('sea_perm_user', $pwd, array('uid' => $su_info['uid'], 'uniacid' => $_W['uniacid']));
