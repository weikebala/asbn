...
if ($operation == 'af_supplier') {
    $status = $_GPC['status'];
    $id = $_GPC['id'];
    $openid = pdo_fetchcolumn('select openid from ' . tablename('sea_af_supplier') . " where uniacid={$_W['uniacid']} and id={$id}");
    if (empty($openid)) {
        message('没有该条申请记录', $this->createPluginWebUrl('supplier/supplier_for'), 'error');
    } else {
        pdo_update('sea_af_supplier', array('status' => $status), array('id' => $id, 'uniacid' => $_W['uniacid']));
        ...
        if ($status == 1) {
            ...
            $data = array();
            $msg = '审核通过成功';
            $supplier_usre = pdo_fetch('select * from ' . tablename('sea_af_supplier') . " where uniacid={$_W['uniacid']} and id={$id}");
            ...
            $perm_role = pdo_fetchcolumn('select id,status from ' . tablename('sea_perm_role') . ' where status1=1 and status=1');
...
$sql = 'select * from ' . tablename('sea_af_supplier') . " where 1 and status=0 {$condition}";
