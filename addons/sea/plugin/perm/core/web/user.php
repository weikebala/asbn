...
if ($operation == 'display') {
	...
	$list = pdo_fetchall('SELECT u.*,r.rolename FROM ' . tablename('sea_perm_user') . ' u  ' . ' left join ' . tablename('sea_perm_role') . ' r on u.roleid =r.id  ' . " WHERE 1 {$condition} ORDER BY id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
	$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('sea_perm_user') . ' u  ' . ' left join ' . tablename('sea_perm_role') . ' r on u.roleid =r.id  ' . " WHERE 1 {$condition} ", $params);
	$pager = pagination($total, $pindex, $psize);
	$roles = pdo_fetchall('select id,rolename from ' . tablename('sea_perm_role') . ' where uniacid=:uniacid and deleted=0', array(':uniacid' => $_W['uniacid']));
	...
	$item = pdo_fetch('SELECT * FROM ' . tablename('sea_perm_user') . ' WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	...
	if (!empty($item)) {
		...
		$role = pdo_fetch('SELECT * FROM ' . tablename('sea_perm_role') . ' WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['roleid']));
		if (!empty($role)) {
	...
	if ($_W['isajax'] && $_W['ispost']) {
		...
		if (!empty($item['id'])) {
			...
			pdo_update('sea_perm_user', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			if (user_check(array('username' => $data['username']))) {
				...
				//$user= pdo_fetch("SELECT * FROM " . tablename('users') ." where username='".$_GPC['username']."'");
				$data['uid'] = $user['uid'];
				$data['password'] = $user['password'];
			} else {
				$endtime = time() + 365 * 24 * 3600;
				$data['uid'] = user_register(array('username' => $data['username'], 'password' => $_GPC['password'], 'endtime' => $endtime));
				pdo_insert('uni_account_users', array('uid' => $data['uid'], 'uniacid' => $data['uniacid'], 'role' => 'operator'));
			}
			pdo_insert('sea_perm_user', $data);
			$id = pdo_insertid();
	...
	ca('perm.user.delete');
	$id = intval($_GPC['id']);
	$item = pdo_fetch('SELECT id,uid,username FROM ' . tablename('sea_perm_user') . " WHERE id = '$id'");
	if (empty($item)) {
		message('抱歉，操作员不存在或是已经被删除！', $this->createPluginWebUrl('perm/user', array('op' => 'display')), 'error');
	}
	pdo_delete('sea_perm_user', array('id' => $id, 'uniacid' => $_W['uniacid']));
