...
if ($operation == 'display') {
    ...
    if (checksubmit('clearlog')) {
        ...
        pdo_query('delete from ' . tablename('sea_perm_log') . ' where  uniacid=:uniacid and  createtime >=:starttime  AND createtime <=:endtime', array(
            ':uniacid' => $_W['uniacid'],
            ':starttime' => $starttime,
            ':endtime' => $endtime
        ));
    ...
    $list  = pdo_fetchall("SELECT  log.* ,u.username FROM " . tablename('sea_perm_log') . " log  " . " left join " . tablename('users') . " u on log.uid = u.uid  " . " WHERE 1 {$condition} ORDER BY id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
    $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('sea_perm_log') . " log  " . " left join " . tablename('users') . " u on log.uid = u.uid  " . " WHERE 1 {$condition} ", $params);
    $pager = pagination($total, $pindex, $psize);
    $types = $this->model->getLogTypes();
} elseif ($operation == 'delete') {
    ca('perm.log.delete');
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id FROM " . tablename('sea_perm_log') . " WHERE id = '$id'");
    if (empty($item)) {
        message('抱歉，日志不存在或是已经被删除！', $this->createPluginWebUrl('perm/log', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_perm_log', array(
        'id' => $id
    ));
