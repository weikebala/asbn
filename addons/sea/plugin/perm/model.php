...
if (!class_exists('PermModel')) {
	class PermModel extends PluginModel
	{
		...
		private function check($permtype = '')
		{
			...
			$user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,u.perms as userperms,r.perms as roleperms from ' . tablename('sea_perm_user') . ' u ' . ' left join ' . tablename('sea_perm_role') . ' r on u.roleid = r.id ' . ' where uid=:uid limit 1 ', array(':uid' => $uid));
		...
		function check_plugin($pluginname = '')
		{
			...
			$ac_perm = pdo_fetch('select  plugins from ' . tablename('sea_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid));
			if (!empty($ac_perm)) {
