...
$id = intval($_GPC['id']);
$coupon = pdo_fetch('select * from ' . tablename('sea_coupon') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
if (empty($coupon)) {
	if ($_W['isajax']) {
		show_json(-1, '未找到优惠券');
	}
...
if ($op == 'display') {
	...
	//修改优惠劵领取超出错误 rex_He
	$coupon['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $coupon['id'], ':uniacid' => $_W['uniacid']));
	if ($coupon['total'] != -1) {
		if (($coupon['total'] - $coupon['gettotal']) <= 0) {
			show_json(-1, '优惠券数量不足');
		}
	}
	if (!$coupon['canget']) {
	...
	$needpay = false;
	if ($coupon['money'] > 0) {
		pdo_delete('sea_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0, 'paystatus' => 0));
		$needpay = true;
		$lastlog = pdo_fetch('select * from ' . tablename('sea_coupon_log') . ' where couponid=:couponid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':couponid' => $id, ':openid' => $openid, ':uniacid' => $_W['uniacid']));
		if (!empty($lastlog)) {
			show_json(1, array('logid' => $lastlog['id']));
		}
	} else {
		pdo_delete('sea_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0));
	}
	...
	if ($needpay) {
		...
		pdo_update('sea_coupon_log', array('paytype' => $useweixin ? 1 : 0), array('id' => $logid));
		if ($useweixin) {
	...
	$logno = pdo_fetchcolumn('select logno from ' . tablename('sea_coupon_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid']));
