...
if ($_W['isajax']) {
	...
	$psize = 10;
	$time = time();
	$sql = 'select id,timelimit,timedays,timestart,timeend,thumb,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax from ' . tablename('sea_coupon') . ' c ';
...
$category = pdo_fetchall('select * from ' . tablename('sea_coupon_category') . ' where uniacid=:uniacid and status=1 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
$this->model->setShare();
