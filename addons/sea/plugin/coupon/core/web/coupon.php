...
if ($operation == 'display') {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		foreach ($_GPC['displayorder'] as $id => $displayorder) {
			pdo_update('sea_coupon', array('displayorder' => $displayorder), array('id' => $id));
		}
	...
	if ($_GPC['type'] != '') {
        ...
        $condition .= ' AND coupontype in (0,1)';
    }
	$sql = 'SELECT * FROM ' . tablename('sea_coupon') . ' ' . " where  1 and {$condition} ORDER BY displayorder DESC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
	...
	foreach ($list as &$row) {
		...
		$row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
		$row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
	}
	unset($row);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_coupon') . " where 1 and {$condition}", $params);
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			if (!empty($data['pwdkey'])) {
				$pwdkey = pdo_fetchcolumn('SELECT pwdkey FROM ' . tablename('sea_coupon') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		$key = 'sea:coupon:' . $id;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'sea', ':name' => $key));
		if (!empty($data['pwdkey'])) {
			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => $key, 'module' => 'sea', 'displayorder' => 0, 'status' => $data['pwdopen']);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'sea', 'content' => $data['pwdkey'], 'type' => 1, 'displayorder' => 0, 'status' => $data['pwdopen']);
