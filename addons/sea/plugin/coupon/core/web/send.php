...
if (!empty($id)) {
	$coupon = pdo_fetch('SELECT * FROM ' . tablename('sea_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	if (empty($coupon)) {
...
if (checksubmit('submit')) {
	...
	if ($class1 == 1) {
		...
		$members = pdo_fetchall('SELECT openid FROM ' . tablename('sea_member') . " WHERE uniacid = '{$_W['uniacid']}'" . $where, array(), 'openid');
		if (!empty($value1)) {
			$levelname = pdo_fetchcolumn('select levelname from ' . tablename('sea_member_level') . ' where id=:id limit 1', array(':id' => $value1));
		} else {
			$levelname = '全部';
		}
		...
		$members = pdo_fetchall('SELECT openid FROM ' . tablename('sea_member') . " WHERE uniacid = '{$_W['uniacid']}'" . $where, array(), 'openid');
		if (!empty($value1)) {
			$groupname = pdo_fetchcolumn('select groupname from ' . tablename('sea_member_group') . ' where id=:id limit 1', array(':id' => $value1));
		} else {
			$groupname = '全部分组';
		}
		$openids = array_keys($members);
		...
		$members = pdo_fetchall('SELECT openid FROM ' . tablename('sea_member') . " WHERE uniacid = '{$_W['uniacid']}' and isagent=1 and status=1 " . $where, array(), 'openid');
		if ($value1 != '') {
			$levelname = pdo_fetchcolumn('select levelname from ' . tablename('sea_commission_level') . ' where id=:id limit 1', array(':id' => $value1));
		} else {
			$levelname = '全部';
		}
		$openids = array_keys($members);
		$plog = "发放优惠券 ID: {$id}  方式: 加盟商-{$levelname} 人数: " . count($member);
	}
	$mopenids = array();
	...
	$members = pdo_fetchall('select id,openid,nickname from ' . tablename('sea_member') . ' where openid in (' . implode(',', $mopenids) . ") and uniacid={$_W['uniacid']}");
	if (empty($members)) {
		message('未找到发送的会员!', '', 'error');
	}
	if ($coupon['total'] != -1) {
	...
	pdo_update('sea_coupon', $upgrade, array('id' => $coupon['id']));
