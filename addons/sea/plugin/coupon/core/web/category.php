...
if ($operation == 'display') {
	...
	if (!empty($_GPC['catname'])) {
		...
		foreach ($_GPC['catid'] as $k => $v) {
			...
			if (empty($v)) {
				...
				pdo_insert('sea_coupon_category', $data);
				$insert_id = pdo_insertid();
				plog('coupon.category.add', "添加分类 ID: {$insert_id}");
			} else {
				pdo_update('sea_coupon_category', $data, array('id' => $v));
	...
	$list = pdo_fetchall('SELECT * FROM ' . tablename('sea_coupon_category') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
} elseif ($operation == 'delete') {
	ca('coupon.category.delete');
	$id = intval($_GPC['id']);
	$item = pdo_fetch('SELECT id,name FROM ' . tablename('sea_coupon_category') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
	if (empty($item)) {
		message('抱歉，分类不存在或是已经被删除！', $this->createPluginWebUrl('coupon/category', array('op' => 'display')), 'error');
	}
	pdo_delete('sea_coupon_category', array('id' => $id));
