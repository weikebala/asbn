...
if (!class_exists('CouponModel')) {
	class CouponModel extends PluginModel
	{
		...
		{
			global $_W;
			$coupon = pdo_fetch('SELECT id,total FROM ' . tablename('sea_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
			if (empty($coupon)) {
				return 0;
			}
			if ($coupon['total'] == -1) {
			...
			$gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('sea_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $couponid, ':uniacid' => $_W['uniacid']));
		...
		function creditshop($logid = 0)
		{
			...
			$log = pdo_fetch('SELECT * FROM ' . tablename('sea_creditshop_log') . ' WHERE `id`=:id and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
			if (!empty($log)) {
				...
				pdo_insert('sea_coupon_log', $couponlog);
				$data = array('uniacid' => $_W['uniacid'], 'openid' => $log['openid'], 'couponid' => $log['couponid'], 'gettype' => 2, 'gettime' => time());
				pdo_insert('sea_coupon_data', $data);
				$coupon = pdo_fetch('select * from ' . tablename('sea_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
				$coupon = $this->setCoupon($coupon, time());
				$set = $this->getSet();
				$this->sendMessage($coupon, 1, $member, $set['templateid']);
				pdo_update('sea_creditshop_log', array('status' => 3), array('id' => $logid));
			}
		}
		...
		function payResult($logno)
		{
			...
			$log = pdo_fetch('SELECT * FROM ' . tablename('sea_coupon_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
			if (empty($log)) {
			...
			if (empty($log['status'])) {
				...
				$update['status'] = 1;
				pdo_update('sea_coupon_log', $update, array('id' => $log['id']));
			...
			$url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=member';
			if ($coupon['coupontype'] == 0) {
				$url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=shop&p=list';
			} else {
				$url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=member&p=recharge';
			}
			if (strexists($url, '/addons/sea/plugin/coupon/core/mobile/')) {
				$url = str_replace('/addons/sea/plugin/coupon/core/mobile/', '/', $url);
			}
			if (strexists($url, '/addons/sea/')) {
				$url = str_replace('/addons/sea/', '/', $url);
			}
		...
		function sendMessage($coupon, $send_total, $member, $templateid = '', $account = null)
		{
			...
			$url = empty($coupon['respurl']) ? $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=coupon&method=my' : $coupon['respurl'];
		...
		function sendReturnMessage($openid, $coupon)
		{
			...
			$url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=coupon&method=my';
			if (!empty($templateid)) {
				m('message')->sendTplNotice($openid, $templateid, $msg, $url);
			} else {
				m('message')->sendCustomNotice($openid, $msg, $url);
		...
		function useRechargeCoupon($log)
		{
			...
			$data = pdo_fetch('select id,openid,couponid,used from ' . tablename('sea_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['couponid'], ':uniacid' => $_W['uniacid']));
			if (empty($data)) {
				return;
			}
			if (!empty($data['used'])) {
			...
			$coupon = pdo_fetch('select enough,backcredit,backmoney,backredpack from ' . tablename('sea_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
			if (empty($coupon)) {
				return;
			}
			...
			pdo_update('sea_coupon_data', array('used' => 1, 'usetime' => time(), 'ordersn' => $log['logno']), array('id' => $data['id']));
		...
		function useConsumeCoupon($orderid = 0)
		{
			...
			$order = pdo_fetch('select ordersn,createtime,couponid from ' . tablename('sea_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
			if (empty($order)) {
				return;
			}
			$coupon = false;
			...
			pdo_update('sea_coupon_data', array('used' => 1, 'usetime' => $order['createtime'], 'ordersn' => $order['ordersn']), array('id' => $order['couponid']));
		}
		...
		function returnConsumeCoupon($order)
		{
			global $_W;
			if (!is_array($order)) {
				$order = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime from ' . tablename('sea_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($order), ':uniacid' => $_W['uniacid']));
			}
			if (empty($order)) {
				return;
			}
			...
			if (!empty($coupon['returntype'])) {
				if (!empty($coupon['used'])) {
					pdo_update('sea_coupon_data', array('used' => 0, 'usetime' => 0, 'ordersn' => ''), array('id' => $order['couponid']));
		...
		function backConsumeCoupon($order)
		{
			global $_W;
			if (!is_array($order)) {
				$order = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime,virtual from ' . tablename('sea_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($order), ':uniacid' => $_W['uniacid']));
			}
			if (empty($order)) {
				return;
			}
			...
			if ($canback) {
				$ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('sea_order_goods') . ' og ' . ' left join ' . tablename('sea_order') . ' o on o.id=og.orderid ' . ' where o.id=:orderid and o.openid=:openid and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $order['openid'], ':orderid' => $order['id']));
				$backcredit = $coupon['backcredit'];
				if (!empty($backcredit)) {
					if (strexists($backcredit, '%')) {
				...
				pdo_update('sea_coupon_data', array('back' => 1, 'backtime' => time()), array('id' => $order['couponid']));
		...
		{
			global $_W;
			return pdo_fetch('select * from ' . tablename('sea_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		}

		function getCouponByDataID($dataid = 0)
		{
			global $_W;
			$data = pdo_fetch('select id,openid,couponid,used,back,backtime from ' . tablename('sea_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $dataid, ':uniacid' => $_W['uniacid']));
			if (empty($data)) {
				return false;
			}
			$coupon = pdo_fetch('select * from ' . tablename('sea_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
			if (empty($coupon)) {
				return false;
			}
		...
		function setCoupon($row, $time, $withOpenid = true)
		{
			...
			if ($withOpenid) {
				...
				$row['canget'] = true;
				if ($row['getmax'] > 0) {
					$gets = pdo_fetchcolumn('select count(*) from ' . tablename('sea_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and gettype=1 limit 1', array(':couponid' => $row['id'], ':openid' => $openid, ':uniacid' => $_W['uniacid']));
		...
		function setShare()
		{
			...
			$set = $this->getSet();
			$openid = m('user')->getOpenid();
			$url = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&p=coupon&m=sea&do=plugin";
        ...
        //关注赠送优惠券
        public function sendRegCoupon($openid,$uniacid){
            if(empty($openid)) return false;
            if(empty($uniacid)) return false;
            $regcoupon = pdo_get("sea_coupon",array("uniacid"=>$uniacid,"coupontype"=>2));
            if(empty($regcoupon)) return false;

            $members = pdo_fetchall('select id,openid,nickname from ' . tablename('sea_member') . ' where openid="'.$openid.'" and uniacid='.$uniacid);
            if (empty($members)) {
                return false;
            }
            $send_total = 1;
            ...
            foreach ($members as $m) {
                for ($i = 1; $i <= $send_total; $i++) {
                    $log = array('uniacid' => $uniacid, 'openid' => $m['openid'], 'logno' => m('common')->createNO('coupon_log', 'logno', 'CC'), 'couponid' => $regcoupon['id'], 'status' => 1, 'paystatus' => -1, 'creditstatus' => -1, 'createtime' => $time, 'getfrom' => 0);
                    pdo_insert('sea_coupon_log', $log);
                    $logid = pdo_insertid();
                    $data = array('uniacid' => $uniacid, 'openid' => $m['openid'], 'couponid' => $regcoupon['id'], 'gettype' => 0, 'gettime' => $time, 'senduid' => 0);
                    pdo_insert('sea_coupon_data', $data);
                }
                $this->sendMessage($regcoupon, $send_total, $m, $set['templateid'], $account);
            }
            return true;
        }
	}
