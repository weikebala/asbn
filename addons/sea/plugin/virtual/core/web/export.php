...
$type   = pdo_fetch('SELECT * FROM ' . tablename('sea_virtual_type') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(
    ':id' => $typeid,
    ':uniacid' => $_W['uniacid']
));
if (empty($type)) {
...
$list      = pdo_fetchall('SELECT d.*,o.carrier,m.avatar,m.nickname FROM ' . tablename('sea_virtual_data') . " d " . " left join " . tablename('sea_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . " left join " . tablename('sea_order') . ' o on o.id = d.orderid ' . " where  1 {$condition} order by usetime desc", $params);
if (empty($list)) {
