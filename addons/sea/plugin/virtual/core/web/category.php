...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['catname'])) {
        ...
        foreach ($_GPC['catname'] as $id => $catname) {
            if ($id == 'new') {
                ...
                pdo_insert('sea_virtual_category', array(
                    'name' => $catname,
                    'uniacid' => $_W['uniacid']
                ));
                ...
                pdo_update('sea_virtual_category', array(
                    'name' => $catname
                ), array(
                    'id' => $id
                ));
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('sea_virtual_category') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY id DESC");
} elseif ($operation == 'delete') {
    ca('virtual.category.delete');
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id,name FROM " . tablename('sea_virtual_category') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
    if (empty($item)) {
        message('抱歉，分类不存在或是已经被删除！', $this->createPluginWebUrl('virtual/category', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_virtual_category', array(
        'id' => $id
    ));
