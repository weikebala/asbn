...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$list = array();
		if (!empty($total)) {
			$sql = 'SELECT id,title,thumb,subtitle,type,credit,money FROM ' . tablename('sea_creditshop_goods') . '  where 1 ' . $condition . ' ORDER BY createtime DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
