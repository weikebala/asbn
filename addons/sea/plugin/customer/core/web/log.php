...
if ($operation == 'display') {
	...
	$list = pdo_fetchall('select log.*, m.nickname,m.avatar,m.realname,m.mobile,g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype from ' . tablename('sea_creditshop_log') . ' log ' . ' left join ' . tablename('sea_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('sea_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('sea_creditshop_goods') . ' g on g.id = log.goodsid' . " where 1 {$condition} ORDER BY log.createtime desc limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
	$total = pdo_fetchcolumn('select count(log.id) from' . tablename('sea_creditshop_log') . ' log ' . ' left join ' . tablename('sea_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('sea_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('sea_creditshop_goods') . ' g on g.id = log.goodsid' . " where 1 {$condition}", $params);
	foreach ($list as &$row) {
		$row['address'] = array();
