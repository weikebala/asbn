...
if ($op == 'display') {
    ...
    $pages    = pdo_fetchall("SELECT * FROM " . tablename('sea_chooseagent') . " WHERE uniacid= :uniacid  " . "ORDER BY savetime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':uniacid' => $_W['uniacid']
    ));
    $pagesnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sea_chooseagent') . " WHERE uniacid= :uniacid " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $total    = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sea_chooseagent') . " WHERE uniacid= :uniacid " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    ...
    if (!empty($pageid)) {
        ...
        $datas = pdo_fetch("SELECT * FROM " . tablename('sea_chooseagent') . " WHERE uniacid= :uniacid and id=:id", array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $pageid
        ));
    ...
    if ($_W['ispost']) {
        if ($apido == 'savepage') {
            ...
            if (empty($pageid)) {
                ...
                $page = pdo_fetch("SELECT * FROM " . tablename('sea_chooseagent') . " WHERE uniacid= :uniacid and id=:id", array(
                    ':uniacid' => $_W['uniacid'],
                    ':id' => $pageid
                ));
                if (empty($page)) {
                    ...
                    $do = pdo_delete('sea_chooseagent', array(
                        'id' => $pageid
                    ));
                    show_json(1);
                }
