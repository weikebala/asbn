...
if ($operation == 'display') {
	...
	if (checksubmit('submit')) {
		ca('poster.clear');
		load()->func('file');
		@rmdirs(IA_ROOT . '/addons/sea/data/postera/' . $_W['uniacid']);
		@rmdirs(IA_ROOT . '/addons/sea/data/qrcode/' . $_W['uniacid']);
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
		pdo_update('sea_postera_qr', array('mediaid' => ''), array('acid' => $acid));
	...
	$list = pdo_fetchall('SELECT * FROM ' . tablename('sea_postera') . " WHERE 1 {$condition} ORDER BY isdefault desc,createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
	foreach ($list as &$row) {
		$row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_postera_log') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
	}
	unset($row);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_postera') . " where 1 {$condition} ", $params);
	...
	$item = pdo_fetch('SELECT * FROM ' . tablename('sea_postera') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	if (!empty($item)) {
	...
	if (checksubmit('submit')) {
		...
		if ($data['isdefault'] == 1) {
			pdo_update('sea_postera', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
		}
		if (!empty($id)) {
			pdo_update('sea_postera', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			plog('poster.edit', "修改超级海报 ID: {$id}");
		} else {
			pdo_insert('sea_postera', $data);
			$id = pdo_insertid();
			plog('poster.add', "添加超级海报 ID: {$id}");
		}
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'sea', ':name' => 'sea:postera:' . $id));
		$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sea:postera:' . $id, 'module' => 'sea', 'displayorder' => 0, 'status' => $data['status']);
		$keyword_data = array('uniacid' => $_W['uniacid'], 'module' => 'sea', 'content' => trim($data['keyword']), 'type' => 1, 'displayorder' => 0, 'status' => $data['status']);
		if (empty($rule)) {
		...
		$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'sea', ':name' => 'sea:postera:auto'));
		if (empty($ruleauto)) {
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sea:postera:auto', 'module' => 'sea', 'displayorder' => 0, 'status' => 1);
			pdo_insert('rule', $rule_data);
			$rid = pdo_insertid();
			$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'sea', 'content' => 'sea_POSTERA', 'type' => 1, 'displayorder' => 0, 'status' => 1);
			pdo_insert('rule_keyword', $keyword_data);
		}
	...
	if (!empty($item['goodsid'])) {
		$goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('sea_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
	}
	if (empty($item)) {
		$starttime = time();
	...
	ca('poster.delete');
	$id = intval($_GPC['id']);
	$poster = pdo_fetch('SELECT id,title FROM ' . tablename('sea_postera') . " WHERE id = '$id'");
	if (empty($poster)) {
		message('抱歉，海报不存在或是已经被删除！', $this->createPluginWebUrl('postera', array('op' => 'display')), 'error');
	}
	pdo_delete('sea_postera', array('id' => $id, 'uniacid' => $_W['uniacid']));
	pdo_delete('sea_postera_log', array('posterid' => $id, 'uniacid' => $_W['uniacid']));
	...
	$id = intval($_GPC['id']);
	$poster = pdo_fetch('SELECT * FROM ' . tablename('sea_postera') . " WHERE id = '$id'");
	if (empty($poster)) {
		message('抱歉，海报不存在或是已经被删除！', $this->createPluginWebUrl('postera', array('op' => 'display')), 'error');
	}
	pdo_update('sea_postera', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
	pdo_update('sea_postera', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));
