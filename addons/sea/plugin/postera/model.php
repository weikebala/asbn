...
if (!class_exists('PosteraModel')) {
	class PosteraModel extends PluginModel
	{
		...
		public function getQR($_var_14, $_var_15)
		{
			...
			$_var_19 = pdo_fetch('select * from ' . tablename('sea_postera_qr') . ' where openid=:openid and acid=:acid and posterid=:posterid limit 1', array(':openid' => $_var_15['openid'], ':acid' => $_var_10, ':posterid' => $_var_14['id']));
			if (empty($_var_19)) {
				...
				$_var_22 = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => $_var_1, 'model' => 0, 'name' => 'sea_POSTERA_QRCODE', 'keyword' => 'sea_POSTERA', 'expire' => $_var_0, 'createtime' => time(), 'status' => 1, 'url' => $_var_8['url'], 'ticket' => $_var_8['ticket']);
				...
				pdo_insert('sea_postera_qr', $_var_19);
				...
				if (time() > $_var_19['endtime']) {
					...
					pdo_update('sea_postera_qr', array('ticket' => $_var_9, 'qrimg' => $_var_21, 'url' => $_var_8['url'], 'endtime' => $_var_18), array('id' => $_var_19['id']));
		...
		{
			$_var_31 = IA_ROOT . '/addons/sea/static/fonts/msyh.ttf';
		...
		{
			global $_W;
			$_var_39 = IA_ROOT . '/addons/sea/data/postera/' . $_W['uniacid'] . '/';
			if (!is_dir($_var_39)) {
				load()->func('file');
				mkdirs($_var_39);
			}
			if (!empty($_var_19['goodsid'])) {
				$_var_40 = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('sea_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_var_19['goodsid'], ':uniacid' => $_W['uniacid']));
				if (empty($_var_40)) {
			...
			$_var_27 = $_W['siteroot'] . 'addons/sea/data/poster/' . $_W['uniacid'] . '/' . $_var_42;
			if (!$_var_38) {
				return $_var_27;
			}
			if ($_var_19['qrimg'] != $_var_19['current_qrimg'] || empty($_var_19['mediaid']) || empty($_var_19['createtime']) || $_var_19['createtime'] + 3600 * 24 * 3 - 7200 < time()) {
				...
				pdo_update('sea_postera_qr', array('mediaid' => $_var_47, 'createtime' => time()), array('id' => $_var_19['id']));
			}
		...
		public function getQRByTicket($_var_9 = '')
		{
			...
			$_var_50 = pdo_fetchall('select * from ' . tablename('sea_postera_qr') . ' where ticket=:ticket and acid=:acid limit 1', array(':ticket' => $_var_9, ':acid' => $_W['acid']));
		...
		public function checkMember($_var_51 = '')
		{
			...
			if (empty($_var_15)) {
				...
				pdo_update('sea_member', $_var_15, array('id' => $_var_15['id']));
				$_var_15['isnew'] = false;
			}
			return $_var_15;
		}
