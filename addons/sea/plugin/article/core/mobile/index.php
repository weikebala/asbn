...
$aid = intval($_GPC['aid']);
if (!empty($aid)) {
	$article = pdo_fetch("SELECT * FROM " . tablename('sea_article') . " WHERE id=:aid and article_state=1 and uniacid=:uniacid limit 1 ", array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
	...
	if (!empty($article)) {
		...
		if (!empty($openid)) {
			$state = pdo_fetch("SELECT * FROM " . tablename('sea_article_log') . " WHERE openid=:openid and aid=:aid and uniacid=:uniacid limit 1 ", array(':openid' => $openid, ':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
			if (empty($state['id'])) {
				$insert = array('aid' => $aid, 'read' => 1, 'uniacid' => $_W['uniacid'], 'openid' => $openid);
				pdo_insert('sea_article_log', $insert);
				$sid = pdo_insertid();
				pdo_update('sea_article', array('article_readnum' => $article['article_readnum'] + 1), array('id' => $article['id']));
			} else {
				if ($state['read'] < 4) {
					pdo_update('sea_article_log', array('read' => $state['read'] + 1), array('id' => $state['id']));
					pdo_update('sea_article', array('article_readnum' => $article['article_readnum'] + 1), array('id' => $article['id']));
				}
			}
		}
