...
if ($_W['isajax'] && $_W['ispost']) {
	if ($apido == 'delarticle') {
		...
		if (!empty($article)) {
			pdo_delete('sea_article', array('id' => $aid));
			$keyword = pdo_fetch("SELECT * FROM " . tablename('rule_keyword') . " WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ", array(':content' => $article['article_keyword'], ':module' => 'sea', ':uniacid' => $_W['uniacid']));
			if (!empty($keyword)) {
				pdo_delete('rule_keyword', array('id' => $keyword['id']));
				pdo_delete('rule', array('id' => $keyword['rid']));
			}
			$article_log = pdo_fetchall("SELECT * FROM " . tablename('sea_article_log') . " WHERE aid=:aid and uniacid=:uniacid ", array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
			if (!empty($article_log)) {
				pdo_delete('sea_article_log', array('aid' => $article['id']));
			}
			$article_share = pdo_fetchall("SELECT * FROM " . tablename('sea_article_share') . " WHERE aid=:aid and uniacid=:uniacid ", array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
			if (!empty($article_log)) {
				pdo_delete('sea_article_share', array('aid' => $article['id']));
			}
		...
		$cid = intval($_GPC['cid']);
		$category = pdo_fetch("SELECT * FROM " . tablename('sea_article_category') . " WHERE id=:cid and uniacid=:uniacid limit 1 ", array(':cid' => $cid, ':uniacid' => $_W['uniacid']));
		if (!empty($category)) {
			$articles = pdo_fetchall("SELECT * FROM " . tablename('sea_article') . " WHERE article_category=:cid and uniacid=:uniacid ", array(':cid' => $cid, ':uniacid' => $_W['uniacid']));
			if (!empty($articles)) {
				die(json_encode(array("result" => "error2")));
			} else {
				pdo_delete('sea_article_category', array('id' => $cid));
				die(json_encode(array('result' => 'success')));
			}
		} else {
			die(json_encode(array('result' => 'error')));
		...
		$cname = ($_GPC['cname']);
		if (!empty($cname)) {
			$cates = pdo_fetch("SELECT * FROM " . tablename('sea_article_category') . " WHERE category_name=:cname and id<>:cid and uniacid=:uniacid limit 1 ", array(':cid' => $cid, ':cname' => $cname, ':uniacid' => $_W['uniacid']));
			if (!empty($cates)) {
		...
		if (is_array($data)) {
			...
			// print_r($arr);exit;
			if (!empty($arr['id'])) {
				$articlekeyword = pdo_fetchcolumn("SELECT article_keyword FROM " . tablename('sea_article') . " WHERE id=:aid and uniacid=:uniacid limit 1 ", array(':aid' => $arr['id'], ':uniacid' => $_W['uniacid']));
			...
			if (empty($arr['id'])) {
				...
				ca('article.page.add');
				pdo_insert('sea_article', $arr);
				$aid = pdo_insertid();
				$desc = 'insert';
			} else {
				ca('article.page.edit');
				pdo_update('sea_article', $arr, array('id' => $arr['id']));
				$aid = $arr['id'];
				$desc = 'update';
			}
			$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'sea', ':name' => "sea:article:" . $arr['id']));
			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sea:article:' . $arr['id'], 'module' => 'sea', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'sea', 'content' => trim($arr['article_keyword']), 'type' => 1, 'displayorder' => 0, 'status' => 1);
		...
		$kw = $_GPC['kw'];
		$goods = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb,hasoption FROM " . tablename('sea_goods') . " WHERE uniacid= :uniacid and status=1 and deleted=0 AND title LIKE :title ", array(':title' => "%{$kw}%", ':uniacid' => $_W['uniacid']));
		...
		$articles = pdo_fetchall("SELECT a.id,a.article_title,a.article_category,c.category_name FROM " . tablename('sea_article') . " a left join " . tablename('sea_article_category') . " c on c.id=a.article_category WHERE a.uniacid= :uniacid " . $where . " and a.article_title LIKE :title ", array(':title' => "%{$keyword}%", ':uniacid' => $_W['uniacid']));
		...
		if (!empty($arr)) {
			$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "sea文章营销入口设置"));
			if (!empty($rule)) {
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sea文章营销入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			if (empty($rule)) {
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
			} else {
			...
			$sys = pdo_fetch("select * from " . tablename('sea_article_sys') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
			if (empty($sys)) {
				$arr['uniacid'] = $_W['uniacid'];
				pdo_insert('sea_article_sys', $arr);
			} else {
				pdo_update('sea_article_sys', $arr, array('uniacid' => $_W['uniacid']));
			}
			die(json_encode(array('result' => 'success')));
		}
	}
