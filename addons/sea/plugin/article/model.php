	...
	class ArticleModel extends PluginModel
	{
		public function doShare($article, $shareid, $myid)
		{
			...
			if (!empty($myid) && $shareid != $myid && !empty($profile['openid'])) {
				$my_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sea_article_share') . " WHERE aid=:aid and click_user=:click_user and uniacid=:uniacid ", array(':aid' => $article['id'], ':click_user' => $myid, ':uniacid' => $_W['uniacid']));
				if (empty($my_click)) {
					$share_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sea_article_share') . " WHERE aid=:aid and click_user=:share_user and share_user=:click_user and uniacid=:uniacid ", array(':aid' => $article['id'], ':share_user' => $shareid, ':click_user' => $myid, ':uniacid' => $_W['uniacid']));
					if (empty($share_click)) {
						$all_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sea_article_share') . " WHERE aid=:aid and share_user=:share_user and uniacid=:uniacid ", array(':aid' => $article['id'], ':share_user' => $shareid, ':uniacid' => $_W['uniacid']));
						if ($all_click < $article['article_rule_allnum']) {
							...
							$day_click = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sea_article_share') . " WHERE aid=:aid and share_user=:share_user and click_date>:day_start and click_date<:day_end and uniacid=:uniacid ", array(':aid' => $article['id'], ':share_user' => $shareid, ':day_start' => $day_start, ':day_end' => $day_end, ':uniacid' => $_W['uniacid']));
							if ($day_click < $article['article_rule_daynum']) {
								...
								if ($article['article_rule_money_total']>0){
									$sql = "select sum(add_money) from ".tablename('sea_article_share').
								...
								pdo_insert('sea_article_share', $insert); 
		...
		function href_replace($lnk)
		{
			global $_GPC;
			$newlnk = $lnk;
			if (strexists($lnk, 'sea') && !strexists($lnk, '&mid')) {
				if (strexists($lnk, '?')) {
