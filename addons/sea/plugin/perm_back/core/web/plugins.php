...
if ($operation == 'display') {
	...
	$list = pdo_fetchall("SELECT p.*,ac.name FROM " . tablename('sea_perm_plugin') . " p  " . " left join " . tablename('account_wechats') . " ac on p.acid = ac.acid  " . " WHERE 1 {$condition} ORDER BY id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
    foreach ($list as &$row) {
        ...
        if (!empty($aplugins)) {
            $row['plugins'] = pdo_fetchall('select name from ' . tablename('sea_plugin') . ' where identity in (' . implode(',', $aplugins) . ')');
        } else {
            $row['plugins'] = array();
        }
    }
    unset($row);
    $total   = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('sea_perm_plugin') . " p  " . " left join " . tablename('users') . " u on p.uid = u.uid  " . " left join " . tablename('account_wechats') . " ac on p.acid = ac.acid  " . " WHERE 1 {$condition} ", $params);
	...
	$plugins = m('plugin')->getAll();
    $id           = intval($_GPC['id']);
    $item         = pdo_fetch("SELECT * FROM " . tablename('sea_perm_plugin') . " WHERE id =:id limit 1", array(
        ':id' => $id
    ));
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_perm_plugin', $data, array(
                'id' => $id
            ));
        } else {
			
            if (empty($data['type'])) {
                $usercount = pdo_fetchcolumn('select count(*) from ' . tablename('sea_perm_plugin') . ' where uid=:uid limit 1', array(
                    ':uid' => $data['uid']
                ));
                if ($usercount > 0) {
                ...
                $wechatcount = pdo_fetchcolumn('select count(*) from ' . tablename('sea_perm_plugin') . ' where acid=:acid limit 1', array(
                    ':acid' => $data['acid']
                ));
                if ($wechatcount > 0) {
            ...
            pdo_insert('sea_perm_plugin', $data);
            $id = pdo_insertid();
    ...
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id FROM " . tablename('sea_perm_plugin') . " WHERE id = '$id'");
    if (empty($item)) {
        message('抱歉，权限设置不存在或是已经被删除！', $this->createPluginWebUrl('perm/plugins', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_perm_plugin', array(
        'id' => $id
    ));
