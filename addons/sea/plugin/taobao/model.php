...
if (!class_exists('TaobaoModel')) {
    class TaobaoModel extends PluginModel
    {
        ...
        {
            global $_W;
            $g = pdo_fetch('select * from ' . tablename('sea_goods') . ' where uniacid=:uniacid and taobaoid=:taobaoid limit 1', array(':uniacid' => $_W['uniacid'], ':taobaoid' => $itemid));
            if ($g) {
            }
        ...
        function save_goods($item = array(), $taobaourl = '')
        {
            ...
            if (p('supplier')) {
                $_var_49 = pdo_fetch('select * from ' . tablename('sea_perm_user') . " where uniacid={$_W['uniacid']} and uid={$_W['uid']} and roleid=(select id from " . tablename('sea_perm_role') . ' where status1=1)');
                if (empty($_var_49)) {
                    $data['supplier_uid'] = 0;
                } else {
            ...
            $goods = pdo_fetch('select * from ' . tablename('sea_goods') . ' where  taobaoid=:taobaoid and uniacid=:uniacid', array(':taobaoid' => $item['itemId'], ':uniacid' => $_W['uniacid']));
            if (empty($goods)) {
                ...
                pdo_update('sea_goods', $data, array('id' => $goodsid));
            }
            $goods_params = pdo_fetchall('select * from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
            $params = $item['params'];
            $paramids = array();
            $displayorder = 0;
            foreach ($params as $p) {
                $oldp = pdo_fetch('select * from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid and title=:title limit 1', array(':goodsid' => $goodsid, ':title' => $p['title']));
                $paramid = 0;
                $d = array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $p['title'], 'value' => $p['value'], 'displayorder' => $displayorder);
                if (empty($oldp)) {
                    pdo_insert('sea_goods_param', $d);
                    $paramid = pdo_insertid();
                } else {
                    pdo_update('sea_goods_param', $d, array('id' => $oldp['id']));
                    $paramid = $oldp['id'];
                }
                $paramids[] = $paramid;
                $displayorder++;
            ...
            if (count($paramids) > 0) {
                pdo_query('delete from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid and id not in (' . implode(',', $paramids) . ')', array(':goodsid' => $goodsid));
            } else {
                pdo_query('delete from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
            }
            $specs = $item['specs'];
            $specids = array();
            $displayorder = 0;
            ...
            foreach ($specs as $spec) {
                $oldspec = pdo_fetch('select * from ' . tablename('sea_goods_spec') . ' where goodsid=:goodsid and propId=:propId limit 1', array(':goodsid' => $goodsid, ':propId' => $spec['propId']));
                $specid = 0;
                $d_spec = array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $spec['title'], 'displayorder' => $displayorder, 'propId' => $spec['propId']);
                if (empty($oldspec)) {
                    pdo_insert('sea_goods_spec', $d_spec);
                    $specid = pdo_insertid();
                } else {
                    pdo_update('sea_goods_spec', $d_spec, array('id' => $oldspec['id']));
                    $specid = $oldspec['id'];
                }
                $d_spec['id'] = $specid;
                $specids[] = $specid;
                ...
                foreach ($spec_items as $spec_item) {
                    ...
                    $oldspecitem = pdo_fetch('select * from ' . tablename('sea_goods_spec_item') . ' where specid=:specid and valueId=:valueId limit 1', array(':specid' => $specid, ':valueId' => $spec_item['valueId']));
                    $spec_item_id = 0;
                    if (empty($oldspecitem)) {
                        pdo_insert('sea_goods_spec_item', $d);
                        $spec_item_id = pdo_insertid();
                    } else {
                        pdo_update('sea_goods_spec_item', $d, array('id' => $oldspecitem['id']));
                ...
                if (count($spec_itemids) > 0) {
                    pdo_query('delete from ' . tablename('sea_goods_spec_item') . ' where specid=:specid and id not in (' . implode(',', $spec_itemids) . ')', array(':specid' => $specid));
                } else {
                    pdo_query('delete from ' . tablename('sea_goods_spec_item') . ' where specid=:specid ', array(':specid' => $specid));
                }
                pdo_update('sea_goods_spec', array('content' => serialize($spec_itemids)), array('id' => $oldspec['id']));
            }
            if (count($specids) > 0) {
                pdo_query('delete from ' . tablename('sea_goods_spec') . ' where goodsid=:goodsid and id not in (' . implode(',', $specids) . ')', array(':goodsid' => $goodsid));
            } else {
                pdo_query('delete from ' . tablename('sea_goods_spec') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
            }
            $minprice = 0;
            ...
            foreach ($options as $o) {
                ...
                $oldoption = pdo_fetch('select * from ' . tablename('sea_goods_option') . ' where goodsid=:goodsid and skuId=:skuId limit 1', array(':goodsid' => $goodsid, ':skuId' => $o['skuId']));
                $option_id = 0;
                if (empty($oldoption)) {
                    pdo_insert('sea_goods_option', $do);
                    $option_id = pdo_insertid();
                } else {
                    pdo_update('sea_goods_option', $do, array('id' => $oldoption['id']));
            ...
            if (count($optionids) > 0) {
                pdo_query('delete from ' . tablename('sea_goods_option') . ' where goodsid=:goodsid and id not in (' . implode(',', $optionids) . ')', array(':goodsid' => $goodsid));
            } else {
                pdo_query('delete from ' . tablename('sea_goods_option') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
            }
            ...
            preg_match('/tfsContent : \'(.*)\'/', $content, $iconv);
            $iconv = iconv('GBK', 'UTF-8', $iconv[1]);
            if (isset($images)) {
                foreach ($images as $img) {
                    $iconv = str_replace($img['taobao'], $img['system'], $iconv);
                }
            }
            $hasoption = 0;
            if (count($options) > 0) {
            ...
            $d = array('content' => $iconv, 'hasoption' => $hasoption);
            if ($minprice > 0) {
                $d['marketprice'] = $minprice;
            }
            pdo_update('sea_goods', $d, array('id' => $goodsid));
        ...
        function saveToLocal($url)
        {
            ...
            $strrchr = strrchr($url, '.');
            if ($strrchr != '.jpeg' && $strrchr != '.gif' && $strrchr != '.jpg' && $ext != '.png') {
                return '';
            }
            $apath = $_W['config']['upload']['attachdir'] . '/';
            $path = 'images/sea/' . $_W['uniacid'] . '/' . date('Y') . '/' . date('m') . '/';
            load()->func('file');
            ...
            fclose($fp2);
            load()->func('file');
            $_var_99 = file_remote_upload($path);
            if ($_var_99 == true) {
                file_delete($path);
            }
            return $path;
