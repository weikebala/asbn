...
if (!class_exists('TaobaoModel')) {
    class TaobaoModel extends PluginModel
    {
        ...
        {
            global $_W;
            $_var_5 = pdo_fetch('select * from ' . tablename('sea_goods') . ' where uniacid=:uniacid and taobaoid=:taobaoid limit 1', array(':uniacid' => $_W['uniacid'], ':taobaoid' => $_var_0));
            if ($_var_5) {
            }
        ...
        function save_goods($_var_12 = array(), $_var_1 = '')
        {
            ...
            $_var_55 = pdo_fetch('select * from ' . tablename('sea_goods') . ' where  taobaoid=:taobaoid and uniacid=:uniacid', array(':taobaoid' => $_var_12['itemId'], ':uniacid' => $_W['uniacid']));
            if (empty($_var_55)) {
                pdo_insert('sea_goods', $_var_10);
                $_var_56 = pdo_insertid();
            } else {
                $_var_56 = $_var_55['id'];
                unset($_var_10['createtime']);
                pdo_update('sea_goods', $_var_10, array('id' => $_var_56));
            }
            $_var_57 = pdo_fetchall('select * from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $_var_56));
            $_var_13 = $_var_12['params'];
            $_var_58 = array();
            $_var_59 = 0;
            foreach ($_var_13 as $_var_41) {
                $_var_60 = pdo_fetch('select * from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid and title=:title limit 1', array(':goodsid' => $_var_56, ':title' => $_var_41['title']));
                $_var_61 = 0;
                $_var_62 = array('uniacid' => $_W['uniacid'], 'goodsid' => $_var_56, 'title' => $_var_41['title'], 'value' => $_var_41['value'], 'displayorder' => $_var_59);
                if (empty($_var_60)) {
                    pdo_insert('sea_goods_param', $_var_62);
                    $_var_61 = pdo_insertid();
                } else {
                    pdo_update('sea_goods_param', $_var_62, array('id' => $_var_60['id']));
                    $_var_61 = $_var_60['id'];
                }
                $_var_58[] = $_var_61;
                $_var_59++;
            ...
            if (count($_var_58) > 0) {
                pdo_query('delete from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid and id not in (' . implode(',', $_var_58) . ')', array(':goodsid' => $_var_56));
            } else {
                pdo_query('delete from ' . tablename('sea_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $_var_56));
            }
            ...
            foreach ($_var_16 as $_var_23) {
                $_var_65 = pdo_fetch('select * from ' . tablename('sea_goods_spec') . ' where goodsid=:goodsid and propId=:propId limit 1', array(':goodsid' => $_var_56, ':propId' => $_var_23['propId']));
                $_var_66 = 0;
                $_var_67 = array('uniacid' => $_W['uniacid'], 'goodsid' => $_var_56, 'title' => $_var_23['title'], 'displayorder' => $_var_59, 'propId' => $_var_23['propId']);
                if (empty($_var_65)) {
                    pdo_insert('sea_goods_spec', $_var_67);
                    $_var_66 = pdo_insertid();
                } else {
                    pdo_update('sea_goods_spec', $_var_67, array('id' => $_var_65['id']));
                    $_var_66 = $_var_65['id'];
                }
                $_var_67['id'] = $_var_66;
                $_var_63[] = $_var_66;
                ...
                foreach ($_var_21 as $_var_22) {
                    ...
                    $_var_71 = pdo_fetch('select * from ' . tablename('sea_goods_spec_item') . ' where specid=:specid and valueId=:valueId limit 1', array(':specid' => $_var_66, ':valueId' => $_var_22['valueId']));
                    $_var_72 = 0;
                    if (empty($_var_71)) {
                        pdo_insert('sea_goods_spec_item', $_var_62);
                        $_var_72 = pdo_insertid();
                    } else {
                        pdo_update('sea_goods_spec_item', $_var_62, array('id' => $_var_71['id']));
                        $_var_72 = $_var_71['id'];
                    }
                    $_var_69++;
                    $_var_68[] = $_var_72;
                ...
                $_var_64[] = $_var_67;
                if (count($_var_68) > 0) {
                    pdo_query('delete from ' . tablename('sea_goods_spec_item') . ' where specid=:specid and id not in (' . implode(',', $_var_68) . ')', array(':specid' => $_var_66));
                } else {
                    pdo_query('delete from ' . tablename('sea_goods_spec_item') . ' where specid=:specid ', array(':specid' => $_var_66));
                }
                pdo_update('sea_goods_spec', array('content' => serialize($_var_68)), array('id' => $_var_65['id']));
            }
            if (count($_var_63) > 0) {
                pdo_query('delete from ' . tablename('sea_goods_spec') . ' where goodsid=:goodsid and id not in (' . implode(',', $_var_63) . ')', array(':goodsid' => $_var_56));
            } else {
                pdo_query('delete from ' . tablename('sea_goods_spec') . ' where goodsid=:goodsid ', array(':goodsid' => $_var_56));
            }
            $_var_73 = 0;
            ...
            foreach ($_var_17 as $_var_40) {
                ...
                $_var_81 = pdo_fetch('select * from ' . tablename('sea_goods_option') . ' where goodsid=:goodsid and skuId=:skuId limit 1', array(':goodsid' => $_var_56, ':skuId' => $_var_40['skuId']));
                $_var_82 = 0;
                if (empty($_var_81)) {
                    pdo_insert('sea_goods_option', $_var_80);
                    $_var_82 = pdo_insertid();
                } else {
                    pdo_update('sea_goods_option', $_var_80, array('id' => $_var_81['id']));
                    $_var_82 = $_var_81['id'];
                }
                $_var_59++;
                $_var_74[] = $_var_82;
            ...
            if (count($_var_74) > 0) {
                pdo_query('delete from ' . tablename('sea_goods_option') . ' where goodsid=:goodsid and id not in (' . implode(',', $_var_74) . ')', array(':goodsid' => $_var_56));
            } else {
                pdo_query('delete from ' . tablename('sea_goods_option') . ' where goodsid=:goodsid ', array(':goodsid' => $_var_56));
            }
            ...
            pdo_update('sea_goods', $_var_62, array('id' => $_var_56));
        ...
        function saveToLocal($_var_6)
        {
            ...
            $_var_95 = 'images/sea/' . $_W['uniacid'] . '/' . date('Y') . '/' . date('m') . '/';
            load()->func('file');
