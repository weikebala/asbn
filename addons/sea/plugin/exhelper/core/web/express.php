...
if ($op == 'list') {
    ...
    if (!empty($cate)) {
        ...
        $list = pdo_fetchall('SELECT * FROM ' . tablename('sea_exhelper_express') . " WHERE type=:type and uniacid=:uniacid {$condition} ORDER BY isdefault desc , id DESC", array(':type' => $cate, ':uniacid' => $_W['uniacid']));
    }
} elseif ($op == 'delete') {
    $id = intval($_GPC['id']);
    $type = $_GPC['type'];
    ...
    $item = pdo_fetch('SELECT id,type FROM ' . tablename('sea_exhelper_express') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if (empty($item)) {
        message('抱歉，模不存在或是已经被删除！', $this->createPluginWebUrl('exhelper/express', array('op' => 'list' . $type)), 'error');
    }
    pdo_delete('sea_exhelper_express', array('id' => $id));
    ...
    $item = pdo_fetch('SELECT id,expressname,type FROM ' . tablename('sea_exhelper_express') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if (empty($item)) {
        message('抱歉，快递单不存在或是已经被删除！', $this->createPluginWebUrl('exhelper/express', array('op' => 'list' . $type)), 'error');
    }
    pdo_update('sea_exhelper_express', array('isdefault' => 0), array('type' => $type, 'uniacid' => $_W['uniacid']));
    pdo_update('sea_exhelper_express', array('isdefault' => 1), array('id' => $id));
    ...
    $printset = pdo_fetch('SELECT * FROM ' . tablename('sea_exhelper_sys') . ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
    if (empty($id)) {
        if ($cate == 1) {
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        ...
        if (!empty($id)) {
            pdo_update('sea_exhelper_express', $data, array('id' => $id));
            if ($cate == 1) {
            ...
            $data['type'] = $cate;
            pdo_insert('sea_exhelper_express', $data);
            $id = pdo_insertid();
            if ($cate == 1) {
        ...
        if (!empty($data['isdefault'])) {
            pdo_update('sea_exhelper_express', array('isdefault' => 0), array('type' => $cate, 'uniacid' => $_W['uniacid']));
            pdo_update('sea_exhelper_express', array('isdefault' => 1), array('type' => $cate, 'id' => $id));
        }
        die(json_encode(array('id' => $id)));
    }
    $item = pdo_fetch('select * from ' . tablename('sea_exhelper_express') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
    if (!empty($item)) {
