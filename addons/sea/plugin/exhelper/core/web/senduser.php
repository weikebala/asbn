...
if ($operation == 'display') {
    ...
    $list = pdo_fetchall('SELECT * FROM ' . tablename('sea_exhelper_senduser') . " WHERE uniacid = '{$_W['uniacid']}' {$condition} ORDER BY isdefault desc , id DESC");
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_exhelper_senduser', $data, array('id' => $id));
            plog('exhelper.senduser.edit', "修改快递单信息 ID: {$id}");
        } else {
            pdo_insert('sea_exhelper_senduser', $data);
            $id = pdo_insertid();
            plog('exhelper.senduser.add', "添加快递单信息 ID: {$id}");
        }
        if (!empty($data['isdefault'])) {
            pdo_update('sea_exhelper_senduser', array('isdefault' => 0), array('uniacid' => $_W['uniacid']));
            pdo_update('sea_exhelper_senduser', array('isdefault' => 1), array('id' => $id));
        }
        message('更新模板成功！', $this->createPluginWebUrl('exhelper/senduser', array('op' => 'display')), 'success');
    }
    $item = pdo_fetch('select * from ' . tablename('sea_exhelper_senduser') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
} elseif ($operation == 'delete') {
    ca('exhelper.senduser.delete');
    $id = intval($_GPC['id']);
    $item = pdo_fetch('SELECT id,sendername FROM ' . tablename('sea_exhelper_senduser') . " WHERE id = '{$id}' AND uniacid=" . $_W['uniacid'] . '');
    if (empty($item)) {
        message('抱歉，模板不存在或是已经被删除！', $this->createPluginWebUrl('exhelper/senduser', array('op' => 'display')), 'error');
    }
    pdo_delete('sea_exhelper_senduser', array('id' => $id));
