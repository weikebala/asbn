...
$is_tradearea = pdo_fetchcolumn('select is_tradearea from ' . tablename('tradearea_set') . ' where uniacid = :uniacid ',array(':uniacid'=>$_W['uniacid']));
...
if ($op == 'search') {
    ...
    if (!empty($_GPC['province'])) {
        $_GPC['province'] = trim($_GPC['province']);
        $condition .= " AND a.province = '{$_GPC['province']}'";
        if (!empty($_GPC['city'])) {
            $_GPC['city'] = trim($_GPC['city']);
            $condition .= " AND a.city = '{$_GPC['city']}'";
            if (!empty($_GPC['district'])) {
                $_GPC['district'] = trim($_GPC['district']);
                $condition .= " AND a.area = '{$_GPC['district']}'";
                if (isset($_GPC['tradeaera_area'])) {

                    $condition .= " AND a.tradearea_id = {$_GPC['tradeaera_area']}";
    ...
    $sql = 'select o.* ,a.realname,m.nickname, d.dispatchname,m.nickname,r.status as refundstatus from ' . tablename('sea_order') . ' o' . ' left join ' . tablename('sea_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('sea_member') . ' m on m.openid=o.openid ' . ' left join ' . tablename('sea_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('sea_dispatch') . ' d on d.id = o.dispatchid ' . " where {$condition} ORDER BY o.createtime DESC,o.status DESC  ";
    ...
    if ($type == 1) {
        ...
        foreach ($orders as $order) {
            ...
            if (!is_array($order_address)) {
                $member_address = pdo_fetch('SELECT * FROM ' . tablename('sea_member_address') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
    ...
    $paytype = array('0' => array('css' => 'default', 'name' => '未支付'), '1' => array('css' => 'danger', 'name' => '余额支付'), '11' => array('css' => 'default', 'name' => '后台付款'), '2' => array('css' => 'danger', 'name' => '在线支付'), '21' => array('css' => 'success', 'name' => '微信支付'), '22' => array('css' => 'warning', 'name' => '支付宝支付'), '23' => array('css' => 'warning', 'name' => '银联支付'), '3' => array('css' => 'primary', 'name' => '货到付款'));
    $orderstatus = array('-1' => array('css' => 'default', 'name' => '已关闭'), '0' => array('css' => 'danger', 'name' => '待付款'), '1' => array('css' => 'info', 'name' => '待发货'), '2' => array('css' => 'warning', 'name' => '待收货'), '3' => array('css' => 'success', 'name' => '已完成'));
    $sql = 'select o.* , a.realname,a.mobile,a.province,a.city,a.area,a.address, d.dispatchname,m.nickname,r.status as refundstatus from ' . tablename('sea_order') . ' o' . ' left join ' . tablename('sea_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('sea_member') . ' m on m.openid=o.openid ' . ' left join ' . tablename('sea_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('sea_dispatch') . ' d on d.id = o.dispatchid ' . ' where o.id in ( ' . implode(',', $arr) . ") and o.uniacid={$_W['uniacid']} ORDER BY o.createtime DESC,o.status DESC  ";
    ...
    foreach ($list as &$value) {
        ...
        $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,g.thumb,g.goodssn,og.optionid,g.unit,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle, og.realprice,og.id as ordergoodid,og.printstate,og.printstate2 from ' . tablename('sea_order_goods') . ' og ' . ' left join ' . tablename('sea_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $value['id']));
    ...
    unset($value);
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_order') . ' o ' . ' left join ' . tablename('sea_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('sea_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('sea_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ") and o.uniacid={$_W['uniacid']}", $paras);
    $totalmoney = pdo_fetchcolumn('SELECT sum(o.price) FROM ' . tablename('sea_order') . ' o ' . ' left join ' . tablename('sea_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('sea_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('sea_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ") and o.uniacid={$_W['uniacid']}", $paras);
    $address = false;
    if (!empty($list)) {
    ...
    if ($_W['ispost']) {
        ...
        if (!empty($gid)) {
            $do = pdo_update('sea_goods', array('shorttitle' => $shorttitle), array('id' => $gid));
    ...
    if ($_W['ispost']) {
        ...
        foreach ($arr as $i => $data) {
            ...
            $ordergood = pdo_fetch('SELECT * FROM ' . tablename('sea_order_goods') . ' WHERE id=:id and uniacid=:uniacid limit 1', array('id' => $ordergoodid, ':uniacid' => $_W['uniacid']));
            if ($pt == 'print1') {
                pdo_update('sea_order_goods', array('printstate' => $ordergood['printstate'] + 1), array('id' => $ordergood['id']));
                $orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_order_goods') . ' WHERE orderid=:orderid and printstate=0 and uniacid= :uniacid ', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
            } elseif ($pt == 'print2') {
                pdo_update('sea_order_goods', array('printstate2' => $ordergood['printstate2'] + 1), array('id' => $ordergood['id']));
                $orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_order_goods') . ' WHERE orderid=:orderid and printstate2=0 and uniacid= :uniacid ', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
            }
            if ($orderprint == 0) {
                $printstatenum = 2;
            } else {
            ...
            if ($pt == 'print1') {
                pdo_update('sea_order', array('printstate' => $printstatenum), array('id' => $orderid));
            } elseif ($pt == 'print2') {
                pdo_update('sea_order', array('printstate2' => $printstatenum), array('id' => $orderid));
            }
        }
    ...
    if ($_W['ispost']) {
        ...
        $expSendTemp = pdo_fetch('SELECT * FROM ' . tablename('sea_exhelper_senduser') . ' WHERE isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
        $expTemp = pdo_fetch('SELECT * FROM ' . tablename('sea_exhelper_express') . ' WHERE type=:type and isdefault=1 and uniacid=:uniacid limit 1', array(':type' => $type, ':uniacid' => $_W['uniacid']));
    ...
    if ($_W['ispost']) {
        ...
        foreach ($ordersns as $ordersn) {
            $orderinfo = pdo_fetch('SELECT id,status,expresssn,expresscom FROM ' . tablename('sea_order') . ' WHERE ordersn=:ordersn and status>-1 and uniacid=:uniacid limit 1', array('ordersn' => $ordersn, ':uniacid' => $_W['uniacid']));
            $arr[] = array('ordersn' => $ordersn, 'status' => $orderinfo['status'], 'expresssn' => $orderinfo['expresssn'], 'expresscom' => $orderinfo['expresscom']);
        }
        $printTemp = pdo_fetch('SELECT id,type,expressname,express,expresscom FROM ' . tablename('sea_exhelper_express') . ' WHERE type=:type and isdefault=1 and uniacid=:uniacid limit 1', array(':type' => 1, ':uniacid' => $_W['uniacid']));
    ...
    if ($_W['ispost']) {
        ...
        $orderinfo = pdo_fetch('SELECT * FROM ' . tablename('sea_order') . ' WHERE ordersn=:ordersn and status>-1 and uniacid=:uniacid limit 1', array('ordersn' => $ordersn, ':uniacid' => $_W['uniacid']));
        if (empty($orderinfo)) {
            die(json_encode(array('result' => 'error', 'resp' => '订单不存在')));
        }
        if ($orderinfo['status'] == 1) {
            pdo_update('sea_order', array('express' => trim($express), 'expresssn' => trim($expresssn), 'expresscom' => trim($expresscom), 'sendtime' => time(), 'status' => 2), array('ordersn' => $ordersn));
            if (!empty($orderinfo['refundid'])) {
                $refund = pdo_fetch('select * from ' . tablename('sea_order_refund') . ' where id=:id limit 1', array(':id' => $orderinfo['refundid']));
                if (!empty($refund)) {
                    pdo_update('sea_order_refund', array('status' => -1), array('id' => $orderinfo['refundid']));
                    pdo_update('sea_order', array('refundid' => 0), array('id' => $orderinfo['id']));
                }
            }
    ...
    if ($_W['ispost']) {
        ...
        foreach ($ordersns as $ordersn) {
            pdo_update('sea_order', array('address_send' => $address_send), array('ordersn' => $ordersn));
        }
        die;
    }
} elseif ($op == 'tradearea_area' && $type == 22) {
    $area_list = pdo_fetchall('select id,area from ' . tablename('tradearea') . ' where uniacid = :uniacid and province = :province and city = :city and district = :district ',array(':uniacid'=>$_W['uniacid'],':province'=>$_GPC['province'],':city'=>$_GPC['city'],':district'=>$_GPC['district']));
    if (!empty($area_list)) {
        show_json(1,$area_list);
    }else{
        show_json(0,'没有商圈数据！');
    }
}
