...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['shorttitle'])) {
        ...
        foreach ($_GPC['shorttitle'] as $id => $shorttitle) {
            pdo_update('sea_goods', array('shorttitle' => $shorttitle), array('id' => $id, 'uniacid' => $_W['uniacid']));
        }
    ...
    $sql = 'SELECT COUNT(*) FROM ' . tablename('sea_goods') . $condition;
    $total = pdo_fetchcolumn($sql, $params);
    if (!empty($total)) {
        $sql = 'SELECT id,title,thumb,shorttitle FROM ' . tablename('sea_goods') . $condition . ' ORDER BY `status` DESC, `displayorder` DESC,
