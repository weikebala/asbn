...
if ($op == 'display') {
    ...
    $member = pdo_fetch("select agentid from " . tablename('sea_member') . " where uniacid=".$_W['uniacid']." and id = ".$mid);
    $agent = pdo_fetch("select * from " . tablename('sea_member') . " where uniacid=".$_W['uniacid']." and agentid = ".$member['agentid']);
    ...
    if($userinfo['nickname'] =="" && $userinfo['headimgurl'] == 132){
        ...
        pdo_update('sea_member', $shopdata, array('id' => $agent['id']));
    }
