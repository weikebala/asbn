...
if ($op == 'display') {
    ...
    $sql = "select dm.*,l.levelname,g.groupname,a.nickname as agentnickname,a.avatar as agentavatar from " . tablename('sea_member') . " dm " . " left join " . tablename('sea_member_group') . " g on dm.groupid=g.id" . " left join " . tablename('sea_member') . " a on a.id=dm.agentid" . " left join " . tablename('sea_member_level') . " l on dm.level =l.id" . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}" . " where 1 {$condition}  ORDER BY dm.id DESC";
    ...
    $total           = pdo_fetchcolumn("select count(*) from" . tablename('sea_member') . " dm " . " left join " . tablename('sea_member_group') . " g on dm.groupid=g.id" . " left join " . tablename('sea_member_level') . " l on dm.level =l.id" . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid" . " where 1 {$condition} ", $params);
        ...
        if(!is_error($fan) && $fan['subscribe'] == 1) {
            ...
            if(!empty($row['uid'])) {
                ...
                if(!empty($shopdata)){
                    ...
                    pdo_update('sea_member', $shopdata, array('openid' => $row['openid']));
                }
    ...
    $userinfo = pdo_fetchall("select * from " . tablename('sea_member') . " where avatar=132");
    foreach ($userinfo as $k => $v) {
        $a = array_rand($ims_sea_member);
        $shopdata = array(
            //"nickname" => $ims_sea_member[$a]['nickname'],
            "avatar" => $ims_sea_member[$a]['avatar'],
            );
        pdo_update('sea_member', $shopdata, array('id' => $v['id']));
    }
}
load()->func('tpl');
