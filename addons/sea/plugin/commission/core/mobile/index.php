...
if ($_W['isajax']) {
    ...
    $member['customercount'] = pdo_fetchcolumn('select count(id) from ' . tablename('sea_member') . ' where agentid=:agentid and ((isagent=1 and status=0) or isagent=0) and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
    if (mb_strlen($member['nickname'], 'utf-8') > 6) {
        $member['nickname'] = mb_substr($member['nickname'], 0, 6, 'utf-8');
    }
	$member['sharesurl']  =  $this->createPluginMobileUrl('commission/shares');
	if(!strpos($member['sharesurl'],'&mid') )
	{
		$member['sharesurl']  = $member['sharesurl'].'&mid='.$member['id'];
	}
    $openselect = false;
