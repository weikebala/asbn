...
if ($op == 'display') {
	if ($_W['isajax']) {
		if (empty($shop['selectgoods'])) {
			$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('sea_goods') . ' where uniacid=:uniacid and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
		} else {
			$goodscount = count(explode(",", $shop['goodsids']));
		}
		$advs = pdo_fetchall("select id,advname,link,thumb from " . tablename('sea_adv') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
	...
	if ($_W['isajax']) {
		$args = array('page' => $_GPC['page'], 'pagesize' => 6, 'order' => 'displayorder desc,sales desc', 'by' => '');
		if (!empty($shop['selectgoods'])) {
		...
		$argshot = array('page' => $_GPC['page'], 'pagesize' => 4, 'ishot' => 1, 'order' => 'displayorder desc,sales desc', 'by' => '');
		$hotgoods = m('goods')->getList($argshot);
		$argsisrecommand = array('page' => $_GPC['page'], 'pagesize' => 2, 'isrecommand' => 1, 'order' => 'displayorder desc,sales desc', 'by' => '');
		$isrecommandgoods = m('goods')->getList($argsisrecommand);
		show_json(1, array('goods' => $goods, 'pagesize' => $args['pagesize'], 'hotgoods' => $hotgoods, 'isrecommandgoods' => $isrecommandgoods));
	}
}else if ($op == 'set') {
	if ($_W['isajax']) {
		if ($_W['ispost']) {
			...
			if (empty($shop['id'])) {
				pdo_insert('sea_commission_shop', $shopdata);
			} else {
				pdo_update('sea_commission_shop', $shopdata, array('id' => $shop['id']));
			}
			show_json(1);
		}
		$shop = pdo_fetch('select * from ' . tablename('sea_commission_shop') . ' where uniacid=:uniacid and mid=:mid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
	...
	if ($_W['isajax']) {
		...
		if (!empty($shop['selectgoods'])) {
			$goodsids = explode(',', $shop['goodsids']);
			if (!empty($goodsids)) {
				$goods = pdo_fetchall('select id,title,marketprice,thumb from ' . tablename('sea_goods') . ' where uniacid=:uniacid and id in ( ' . trim($shop['goodsids']) . ')', array(':uniacid' => $_W['uniacid']));
