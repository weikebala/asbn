...
if (empty($infourl)) {
	...
	if (!empty($goodsid)) {
		$goods = pdo_fetch('select * from ' . tablename('sea_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $goodsid));
...
$share_my_Url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=commission&method=myshop&mid=' . $member['id'];
if (empty($infourl) && $_W['isajax']) {
    $p = p('poster');
    $shareUrl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=commission&method=myshop&mid=' . $member['id'];
    if ($share_goods) {
        if ($p) {
