...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$commissioncount = 0;
		$list = pdo_fetchall("select * from " . tablename('sea_commission_apply') . " where 1 {$condition} order by id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('sea_commission_apply') . " where 1 {$condition}", $params);
		foreach ($list as &$row) {
		...
		$id = intval($_GPC['id']);
		$apply = pdo_fetch('select * from ' . tablename('sea_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(':id' => $id, ':mid' => $mid, ':uniacid' => $uniacid));
		if (empty($apply)) {
		...
		$pindex = max(1, intval($_GPC['page']));
		$psize = 5;
		$apply = pdo_fetch('select orderids from ' . tablename('sea_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(':id' => $id, ':mid' => $mid, ':uniacid' => $uniacid));
		if (empty($apply)) {
			show_json(0, array('message' => '未找到提现申请!'));
		}
		$orderids = iunserializer($apply['orderids']);
		...
		$list = pdo_fetchall("select o.id,o.agentid, o.ordersn,o.price,o.goodsprice, o.dispatchprice,o.createtime, o.paytype from " . tablename('sea_order') . ' o ' . " left join " . tablename('sea_member') . " m on o.openid = m.openid " . " where  o.id in ( " . implode(",", $ids) . " ) LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
		$totalcommission = 0;
		$totalpay = 0;
		foreach ($list as &$row) {
			$ordercommission = 0;
