...
ca('commission.cover');
$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "sea加盟中心入口设置"));
if (!empty($rule)) {
...
if (checksubmit('submit')) {
	...
	$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sea加盟中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
	...
	plog('commission.cover', '修改加盟商入口设置');
	message('加盟中心入口设置成功!', referer(), 'success');
}
load()->func('tpl');
