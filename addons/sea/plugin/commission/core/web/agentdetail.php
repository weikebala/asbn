...
if ($operation == 'display') {
    ...
    $sql = "select dm.*,dm.nickname,dm.avatar,s.name as shop_name,s.logo as log from " . tablename('sea_member') .  " dm left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid and f.uniacid={$_W['uniacid']}". " left join " . tablename('sea_commission_shop') . " s on s.mid=dm.id and s.uniacid={$_W['uniacid']}" . " where dm.uniacid = " . $_W['uniacid'] . " and dm.isagent =1 and dm.agentlevels=0  {$condition} ORDER BY dm.agenttime desc";
    $sql .= " limit " . ($pindex - 1) * $psize . ',' . $psize;
    $list  = pdo_fetchall($sql, $params);
    $total = pdo_fetchcolumn("select count(dm.id) from" . tablename('sea_member') . " dm  " . " left join " . tablename('sea_member') . " p on p.id = dm.agentid " . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid" . " where dm.uniacid =" . $_W['uniacid'] . " and dm.isagent =1 and dm.agentlevels=0 {$condition}", $params);
    foreach ($list as &$row) {
