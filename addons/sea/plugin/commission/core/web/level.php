...
if ($operation == 'display') {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('sea_commission_level') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY commission1 asc");
	...
	$level = pdo_fetch("SELECT * FROM " . tablename('sea_commission_level') . " WHERE id = '$id'");
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('sea_commission_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			plog('commission.level.edit', "修改加盟商等级 ID: {$id}");
		} else {
			pdo_insert('sea_commission_level', $data);
			$id = pdo_insertid();
			plog('commission.level.add', "添加加盟商等级 ID: {$id}");
		}
	...
	$id = intval($_GPC['id']);
	$level = pdo_fetch("SELECT id,levelname FROM " . tablename('sea_commission_level') . " WHERE id = '$id'");
	if (empty($level)) {
		message('抱歉，等级不存在或是已经被删除！', $this->createPluginWebUrl('commission/level', array('op' => 'display')), 'error');
	}
	pdo_delete('sea_commission_level', array('id' => $id, 'uniacid' => $_W['uniacid']));
	plog('commission.level.delete', "删除加盟商等级 ID: {$id} 等级名称: {$level['levelname']}");
