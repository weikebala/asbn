...
if ($operation == 'display') {
    ...
    if ($_GPC['export'] == '1') {
        ...
        plog('commission.agent.export', '导出加盟商数据');
        foreach ($list as &$row) {
        ...
        m('excel')->export($list, array(
            "title" => "加盟商数据-" . date('Y-m-d-H-i', time()),
            "columns" => array(
                array(
                    'title' => 'ID',
                    'field' => 'id',
...
}elseif($operation == 'agentdetail'){//加盟详情
    ca('commission.agent.view');
    $level     = $this->set['level'];
    $pindex    = max(1, intval($_GPC['page']));
    $psize     = 20;
