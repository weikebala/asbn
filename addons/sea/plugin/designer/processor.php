...
class DesignerProcessor extends PluginProcessor
{
    ...
    public function respond($obj = null)
    {
        ...
        if ($msgtype == 'text' || $event == 'click') {
            $page = pdo_fetch('select * from ' . tablename('sea_designer') . ' where keyword=:keyword and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':keyword' => $content
            ));
            if (empty($page)) {
            ...
            $news    = array(
                array(
                    ...
                    'picurl' => $r_img,
                    'description' => $r_desc,
                    'url' => $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=designer&pageid=' . $page['id']
                )
            );
