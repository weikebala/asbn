...
if (!class_exists('CreditshopModel')) {
    class CreditshopModel extends PluginModel
    {
        public function getGoods($id, $member)
        {
            ...
            $goods = pdo_fetch('select * from ' . tablename('sea_creditshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
            if (empty($goods)) {
            ...
            if (empty($goods['canbuy'])) {
                ...
                if ($goods['total'] > 0) {
                    $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('sea_creditshop_log') . "  where goodsid=:goodsid and status>=2  and uniacid=:uniacid  ", array(
                        ':goodsid' => $id,
                        ':uniacid' => $_W['uniacid']
                    ));
                ...
                if ($goods['canbuy']) {
                    if ($goods['totalday'] > 0) {
                        $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('sea_creditshop_log') . "  where goodsid=:goodsid and status>=2 and  date_format(from_UNIXTIME(`createtime`),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') and uniacid=:uniacid  ", array(
                            ':goodsid' => $id,
                            ':uniacid' => $_W['uniacid']
                        ));
                ...
                if ($goods['canbuy']) {
                    if ($goods['chanceday'] > 0) {
                        $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('sea_creditshop_log') . "  where goodsid=:goodsid and openid=:openid and status>0 and  date_format(from_UNIXTIME(`createtime`),'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') and uniacid=:uniacid  ", array(
                            ':goodsid' => $id,
                            ':uniacid' => $_W['uniacid'],
                ...
                if ($goods['canbuy']) {
                    if ($goods['chance'] > 0) {
                        $logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('sea_creditshop_log') . '  where goodsid=:goodsid and openid=:openid and status>0 and  uniacid=:uniacid  ', array(
                            ':goodsid' => $id,
                            ':uniacid' => $_W['uniacid'],
                            ':openid' => $member['openid']
                        ));
                ...
                if ($goods['canbuy']) {
                    if ($goods['usermaxbuy'] > 0) {
                        $logcount = pdo_fetchcolumn('select ifnull(sum(total),0)  from ' . tablename('sea_creditshop_log') . '  where goodsid=:goodsid and openid=:openid  and uniacid=:uniacid ', array(
                            ':goodsid' => $id,
                            ':uniacid' => $_W['uniacid'],
                            ':openid' => $member['openid']
                        ));
        ...
        {
            global $_W;
            $ecount = pdo_fetchcolumn('select count(*) from ' . tablename('sea_creditshop_log') . ' where uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid']
            ));
            if ($ecount < 99999999) {
                $ecount = 8;
            ...
            while (1) {
                $c = pdo_fetchcolumn('select count(*) from ' . tablename('sea_creditshop_log') . ' where uniacid=:uniacid and eno=:eno limit 1', array(
                    ':uniacid' => $_W['uniacid'],
                    ':eno' => $eno
                ));
                if ($c <= 0) {
        ...
        public function sendMessage($id = 0)
        {
            ...
            $log = pdo_fetch('select * from ' . tablename('sea_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
            if (empty($log)) {
            ...
            $set       = $this->getSet();
            $tm        = $set['tm'];
            $detailurl = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=creditshop&method=log&op=detail&&id=' . $id;
            if (strexists($detailurl, '/addons/sea/')) {
                $detailurl = str_replace("/addons/sea/", '/', $detailurl);
            }
            if ($log['status'] == 2) {
                if (!empty($type)) {
                ...
                if ($log['dispatchstatus'] == 1 || $log['dispatchstatus'] == -1) {
                    ...
                    if (!empty($log['addressid'])) {
                        $address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('sea_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(
                            ':id' => $log['addressid'],
                            ':uniacid' => $_W['uniacid']
                        ));
                        if (!empty($address)) {
        ...
        {
            global $_W, $_GPC;
            $path = IA_ROOT . "/addons/sea/data/creditshop/" . $_W['uniacid'];
            if (!is_dir($path)) {
                load()->func('file');
                mkdirs($path);
            }
            $url         = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=creditshop&method=exchange&op=exchange&id=' . $logid;
            ...
            return $_W['siteroot'] . '/addons/sea/data/creditshop/' . $_W['uniacid'] . '/' . $file;
        }
        function perms()
        {
            return array(
