...
$cate = pdo_fetch('select id,name from ' . tablename('sea_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cateid, ':uniacid' => $uniacid));
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('sea_creditshop_goods') . " where 1 {$condition}";
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!empty($total)) {
			$sql = 'SELECT id,title,thumb,subtitle,type,credit,money FROM ' . tablename('sea_creditshop_goods') . '  where 1 ' . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
