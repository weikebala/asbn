...
if ($_W['isajax']) {
	if ($operation == 'display') {
		if (!empty($goods)) {
			pdo_update('sea_creditshop_goods', array('views' => $goods['views'] + 1), array('id' => $id));
		}
		if ($goods['type'] == 0) {
			$stores = array();
		...
		$needpay = false;
		if ($goods['money'] > 0) {
			pdo_delete('sea_creditshop_log', array('goodsid' => $id, 'openid' => $openid, 'status' => 0, 'paystatus' => 0));
			$needpay = true;
			$lastlog = pdo_fetch('select * from ' . tablename('sea_creditshop_log') . ' where goodsid=:goodsid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':goodsid' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
			if (!empty($lastlog)) {
				show_json(1, array('logid' => $lastlog['id']));
			}
		} else {
			pdo_delete('sea_creditshop_log', array('goodsid' => $id, 'openid' => $openid, 'status' => 0));
		}
		$dispatchstatus = 0;
		...
		pdo_insert('sea_creditshop_log', $log);
		$logid = pdo_insertid();
		if (!empty($log['realname']) && !empty($log['mobile'])) {
			$up = array('realname' => $log['realname'], 'mobile' => $log['mobile']);
			pdo_update('sea_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			if (!empty($member['uid'])) {
				load()->model('mc');
		...
		if ($needpay) {
			...
			pdo_update('sea_creditshop_log', array('paytype' => $useweixin ? 1 : 0), array('id' => $logid));
			if ($useweixin) {
		...
		$log = pdo_fetch('select * from ' . tablename('sea_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $logid, ':uniacid' => $uniacid));
		if (empty($log)) {
			show_json(-1, '服务器错误!');
		}
		if ($log['status'] >= 1) {
		...
		pdo_update('sea_creditshop_goods', array('joins' => $goods['joins'] + 1), array('id' => $id));
		...
		pdo_update('sea_creditshop_log', $update, array('id' => $logid));
		if ($status == 2) {
