...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('sea_creditshop_log') . " log where 1 {$condition}";
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!empty($total)) {
			$sql = 'SELECT log.id,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money,g.isverify,g.goodstype,log.addressid,log.storeid FROM ' . tablename('sea_creditshop_log') . ' log ' . ' left join ' . tablename('sea_creditshop_goods') . ' g on log.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('sea_creditshop_log') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
		if (empty($log)) {
			show_json(-1, '兑换记录不存在!');
		}
		...
		if (!empty($log['addressid'])) {
			$address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('sea_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $uniacid, ':openid' => $openid));
		}
		...
		if (empty($log['storeid'])) {
			if (!empty($goods['isverify'])) {
				...
				if (empty($storeids)) {
					$stores = pdo_fetchall('select * from ' . tablename('sea_store') . ' where  uniacid=:uniacid and status=1', array(':uniacid' => $_W['uniacid']));
				} else {
					$stores = pdo_fetchall('select * from ' . tablename('sea_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1', array(':uniacid' => $_W['uniacid']));
				}
			}
		} else {
			$store = pdo_fetch('select * from ' . tablename('sea_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid']));
		}
		...
		if ($goods['dispatch'] <= 0) {
			pdo_update('sea_creditshop_log', array('dispatchstatus' => 1, 'addressid' => $addressid), array('id' => $log['id']));
		...
		if (empty($dispatchno)) {
			...
			pdo_update('sea_creditshop_log', array('dispatchno' => $dispatchno, 'addressid' => $addressid), array('id' => $log['id']));
		}
		$params = array();
		...
		if (!empty($upgrade)) {
			pdo_update('sea_creditshop_log', $upgrade, array('id' => $log['id']));
		}
		if (!empty($upgrade1)) {
			pdo_update('sea_member', $upgrade1, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			if (!empty($member['uid'])) {
				load()->model('mc');
