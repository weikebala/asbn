...
if ($operation == 'display') {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		foreach ($_GPC['displayorder'] as $id => $displayorder) {
			pdo_update('sea_creditshop_goods', array('displayorder' => $displayorder), array('id' => $id));
		}
	...
	$sql = 'SELECT * FROM ' . tablename('sea_creditshop_goods') . " where  1 and {$condition} ORDER BY displayorder DESC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_creditshop_goods') . " where 1 and {$condition}", $params);
	$pager = pagination($total, $pindex, $psize);
	$category = pdo_fetchall('select id,name,thumb from ' . tablename('sea_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('sea_creditshop_goods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			plog('creditshop.goods.edit', "编辑积分商城商品 ID: {$id} <br/>商品名称: {$data['title']}");
		} else {
			pdo_insert('sea_creditshop_goods', $data);
			$id = pdo_insertid();
	...
	$item = pdo_fetch('SELECT * FROM ' . tablename('sea_creditshop_goods') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	if ($item['showlevels'] != '') {
	...
	$category = pdo_fetchall('select id,name,thumb from ' . tablename('sea_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']));
} elseif ($operation == 'delete') {
	ca('creditshop.goods.delete');
	$id = intval($_GPC['id']);
	$item = pdo_fetch('SELECT id,title FROM ' . tablename('sea_creditshop_goods') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	if (empty($item)) {
		message('抱歉，商品不存在或是已经被删除！', $this->createPluginWebUrl('creditshop/goods', array('op' => 'display')), 'error');
	}
	pdo_update('sea_creditshop_goods', array('deleted' => 1), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	if (in_array($type, array('istop', 'isrecommand', 'istime', 'status'))) {
		$data = ($data == 1 ? '0' : '1');
		pdo_update('sea_creditshop_goods', array($type => $data), array('id' => $id, 'uniacid' => $_W['uniacid']));
		if ($type == 'istop') {
			$typestr = '置顶';
