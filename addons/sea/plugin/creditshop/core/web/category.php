...
if ($operation == 'display') {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		foreach ($_GPC['displayorder'] as $id => $displayorder) {
			pdo_update('sea_creditshop_category', array('displayorder' => $displayorder), array('id' => $id));
		}
	...
	$list = pdo_fetchall("SELECT * FROM " . tablename('sea_creditshop_category') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('sea_creditshop_category', $data, array('id' => $id));
			plog('creditshop.category.edit', "修改积分商城分类 ID: {$id}");
		} else {
			pdo_insert('sea_creditshop_category', $data);
			$id = pdo_insertid();
	...
	$item = pdo_fetch("select * from " . tablename('sea_creditshop_category') . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $id, ":uniacid" => $_W['uniacid']));
} elseif ($operation == 'delete') {
	ca('creditshop.category.delete');
	$id = intval($_GPC['id']);
	$item = pdo_fetch("SELECT id,name FROM " . tablename('sea_creditshop_category') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
	if (empty($item)) {
		message('抱歉，分类不存在或是已经被删除！', $this->createPluginWebUrl('creditshop/category', array('op' => 'display')), 'error');
	}
	pdo_delete('sea_creditshop_category', array('id' => $id));
