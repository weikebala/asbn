...
if ($operation == 'display') {
	...
	if (!empty($_GPC['displayorder'])) {
		...
		foreach ($_GPC['displayorder'] as $id => $displayorder) {
			pdo_update('sea_creditshop_adv', array('displayorder' => $displayorder), array('id' => $id));
		}
		plog('creditshop.adv.edit', '批量修改幻灯片的排序');
		message('分类排序更新成功！', $this->createPluginWebUrl('creditshop/adv', array('op' => 'display')), 'success');
	}
	$list = pdo_fetchall("SELECT * FROM " . tablename('sea_creditshop_adv') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY displayorder DESC");
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('sea_creditshop_adv', $data, array('id' => $id));
			plog('creditshop.adv.edit', "修改积分商城幻灯片 ID: {$id}");
		} else {
			pdo_insert('sea_creditshop_adv', $data);
			$id = pdo_insertid();
			plog('creditshop.adv.add', "添加积分商城幻灯片 ID: {$id}");
		}
		message('更新幻灯片成功！', $this->createPluginWebUrl('creditshop/adv', array('op' => 'display')), 'success');
	}
	$item = pdo_fetch("select * from " . tablename('sea_creditshop_adv') . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $id, ":uniacid" => $_W['uniacid']));
} elseif ($operation == 'delete') {
	ca('creditshop.adv.delete');
	$id = intval($_GPC['id']);
	$item = pdo_fetch("SELECT id,advname FROM " . tablename('sea_creditshop_adv') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
	if (empty($item)) {
		message('抱歉，幻灯片不存在或是已经被删除！', $this->createPluginWebUrl('creditshop/adv', array('op' => 'display')), 'error');
	}
	pdo_delete('sea_creditshop_adv', array('id' => $id));
