...
if ($operation == 'display') {
	...
	$sql = 'select log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype,g.isverify,g.goodstype as iscoupon,s.storename,s.address as storeaddress from ' . tablename('sea_creditshop_log') . ' log ' . ' left join ' . tablename('sea_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('sea_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('sea_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('sea_creditshop_goods') . ' g on g.id = log.goodsid' . " where 1 {$condition} ORDER BY log.createtime desc ";
	if (empty($_GPC['export'])) {
		$sql .= ' limit ' . ($pindex - 1) * $psize . ',' . $psize;
	}
	$list = pdo_fetchall($sql, $params);
	$total = pdo_fetchcolumn('select count(log.id) from' . tablename('sea_creditshop_log') . ' log ' . ' left join ' . tablename('sea_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('sea_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('sea_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('sea_creditshop_goods') . ' g on g.id = log.goodsid' . " where 1 {$condition}", $params);
	foreach ($list as &$row) {
		$row['address'] = array();
		if (!empty($row['addressid'])) {
			$row['address'] = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('sea_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
		} else {
	...
	$id = intval($_GPC['id']);
	$log = pdo_fetch('select * from ' . tablename('sea_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	if (empty($log)) {
	...
	if (!empty($log['addressid'])) {
		$address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('sea_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
	}
	$address['logid'] = $id;
	...
	pdo_update('sea_creditshop_log', array('status' => 3, 'usetime' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
