...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ca('poster.clear');
        load()->func('file');
        @rmdirs(IA_ROOT . '/addons/sea/data/poster/' . $_W['uniacid']);
    ...
    $list  = pdo_fetchall("SELECT * FROM " . tablename('sea_poster') . " WHERE 1 {$condition} ORDER BY isdefault desc,createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
    foreach ($list as &$row) {
    	$row['times'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_poster_scan') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
    	$row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('sea_poster_log') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
    }
    unset($row);
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('sea_poster') . " where 1 {$condition} ", $params);
    ...
    $item = pdo_fetch("SELECT * FROM " . tablename('sea_poster') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    if (!empty($item)) {
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('sea_poster', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
            plog('poster.edit', "修改超级海报 ID: {$id}");
        } else {
            pdo_insert('sea_poster', $data);
            $id = pdo_insertid();
    ...
    ca('poster.delete');
    $id     = intval($_GPC['id']);
    $poster = pdo_fetch("SELECT id,title FROM " . tablename('sea_poster') . " WHERE id = '$id'");
    if (empty($poster)) {
        message('抱歉，海报不存在或是已经被删除！', $this->createPluginWebUrl('poster', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('sea_poster', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    pdo_delete('sea_poster_log', array(
        'posterid' => $id,
        'uniacid' => $_W['uniacid']
    ));
    ...
    $id     = intval($_GPC['id']);
    $poster = pdo_fetch("SELECT * FROM " . tablename('sea_poster') . " WHERE id = '$id'");
    if (empty($poster)) {
        message('抱歉，海报不存在或是已经被删除！', $this->createPluginWebUrl('poster', array(
            'op' => 'display'
        )), 'error');
