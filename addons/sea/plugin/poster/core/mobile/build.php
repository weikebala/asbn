...
if (strexists($content, '+')) {
    $msg    = explode('+', $content);
    $poster = pdo_fetch('select * from ' . tablename('sea_poster') . ' where keyword=:keyword and type=3 and isdefault=1 and uniacid=:uniacid limit 1', array(
        ':keyword' => $msg[0],
        ':uniacid' => $_W['uniacid']
    ));
    if (empty($poster)) {
    ...
    $poster = pdo_fetch('select * from ' . tablename('sea_poster') . ' where keyword=:keyword and isdefault=1 and uniacid=:uniacid limit 1', array(
        ':keyword' => $content,
        ':uniacid' => $_W['uniacid']
    ));
    if (empty($poster)) {
...
if ($member['isagent'] != 1 || $member['status'] != 1) {
    if (empty($poster['isopen'])) {
        $opentext = !empty($poster['opentext']) ? $poster['opentext'] : '您还不是我们加盟商，去努力成为加盟商，拥有你的专属海报吧!';
