...
if (!class_exists('PosterModel')) {
    class PosterModel extends PluginModel
    {
        public function checkScan()
        {
            ...
            $poster = pdo_fetch('select id,times from ' . tablename('sea_poster') . ' where id=:id and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':id' => $posterid
            ));
            if (empty($poster)) {
        ...
        public function scanTime($openid, $from_openid, $poster)
        {
            ...
            global $_W, $_GPC;
            $scancount = pdo_fetchcolumn('select count(*) from ' . tablename('sea_poster_scan') . ' where openid=:openid  and posterid=:posterid and uniacid=:uniacid limit 1', array(
                ':openid' => $openid,
                ':posterid' => $poster['id'],
                ':uniacid' => $_W['uniacid']
            ));
            if ($scancount <= 0) {
                ...
                pdo_insert('sea_poster_scan', $scan);
                pdo_update('sea_poster', array(
        ...
        public function createCommissionPoster($openid, $goodsid = 0)
        {
            ...
            $poster = pdo_fetch('select * from ' . tablename('sea_poster') . ' where uniacid=:uniacid and type=:type and isdefault=1 limit 1', array(
                ':uniacid' => $_W['uniacid'],
                ':type' => $type
            ));
            if (empty($poster)) {
		...
		{
			global $_W, $_GPC;
			$scene_str = md5("sea_poster:{$_W['uniacid']}:{$member['openid']}:{$poster['id']}");
        ...
        public function getQR($poster, $member, $goodsid = 0)
        {
            ...
            if ($poster['type'] == 1) {
                ...
                $qr    = pdo_fetch('select * from ' . tablename('sea_poster_qr') . ' where openid=:openid and acid=:acid and type=:type limit 1', array(
                ...
                $qr    = pdo_fetch('select * from ' . tablename('sea_poster_qr') . ' where openid=:openid and acid=:acid and type=:type and goodsid=:goodsid limit 1', array(
                    ':openid' => $member['openid'],
                    ':acid' => $_W['acid'],
                    ':type' => 3,
                    ':goodsid' => $goodsid
                ...
                $qr         = pdo_fetch('select * from ' . tablename('sea_poster_qr') . ' where openid=:openid and acid=:acid and type=4 limit 1', array(
                    ':openid' => $member['openid'],
                    ':acid' => $acid
                ));
                if (empty($qr)) {
        ...
        public function createPoster($poster, $member, $qr, $upload = true)
        {
            ...
            if (!empty($qr['goodsid'])) {
                $goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('sea_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                    ':id' => $qr['goodsid'],
                    ':uniacid' => $_W['uniacid']
                ));
                if (empty($goods)) {
		...
		'bg' => $poster['bg'], 
            ...
            $img = $_W['siteroot'] . "addons/sea/data/poster/" . $_W['uniacid'] . "/" . $file;
            if (!$upload) {
                return $img;
            }
        ...
        public function getQRByTicket($ticket = '')
		{
			...
			$qrs = pdo_fetchall('select * from ' . tablename('sea_poster_qr') . ' where ticket=:ticket and acid=:acid and type=4 limit 1', array(':ticket' => $ticket, ':acid' => $_W['acid']));
			$count = count($qrs);
			if ($count <= 0) {
				return false;
			}
        ...
        public function checkMember($openid = '')
        {
            ...
            if (empty($member)) {
                ...
                pdo_update('sea_member', $member, array(
                    'id' => $member['id']
                ));
                $member['isnew'] = false;
            }
