...
class PosterProcessor extends PluginProcessor
{
    ...
    /*private function responseText($obj)
    {
        ...
        $resp = ihttp_post($_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=poster&method=build', array(
	...
	private function responseText($obj)
	{
		...
		$timeout = 4;
		load()->func('communication');
		$resp = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=sea&do=plugin&p=poster&method=build&timestamp=' . time();
    ...
    private function responseScan($obj)
    {
        ...
        $poster = pdo_fetch('select * from ' . tablename('sea_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($poster)) {
            return $this->responseDefault($obj);
        ...
        if (empty($url)) {
            if ($qrmember['isagent'] == 1 && $qrmember['status'] == 1) {
                $url = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=sea&do=plugin&p=commission&method=myshop&mid=" . $qrmember['id'];
            } else {
                $url = $_W['siteroot'] . "app/index.php?i={$_W['uniacid']}&c=entry&m=sea&do=shop&mid=" . $qrmember['id'];
            }
        }
    ...
    private function responseSubscribe($obj)
    {
        ...
        $qrmember = m('member')->getMember($qr['openid']);
        $log      = pdo_fetch('select * from ' . tablename('sea_poster_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(
            ':openid' => $openid,
            ':posterid' => $poster['id'],
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($log) && $openid != $qr['openid']) {
			...
			if (!empty($_var_27)) {
				pdo_update('sea_poster_log', $_var_27, array('id' => $log['id']));
			}
		}
						...
						if (!empty($poster['bedown'])) {
							if (empty($member['agentid'])) {
								if (empty($member['fixagentid'])) {
									pdo_update('sea_member', array('agentid' => $qrmember['id'], 'childtime' => $time), array('id' => $member['id']));
							...
							if (!empty($poster['beagent'])) {
								...
								pdo_update('sea_member', array('isagent' => 1, 'status' => $status, 'agenttime' => $time), array('id' => $member['id']));
								if ($status == 1) {
