	...
	public function payResult($params) {
		...
		if(empty($order_out['status'])){
			...
			if ($params['result'] == 'success' && $params['from'] == 'notify') {
				file_put_contents(MB_ROOT."/notify.log", var_export($nowtuan, true).PHP_EOL, FILE_APPEND);
				/*支付成功消息模板*/
				$time = date('Y-m-d H:i:s',time());
				$url = $_W['siteroot'] . 'app/' . $this -> createMobileUrl('myorder', array('id' => $order_out['id']));
				$sendmessage = new Message();
				if($order_out['ordertype']==1){
					$res = $sendmessage -> daipay_success($order_out['openid'], $fee,$order_out['othername'],$order_out['orderno'], $goodsInfo['gname'],$time, $order_out['message'],$this, $url);
				}else{
		...
		if ($params['result'] == 'success' && $params['from'] == 'return') {
			...
			if ($params['type'] == $credit) {
				if($order_out['is_tuan']==2){
					echo "<script>location.href='".$this->createMobileUrl('index',array('result'=>'success','ordertype'=>$order_out['ordertype']))."';</script>";
					exit;
				}else{
					if ($order_out['is_tuan'] == 0) {
					echo "<script> location.href='" . $this -> createMobileUrl('myorder',array('payreslut' => 'success','ordertype'=>$order_out['ordertype'])) . "';</script>";
					exit ;
					} else {
						echo "<script>  location.href='" . $this -> createMobileUrl('group', array('tuan_id' => $order_out['tuan_id'],'payreslut' => 'success','ordertype'=>$order_out['ordertype'])) . "';</script>";
						exit ;
					}
				}
				...
				if($order_out['is_tuan']==2){
					echo "<script>location.href='".$_W['siteroot'].'app/'.$this->createMobileUrl('index',array('result'=>'success','ordertype'=>$order_out['ordertype']))."';</script>";
					exit;
				}else{
					if ($order_out['is_tuan'] == 0) {
					echo "<script>  location.href='" . $_W['siteroot'] . 'app/' . $this -> createMobileUrl('myorder',array('payreslut' => 'success','ordertype'=>$order_out['ordertype'])) . "';</script>";
					exit ;
					} else {
						echo "<script>  location.href='" . $_W['siteroot'] . 'app/' . $this -> createMobileUrl('group', array('tuan_id' => $order_out['tuan_id'],'payreslut' => 'success','ordertype'=>$order_out['ordertype'])) . "';</script>";
						exit ;
					}
				}
