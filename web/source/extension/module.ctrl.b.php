...
if($do == 'installed') {
	...
	if (!empty($modules)) {
		foreach ($modules as $mid => $module) {
			...
			$modules[$mid]['official'] = empty($module['issystem']) && (strexists($module['author'], 'iseasoft Team') || strexists($module['author'], '海软团队'));
...
if($do == 'permission') {
	...
	if (empty($module)) {
		message('你访问的模块不存在. 或许你愿意去海软云服务平台看看. ', ADDONS_URL.'/web/index.php?keyword=' . $_GPC['title']);
	}
...
<manifest xmlns="http://www.iseasoft.cn" versionCode="{$versions}">
...
class {$name}ModuleProcessor extends WeModuleProcessor {
	public function respond() {
		...
		//这里定义此模块进行消息处理时的具体过程, 请查看海软文档来编写你的代码
	}
}
TPL;
	}
