...
if($do == 'installed') {
	$_W['page']['title'] = '已安装的PC站风格 - 风格主题 - 扩展';
	$templateids = array();
	$where = (empty($_GPC['type']) || $_GPC['type'] == 'all') ? '' : " WHERE `type` = '{$_GPC['type']}'";
	$templates = pdo_fetchall("SELECT * FROM ".tablename('fournet_pctemplates') . $where);
	foreach($templates as $tpl) {
		$templateids[] = $tpl['name'];
	}
	
	$temtypes = ext_pctemplate_type();
	template('extension/pc');
}

if($do == 'prepared') {
	$_W['page']['title'] = '安装PC站风格 - 风格主题 - 扩展';
	$templateids = array();
	$templates = pdo_fetchall("SELECT * FROM ".tablename('fournet_pctemplates'));
	foreach($templates as $tpl) {
		$templateids[] = $tpl['name'];
	}
...
if($do == 'batch-install') {
	if($_W['ispost']) {
		...
		if (pdo_fetchcolumn("SELECT id FROM ".tablename('fournet_pctemplates')." WHERE name = '{$m['name']}'")) {
			exit('error');
		}
				unset($m['settings']);
		
		if (pdo_insert('fournet_pctemplates', $m)) {
			exit('success');
		} else {
			exit('error');
		}
...
if($do == 'install') {
	...
	$id = $_GPC['templateid'];
	if (pdo_fetchcolumn("SELECT id FROM ".tablename('fournet_pctemplates')." WHERE name = :name", array(':name' => $id))) {
	...
	if (!empty($manifest)) {
		...
		if(is_error($r)) {
			message($r['message'], url('extension/pc/prepared'), 'error');
		}
	}
	...
	if (pdo_fetchcolumn("SELECT id FROM ".tablename('fournet_pctemplates')." WHERE name = '{$manifest['name']}'")) {
		message('模板已经安装或是唯一标识已存在！', '', 'error');
	}
		if (pdo_insert('fournet_pctemplates', $manifest)) {
		$tid = pdo_insertid();
	} else {
		message('模板安装失败, 请联系模板开发者！');
	}
	if($id && $post_groups) {
		if (!pdo_fetchcolumn("SELECT id FROM ".tablename('fournet_pctemplates')." WHERE id = {$tid}")) {
	...
	message('模板安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！', url('extension/pc'), 'success');
}

if($do == 'uninstall') {
	$name = pdo_fetchcolumn('SELECT name FROM ' . tablename('fournet_pctemplates') . ' WHERE id = :id', array(':id' => intval($_GPC['id'])));
	if($name == 'default') {
		message('默认模板不能卸载', '', 'error');
	}
	if (pdo_delete('fournet_pctemplates', array('id' => intval($_GPC['id'])))) {
...
if($do == 'upgrade') {
	...
	if($check == 1) {
		...
		$templates = pdo_fetchall('SELECT id,name,version FROM ' . tablename('fournet_pctemplates'), array(), 'name');
		$upgrade = array();
		$mods = array();
		$ret = cloud_t_query();
		if(!is_error($ret)) {
	...
	if($batch == 1) {
		...
		if(empty($wait_upgrade)) {
			...
			message('您的模板已经是最新版本', url('extension/pc'), 'success');
		}
	...
	$theme = pdo_fetch("SELECT id, name, title FROM " . tablename('fournet_pctemplates') . " WHERE name = :name", array(':name' => $id));
	if (empty($theme)) {
		if($batch == 1) {
			cache_write('upgrade:template', iserializer($wait_upgrade));
			message($theme['title'] . ' 模板已经被卸载或是不存在。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('extension/pc/upgrade', array('batch' => 1)), 'success');
		}
	...
	if (empty($manifest)) {
		if($batch == 1) {
			cache_write('upgrade:template', iserializer($wait_upgrade));
			message($theme['title'] . ' 模块安装配置文件不存在或是格式不正确。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('extension/pc/upgrade', array('batch' => 1)), 'success');
		}
	...
	if(ver_compare($theme['version'], $packet['version']) != -1) {
		if($batch == 1) {
			cache_write('upgrade:template', iserializer($wait_upgrade));
			message($theme['title'] . ' 模板版本不低于要更新的版本。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('extension/pc/upgrade', array('batch' => 1)), 'success');
		}
		message('已安装的模板版本不低于要更新的版本, 操作无效.');
	}
	pdo_update('fournet_pctemplates', array('version' => $packet['version']), array('id' => $theme['id']));
	if($batch == 1) {
		cache_write('upgrade:template', iserializer($wait_upgrade));
		message($theme['title'] . ' 模板更新成功。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('extension/pc/upgrade', array('batch' => 1)), 'success');
	}
	message('模板更新成功！', url('extension/pc'), 'success');
}
...
if ($do == 'designer') {
	...
	$_W['page']['title'] = '设计PC站风格 - 风格主题 - 扩展';
	
	$available['download'] = class_exists('ZipArchive');
	
	$available['create'] = is_writable(IA_ROOT . '/app/pc');
	...
	if (checksubmit('submit') && $available[$_GPC['method']]) {
		...
		if ($_GPC['method'] == 'create') {
			$tpldir = IA_ROOT . '/app/pc/' . strtolower($t['template']['identifie']);
			if (is_dir($tpldir)) {
	...
	template('extension/pc_desitemp');
}
...
if($do == 'check') {
	if($_W['isajax']) {
		...
		if ($foo == 'upgrade') {
			...
			$templateids = array();
			$templates = pdo_fetchall("SELECT `name` FROM " . tablename('fournet_pctemplates') . ' ORDER BY `id` ASC');
			if(!empty($templates)) {
