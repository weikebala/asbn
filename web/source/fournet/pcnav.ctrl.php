...
$titles = array('home'=>'PC站首页导航图标', 'profile'=>'个人中心功能条目');
...
if($do == 'post') {
	...
	if(!empty($id)) {
		$item = pdo_fetch("SELECT * FROM " . tablename('fournet_pcnav') . " WHERE `uniacid` = :uniacid AND `id` = :id" , array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if(empty($item)) {
	...
	if(checksubmit('submit')) {
		...
		if(empty($id)) {
			...
			pdo_update('fournet_pcnav', $data, array('id' => $id));
		}
		message('导航更新成功！', url('fournet/pcnav', array('do'=>$type['name'], 'multiid' => $_GPC['multiid'], 'f' => $_GPC['f'])), 'success');
	}
	template('fournet/pcnav');
}
...
if($do == 'delete') {
	...
	pdo_delete('fournet_pcnav', array('id' => $id));
...
if($do == 'saves') {
	...
	foreach($titles as $key => $t) {
		...
		if(!empty($t)) {
			...
			pdo_update('fournet_pcnav', $rec, $filter);
		}
	}
...
if($do == 'display') {
	if(!empty($module)) {
		...
		$tytitle = array('home' => 'PC站首页导航图标', 'profile' => '个人中心功能条目');
	...
	if($_W['ispost']) {
		...
		if(is_array($set)) {
			if (!empty($set['id'])) {
				$sql = $sql = 'SELECT * FROM ' . tablename('fournet_pcnav') . ' WHERE id = :id';
				$pars[':id'] = $set['id'];
			} else {
				$sql = 'SELECT * FROM ' . tablename('fournet_pcnav') . ' WHERE `position`=:position AND `uniacid`=:uniacid AND `module`=:module AND `url`=:url';
			...
			if(!empty($nav)) {
				...
				if(pdo_update('fournet_pcnav', $record, array('id' => $nav['id'])) !== false) {
					exit('success');
				}
			} else {
				$nav = array();
				...
				if(pdo_insert('fournet_pcnav', $nav)) {
					exit('success');
				}
			}
		}
	...
	$sql = 'SELECT * FROM ' . tablename('fournet_pcnav') . ' WHERE `uniacid`=:uniacid AND `position`=' . $type['position'] . $condition . ' ORDER BY `displayorder` DESC, id ASC';
	...
	$styleid = pdo_fetchcolumn("SELECT styleid FROM ".tablename('fournet_pcmulti')." WHERE id = '{$siteid}'");
	if (!empty($styleid)) {
		$style = pdo_fetch("SELECT templateid, name FROM ".tablename('fournet_pcstyles')." WHERE id = '{$styleid}'");
	}
	$template = pdo_fetch("SELECT * FROM ".tablename('fournet_pctemplates')." WHERE id = '{$style['templateid']}'");
	...
	if (isset($manifest['sections']) && $manifest['sections'] != $template['sections']) {
		...
		pdo_update('fournet_pctemplates', array('sections' => $manifest['sections']), array('id' => $template['id']));
	}
	template('fournet/pcnav');
}
