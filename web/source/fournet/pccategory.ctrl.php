...
uni_user_permission_check('fournet_pccategory');
...
if ($do == 'display') {
	...
	if (!empty($parentid)) {
		...
		if (empty($parent)) {
			message('抱歉，上级分类不存在或是已经被删除！', url('fournet/pccategory/display'), 'error');
		}
	}
	...
	if (checksubmit('submit')) {
		...
		message('更新分类成功！', url('fournet/pccategory'), 'success');
	}
	template('fournet/pccategory');
} elseif ($do == 'fetch') {
	...
	if (empty($category)) {
		message('抱歉，分类不存在或是已经被删除！', url('fournet/pccategory/display'), 'error');
	}
	...
	message('分类删除成功！', url('fournet/pccategory'), 'success');
} elseif($do == 'check') {
	$styleid = intval($_GPC['styleid']);
