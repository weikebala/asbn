...
if($do == 'display') {
		...
		if(!empty($list)) {
			...
			for($i = 0; $i < $size; $i++) {
				...
				foreach($buffer as $row) {
					if(strexists($row['email'], 'iseasoft.cn')) {
						$row['email'] = '';
					}
	...
	if(!empty($list)) {
		foreach($list as &$li) {
			if(empty($li['email']) || (!empty($li['email']) && substr($li['email'], -6) == 'iseasoft.cn' && strlen($li['email']) == 39)) {
...
if($do == 'post') {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($_GPC)) {
			...
			if(!empty($_GPC['fanid'])) {
					...
					$_GPC['email'] = md5($_GPC['openid']) . '@iseasoft.cn';
				}
	...
	if(!empty($profile)) {
		if(empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'], -6) == 'iseasoft.cn' && strlen($profile['email']) == 39)) {
