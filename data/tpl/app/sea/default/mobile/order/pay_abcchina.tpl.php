...
<title><?php  if(!empty($orderid)) { ?>农行支付<?php  } else { ?>农行充值<?php  } ?></title>
<style type="text/css">
...
var logid = "<?php  echo $logid;?>";
    require(['tpl', 'core'], function(tpl, core) {
        core.json('order/pay_abcchina',{orderid:orderid, logid:logid, openid:"<?php  echo $openid;?>"},function(json){
            ...
            if(!result.abcchina.success){
                if(orderid!='0'){
                    core.message('农行支付参数错误!',"<?php  echo $this->createMobileUrl('order/pay',array('orderid'=>$orderid))?>",'error');
               }else if(logid!='0'){
                    core.message('农行支付参数错误!',"<?php  echo $this->createMobileUrl('member/recharge')?>",'error');
                }
                 return;
            }
            var height =$(document.body).height();
             showIframe(result.abcchina.url,"100%",height);
             },true,true);
     });
</script>
