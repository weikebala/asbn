...
.order_sub11 {height:44px; margin:14px 5px;background:#1962ad; border-radius:4px; text-align:center; font-size:16px; line-height:44px; color:#fff;}
...
<script id='tpl_order_info' type='text/html'>
    ...
    <%if abcchinapay.success%><div class="button order_sub3" >农行支付</div><%/if%>
    
    <%if credit.success %>
        <div class="button order_sub11">余额支付(当前余额:<%credit.current%>)</div>
...
<script id='tpl_order_pay' type='text/html'>
    ...
    <%if address%>
        <img src="../addons/sea/template/mobile/default/static/images/pay_ok.png" style="width:100%;" />
     <%/if%>
     <%if order.dispatchtype=='1' && order.isverify!='1'%>
        <img src="../addons/sea/template/mobile/default/static/images/pay_carrier.png" style="width:100%;" />
     <%/if%>
     <%if order.isverify=='1'%>
        <img src="../addons/sea/template/mobile/default/static/images/pay_verify.png" style="width:100%;" />
     <%/if%>
     <%if order.virtual!='0'%>
        <img src="../addons/sea/template/mobile/default/static/images/pay_virtual.png" style="width:100%;" />
     <%/if%>
	 <%if order.isvirtual=='1'%>
        <img src="../addons/sea/template/mobile/default/static/images/pay_success.png?v=1" style="width:100%;" />
     <%/if%>
    <div class="order_main" >
        <%if address%>
...
<script id='tpl_order_cash' type='text/html'>
    ...
    <img src="../addons/sea/template/mobile/default/static/images/pay_cash.png" style="width:100%;" />
    <div class="order_main" >
        <%if address%>
        <div class="line"><div class="label">收货人</div><div class="info"><div class='inner'><%address.realname%> <%address.mobile%></div></div></div>
        <div class="line"><div class="label">收货地址</div><div class="info"><div class='inner'><%address.address%></div></div></div>
    ...
    require(['tpl', 'core'], function(tpl, core) {
        core.json('order/pay',{orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>"},function(json){
           ...
           if(result.alipay.success){
            ...
            if(result.abcchinapay.success){

                $('.order_sub3').click(function(){

                    var deduct = ($('#deductmoney').length>0 &&$('#deductmoney').attr('on')=='1' )?1:0 ;
                    core.json('order/pay', {op: 'pay',type: 'abcchinapay', orderid:'<?php  echo $_GPC['orderid'];?>',openid:"<?php  echo $openid;?>",deduct:deduct}, function (rjson) {
                        if(rjson.status!=1){
                            $('.button').removeAttr('submitting');
                            core.tip.show(rjson.result);
                            return;
                        ...
                        //virtual
                        location.href = core.getUrl('order/pay_abcchina',{orderid:'<?php  echo $_GPC['orderid'];?>'});
                        return;
                    },true,true);
                })
            }
