...
<style type="text/css">
	...
	#confirm_container{bottom: 30px;position: relative;}
    .addorder_topbar {height:34px; background:#5f6e8b; padding:15px; padding-top:40px}
    ...
    .addorder_good {height:auto;padding:10px;background:#fff;  margin-top:6px; border-bottom:1px solid #eaeaea; border-top:1px solid #eaeaea;}
    ...
    .addorder_good .good {height:76px; width:100%; padding:10px 0px; border-bottom:1px solid #eaeaea;}
    ...
    .addorder_price {height:auto;  background:#fff; padding:5px 10px; margin-top:6px; border-bottom:1px solid #eaeaea; border-top:1px solid #eaeaea;} 
    ...
    .addorder_pay {height:60px; width:94%; background:#fff; padding:0px 3%; margin-top:30px; border-top:1px solid #eaeaea;position: fixed;bottom: 0;left: 0}
...
.select { -webkit-appearance: none }
    ...
    .carrier_input_info {height:auto;width:100%; background:#fff; margin-top:6px; border-bottom:1px solid #e8e8e8; border-top:1px solid #e8e8e8;}
      ...
      <%if isverify || isvirtual || goods.type==2%>
          ...
          <%if carrier_list.length>0%>
              <div class="addorder_nav">
                <div class="nav selected" data-nav='0'>配送核销</div>
    ...
    <div class="addorder_good">
        ...
        <div class="good" data-totalmaxbuy="<%g.totalmaxbuy%>">
            ...
            <div class="price">
				...
				<div class='pnum'><span class='customspricelv'>税率<%g.customs%>%</span>
				<span class='customspricelvs' style="display:none"><%g.customs%></span></div>
			   <%else%>
                <div class='pnum'><span class='total'>×<%g.total%></span></div>
				<div class='pnum'><span class='total'>税率<%g.customs%>%</span></div>
                <%/if%>
            </div>
        </div>
        <%/each%>
...
<?php  if(!empty($order_formInfo)) { ?>
    ...
    <div class="addorder_price"  style="">
        ...
        <div class="price" style="border:none;">
            ...
            <%/if%>
            <div id="deductenoughdp" class="line" style="line-height:26px;<%if !saleset.enoughdeductdp %>display:none<%/if%>"  >-单品满减<span>-￥<span id="deductenough_moneydp"><%saleset.enoughdeductdp%></span><span></div>
            <div id="deductenoughzk" class="line" style="line-height:26px;<%if !saleset.showenoughzk %>display:none<%/if%>"  >-单笔满 <d style='color:#ff6600' id="deductenough_enoughzk"><%saleset.enoughmoneyzk%></d> 元折扣<%saleset.enoughdeductzk%>折<span>-￥<span id="deductenough_moneyzk"><%saleset.deductzk%></span><span></div>
            <div id="deductenough" class="line" style="line-height:26px;<%if !saleset.showenough %>display:none<%/if%>"  >-单笔满 <d style='color:#ff6600' id="deductenough_enough"><%saleset.enoughmoney%></d> 元立减  <span>-￥<span id="deductenough_money"><%saleset.enoughdeduct%></span><span></div>
            <div class="line" style="line-height:26px;">+运费<span>￥<span class='dispatchprice'><%dispatchprice%></span></span></div>
            <div class="line" style="line-height:26px;">+税<span>￥<span class='goodscustomsprice'><%goodscustoms_price%></span></span></div>
    ...
    <%/if%>
	 <div class="addorder_price" style="margin-top:10px; padding:0;padding-bottom:56px; height:1px;"></div>
...
function change_selectCity(){
	...
	function show_address(flag) {
                ...
                var addressid = $('#edit_addressid').val();
                $.ajax({
...
var fromcart = 0;
    require(['tpl', 'core'], function(tpl, core) {
        ...
        core.json('order/confirm', {id:'<?php  echo intval($_GPC['id'])?>', optionid:'<?php  echo intval($_GPC['optionid'])?>', total:'<?php  echo intval($_GPC['total'])?>',cartids:"<?php  echo $_GPC['cartids'];?>",stockModel:"<?php  echo $_GPC['stockModel'];?>"}, function(json){
        ...
        $('#confirm_container').html(tpl('tpl_confirm_info', json.result));
           ...
           $('.leftnav').click(function(){
               ...
               oldnum=num;
                ...
                input.val(num);
               if(oldnum!=num) {
                   location.href =location.href.replace(/total=[\d]{1,}/,'total='+num).replace(/code=.*/,'');
               }
               else{
                   return;
               }
				...
				customsprice =parseFloat( $(this).closest('.good').find('.marketprice').html().replace(",","") * $(this).closest('.good').find('.customspricelvs').html().replace(",","")/100)* num;
                
                $('.goodsprice').html( marketprice.toFixed(2) );
                $('.goodscustomsprice').html( customsprice.toFixed(2) );
             ...
             $('.rightnav').click(function(){
                ...
                var customsprice =parseFloat( $(this).closest('.good').find('.marketprice').html().replace(",","") * $(this).closest('.good').find('.customspricelvs').html().replace(",","")/100)* num;
                $('.goodsprice').html( marketprice.toFixed(2) );
                $('.goodscustomsprice').html( customsprice.toFixed(2) );
                     if( $('.memberdiscount').length>0){
                var discountprice = marketprice - parseFloat( $('.memberdiscount').html().replace(",","") ) / 10 * marketprice;
     ...
     $('.shownum').blur(function(){
	   ...
	   if($('#sel-provance').val()=='请选择省份'){
        ...
        function getDispatchPrice(zt) {
            ...
            core.json('order/confirm', {
                ...
                discountprice:parseFloat($(".discountprice").html()),
                deductenough_moneydp:parseFloat($("#deductenough_moneydp").text()),
                ...
                if (getjson.status == 1) {
					...
					if(getjson.result.deductenoughzk_money>0){
						$('#deductenoughzk').show();
						$('#deductenough_moneyzk').html( getjson.result.deductenoughzk_money);
						$('#deductenough_enoughzk').html( getjson.result.deductenoughzk_enough);
					} else{
						$('#deductenoughzk').hide();
						$('#deductenough_moneyzk').html( '0');
						$('#deductenough_enoughzk').html( '0');
					}
        ...
        function calctotalprice() {
            var goodsprice = parseFloat($('.goodsprice').html().replace(',',''));//79
            var dispatchprice = parseFloat($(".dispatchprice").html().replace(',',''));//9
            
            var discountprice =0;
            if($('.discountprice').length>0){
                 discountprice = parseFloat($(".discountprice").html().replace(',',''));//0
            }
	        var totalprice = goodsprice - discountprice;
            var enoughprice =0;
            var enoughpricedp =0;
            var enoughpricezk =0;
            if($("#deductenough_moneydp").length>0 && $("#deductenough_moneydp").html()!=''){
                enoughpricedp = parseFloat($("#deductenough_moneydp").html().replace(',',''));
            }
            if($("#deductenough_money").length>0 && $("#deductenough_money").html()!=''){
                 enoughprice = parseFloat($("#deductenough_money").html().replace(',',''));
            }
            if($("#deductenough_moneyzk").length>0 && $("#deductenough_moneyzk").html()!=''){
                enoughpricezk = parseFloat($("#deductenough_moneyzk").html().replace(',',''));
            }
            var goodscustomsprice = parseFloat($(".goodscustomsprice").html().replace(',',''));//9.4
	   <?php  if($hascouponplugin) { ?>
	       totalprice = calcCouponPrice(totalprice);
	   <?php  } ?>
            totalprice = totalprice-enoughpricedp - enoughprice-enoughpricezk +goodscustomsprice+ dispatchprice;//88
        ...
        $('.paysub').click(function() {
            ...
            var dispatchid = $("#dispatchid").val();//配送方式
            var carrierid = $("#carrierid").val();//自提地址
            var addressid = $("#addressid").val();//收货地址id
            var goods = $("#goods").val();//商品信息   good_id,optionid,totle|good_id,optionid,totle
            var carrier_realname = $.trim( $('#carrier_input_realname').val() );//自提者姓名
            var carrier_mobile = $.trim( $('#carrier_input_mobile').val() );//自提者电话
            if (goods == '') {
	  ...
	  <?php  } ?>
		...
		//alert(create_json.status);
		//alert(create_json.result);
		//return;
