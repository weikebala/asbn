...
<div class="cart_top">
    ...
    <div class="cart_main">
          ...
          <%each list as value1%>
        <div class="title" style="color:#666;padding-left:15px;background-color:#e8e8e8;height:40px;line-height: 40px; font-weight: bold; "> <%value1.name%>(<%value1.count%>)</div>
            <%each value1.list as value%>
                 <div class="cart_good"
                     ...
                     <div class="info">
                         <div class='inner' >
                          <div class="name" onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%value.goodsid%>'"><%value.title%></div>
                           <div class='name' style="heightL20px;line-height:20px;">税率:<%value.customs%>%</div>
                            <%if value.optionid!='0'%>
                             <div class="optionsel">
                                <div class='content'>
		     	...
		     	$("#option_stock").html(stock);	
                        ...
                        $('.paysub').click(function(){
                          ...
                          var ids = [];
                            ...
                            var stockModelNum=1;
                          $('.cart_good[sel=1]').each(function(){
                              if(stockModelStr=="")stockModelStr=$(this).data("stockmodel");
                              if(stockModelStr!=""&&$(this).data("stockmodel")!=stockModelStr){
                                  stockModelNum+=1;
                              }
                               ids.push($(this).data('cartid'));
                              stockModels.push($(this).data("stockmodel"));
                          });
                          if(stockModelNum>1){
                              core.tip.show('一个订单只能选择同仓库的商品！');return;
                          }
                          location.href = core.getUrl('order/confirm',{cartids:ids.join(','),stockModel:stockModelStr});
                        })
