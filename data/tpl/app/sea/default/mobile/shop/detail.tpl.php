...
.good_topbar .right { float:right; width:50px;height:30px;margin-left:10px;margin-top:5px;}
...
<script id='goods_info'  type='text/html'>
    ...
    <div class="good_choose">
        <div class="info">
             ...
             <div class="right">
                   ...
                   <div class="stock">库存:<span id='option_stock'><%goods.total%></span>件</div>
    ...
    <div class="good_info1">
      ...
      <div class="other1">
        ...
        <%/if%>
		<!--满额打折优惠信息-->
		<%if saleset && saleset.enoughszk.length>0%>
			<div class="other1">
				<div class='left'>
					<span><i class='fa fa-birthday-cake'></i> 单笔订单
				</div>
				<div class='right'>
					<div class='inner'> 
						<%each saleset.enoughszk as e i%><%if i<=1%>满 <span style='color:#ff6600'><%e.enoughzk%></span> 元立享 <span style='color:#ff6600'><%e.givezk*10%></span> 折优惠</span>;<br/><%/if%><%/each%>
						<%if saleset.enoughszk.length>2%><span id='morebtn' onclick='showMoreActivity()'><i class='fa fa-angle-down'></i> 查看更多优惠<br/></span><%/if%>
						<div id='moreactivity' style='display:none'>
						<%each saleset.enoughszk as e i%><%if i>1%>满 <span style='color:#ff6600'><%e.enoughzk%></span> 元立享 <span style='color:#ff6600'><%e.givezk*10%></span> 折优惠</span>;<br/><%/if%><%/each%>
						</div>
					</div>
   ...
   <div id="notbuy" class="good_speci" style="display:none"></div>
    ...
    <?php  if($warmdata) { ?>
        <div class="good_param">
            <div class="pro-info-ts">
                <span class="proxx-rt" style="color:#999;margin-left: 5px;width: 93%"><?php  echo $warmdata;?></span>
            </div>
        </div>
    <?php  } ?>

    <?php  if($goods['content_text'] && $goods['jiesao_thumb']) { ?>
    <div class="good_content_text" style="height: 100px;padding-top: 10px">
        <img style="width: 90px;height:90px;float: left;margin-left: 5px" src="/attachment/<?php  echo $goods['jiesao_thumb'];?>">
        <div class="content_text" style=" color:#666;float: left;width: 65%;height: 90px; padding-left: 5px;overflow: hidden;text-overflow: ellipsis;padding:2px;font-size: 12px;text-indent:2em">
...
<script id='tpl_recommand' type='text/html'>
    ...
    <div class="good" data-goodsid='<%g.id%>'>
        <img src='<%g.thumb%>'/>
        <div class="name" style="font-size:13px"><%g.title%></div>
        <div class="price">￥<%g.marketprice%> <%if g.productprice>0 && g.marketprice!=g.productprice%><span>￥<%g.productprice%></span><%/if%></div>
    </div>
    <%/each%>
</script>
