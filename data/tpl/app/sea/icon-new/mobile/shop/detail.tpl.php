...
<style type="text/css">
    ...
    .good_info2 {height:auto; width:100%; background:#fff; margin-top:14px;margin-bottom: 50px; }
    ...
    .content_text{font-size:14px;text-indent: 2em}
    ...
    .good_bottom .cs{height:42px; width:15%; background:#fdfdfd; float:left; padding-top:7px; border-top:1px solid #e1e1e1; text-align:center; font-size:20px; color:#666;line-height:10px; position:relative;}
    ...
    div.flicking_con a{position:relative; width:9px;height:9px;background:url('../addons/sea/template/mobile/default/static/images/dot.png') 0 0 no-repeat;display:inline-block;text-indent:-1000px}
    ...
    #gysgoods  {height:auto; width:100%; background:#fff; overflow:hidden;float:left; color:#333}
    #gysgoods .good {height:auto; width:46%; padding:0px 2% 10px; float:left;}
    #gysgoods .good img {height:120px; width:100%;}
    #gysgoods .good .name {height:20px; width:100%; font-size:15px; line-height:20px; color:#666; overflow:hidden;}
    #gysgoods .good .price {height:20px; width:100%; color:#f03; font-size:14px;}
    #gysgoods .good .price span {color:#aaa; font-size:12px; text-decoration:line-through;}
...
<script id='goods_info'  type='text/html'>
    ...
    <div class="good_choose">
        <div class="info">
            ...
            <div class="right">
                ...
                <div class="stock">库存:<span id='option_stock'><%goods.total%></span>件&nbsp;&nbsp;税率:<span id='option_rate'>11.9</span>% </div>
        ...
        <div class="other">
            ...
            <?php  } ?>
            <style>
                .good_choose .other .warehouse  button { padding:5px; border:1px #ccc solid; margin:5px; color:#333; float:left; background: #eee;-webkit-appearance: none;  }
            </style>
            <div class='warehouse'>
                <div class='label'>发货仓</div>
                <div>
                    <button class="stock_select" id='sea' onclick="javascript:stockSelect(this);" stock="<%goods.stock_sea%>" rate="<%goods.sea_rate%>" isselect="false">海外直邮</button>&nbsp;&nbsp;
                    <button class="stock_select" id='bonded' onclick="javascript:stockSelect(this);" stock="<%goods.stock_bonded%>"  rate="<%goods.bonded_rate%>" isselect="false">进口保税</button>&nbsp;&nbsp;
                    <button class="stock_select" id='ordinary' onclick="javascript:stockSelect(this);" stock="<%goods.stock_ordinary%>"  rate="<%goods.ordinary_rate%>" isselect="false">贸易进口</button>&nbsp;&nbsp;
                </div>
            </div>
    ...
    <div class="good_info1">
        ...
        <div class="price" id="">税率:<%goods.customs%>%</div>
    ...
    <div id="gysgoods">
        ...
        <div class="good" data-goodsid='<%g.id%>'>
            <img src='/attachment/<%g.thumb%>' style="height: 331px;"/>
    ...
    <?php  if($goods['content_text'] && $goods['jiesao_thumb']) { ?>
    <div class="good_content_text">
        <img style="width: 20%;height:90px;float: left;" src="/attachment/<?php  echo $goods['jiesao_thumb'];?>">
        <div class="content_text" style=" color:#666;float: left;width: 78%;height: 95px;overflow: hidden;text-overflow: ellipsis;padding:2px;font-size: 12px;text-indent:2em">
            <?php  echo $goods['content_text'];?>
        </div>
    </div>
    <?php  } ?>
    <!-- update 11-25 -->
    <style type="text/css">
        ...
        #recommand_container .good{width: 20%;}
        #recommand_container .good img{
            height: 72px !important;;
        }
        #recommand_container .good .price{
            height: 26px;
        }
    </style>
    <%if params.length>0%>
    <div class="good_param">
        ...
        <div class="pro-info-ts"><span class="proxx-left">【<%value.title%>】:</span>
            <span class="proxx-rt"><%value.value%></span>
        </div>
        <%/each%>
    </div>
    <%/if%>
    <!-- end update 11-25 -->
    <!--update 11-29-->
    <div class="good_param" id="goog_recom_div" style="display: none">
        <p style="font-size:14px;">&nbsp;&nbsp;推荐商品</p>
    ...
    <%/if%>
    <div class="buy" onclick="choose('buy')" <%if shopset.cservice &&  goods.canaddcart%>style="width:27%"<%/if%>>立即购买</div>
    <%else%>
    <%if goods.canaddcart%>
    <div class="cart" onclick="location.href='<?php  echo $this->createMobileUrl('shop/cart')?>'"><i class="fa fa-shopping-cart"></i><br><span>购物车</span>
        <%if cartcount>0%><b><%cartcount%></b><%/if%>
        ...
        core.json('shop/detail',{id:'<?php  echo $_GPC['id'];?>',mid:"<?php  echo $mid;?>"},function(json){
            ...
            <?php  if(!empty($formInfo)) { ?>
                ...
                <?php  } ?>
                    ...
                    core.json('shop/util',{op:'recommand',goodsid:'<?php  echo $_GPC['id'];?>'},function(json){
                        if(json.result.list.length<=0){
                            $('#goog_recom_div').hide();
                            return;
                        }
...
<script type="text/javascript">
    ...
    function choose(act){
        ...
        if( action=='cart'||action=='buy'){
            ...
            function stockSelect(obj){
                rate=$(obj).attr("rate");
                stock=$(obj).attr("stock");
                $(".stock_select").attr("isselect","false");
                $(".stock_select").css("border","1px #ccc solid");
                $(obj).attr("isselect","true");
                $(obj).css("border",'#ff771b 1px solid')
                $("#option_stock").text(stock);
                $("#option_rate").text(rate);
            };
            function choose2(direct,close){
                require(['core'],function(core){
                    ...
                    if($(".stock_select").size()>0){
                        if($(".stock_select[isselect='true']").size()>0){
                            stock=$(".stock_select[isselect='true']").attr('stock');
                            if(stock<$('#total').val()){
                                core.tip.show('库存不足') ;
                                return;
                            }
                    ...
                    if(action=='cart'){
                        var data = {};
                        stockModel=$(".stock_select[isselect='true']").attr("id");
                        stockModel= $.inArray(stockModel,['sea','bonded','ordinary'])>-1?stockModel:"ordinary";
                        <?php  if(!empty($formInfo)) { ?>
                            ...
                            <?php  } ?>
                                ...
                                core.json('shop/cart',{op:'add', id:'<?php  echo $_GPC['id'];?>',optionid:$('#optionid').val(),total:$('#total').val(),stockModel:stockModel,diyformdata:data},function(ret){
                                    if(ret.status==1){
                                        core.tip.show(ret.result.message);
                                        var cartdiv = $('.cart');
                                        if( cartdiv.find('b').length<=0){
                                ...
                                <?php  } else { ?>
                                location.href = "<?php  echo $this->createMobileUrl('member/login')?>&id=<?php  echo $_GPC['id'];?>&optionid=" +$("#optionid").val() + "&total=" + $("#total").val()+ "&stockModel=" + stockModel;
                                <?php  } ?>
                                    //end openid
                                ...
                                <?php  if(!empty($formInfo)) { ?>
                                    ...
                                    require(['core'],function(core){
                                        core.json('shop/detail', data, function(create_json) {
                                            if (create_json.status == 1) {
                                                location.href = "<?php  echo $this->createMobileUrl('order/confirm')?>&id=<?php  echo $_GPC['id'];?>&optionid=" +$("#optionid").val() + "&total=" + $("#total").val()+ "&gdid="+create_json.result.goods_data_id+ "&stockModel=" + stockModel;;
                                            } else {
                                                core.tip.show("提交失败!")
                                            }
                                    ...
                                    <?php  } else { ?>
                                    location.href = "<?php  echo $this->createMobileUrl('order/confirm')?>&id=<?php  echo $_GPC['id'];?>&optionid=" +$("#optionid").val() + "&total=" + $("#total").val()+ "&stockModel=" + stockModel;;
                                    <?php  } ?>
