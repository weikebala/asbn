...
<?php  if($do == 'category') { ?>
<div class="clearfix">
	<form action="<?php  echo url('article/product/category');?>" method="post" class="form-horizontal" role="form">
		<div class="form-group">
			<div class="col-sm-8 col-lg-9 col-xs-12">
				<a href="<?php  echo url('article/product/category_post');?>" target="_blank" class="btn btn-success col-lg-1">添加分类</a>
			</div>
		</div>
				...
				<table class="table table-hover">
					...
					<?php  if(is_array($data)) { foreach($data as $da) { ?>
						...
						<tr>
							...
							<td class="text-right">
								<a href="<?php  echo url('article/product/category_del', array('id' => $da['id']));?>" class="btn btn-default" onclick="if(!confirm('删除分类后，该分类下产品也会被删除，确定删除吗？')) return false;">删除</a>
							</td>
						</tr>
					<?php  } } ?>
					</tbody>
...
<div class="clearfix">
	<form action="<?php  echo url('article/product/category_post');?>" method="post" class="form-horizontal" role="form">
		<div class="panel panel-default">
			<div class="panel-heading">产品分类</div>
