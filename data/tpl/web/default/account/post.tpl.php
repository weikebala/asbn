...
<div class="clearfix">
	<form action="" method="post"  class="form-horizontal" role="form" enctype="multipart/form-data" id="form1">
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<p class="form-control-static">在微信公众号请求用户网页授权之前，开发者需要先到公众平台网站的【开发者中心】<b>网页服务</b>中配置授权回调域名。<?php  if($_W['isfounder']) { ?><a href="http://www.iseasoft.cn//" target="_black">查看详情</a><?php  } ?></p>
			</div>
		</div>
		<?php  if(!empty($acid)) { ?>
			<div class="form-group">
	...
	<?php  if($_W['isfounder']) { ?>
		...
		<div class="form-group">
			...
			<div class="col-sm-9 col-xs-12">
				<?php  echo tpl_edit_sms('balance', $notify['sms']['balance'], $account['uniacid'], url('account/post/edit_sms'))?>
			</div>
		</div>
		<div class="form-group">
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">短信签名</label>
			<div class="col-sm-9 col-xs-12">
				<input type="text" name="signature" class="form-control" value="<?php  echo $notify['sms']['signature'];?>" />
