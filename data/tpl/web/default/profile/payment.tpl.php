...
<script type="text/javascript">
	...
	require(['angular'], function(angular){
		angular.module('app', []).controller('paySetting', function($scope){
            ...
            $scope.abcchinapay = {
                switch: <?php  if($pay['abcchinapay']['switch']) { ?>'true'<?php  } else { ?>'false'<?php  } ?>
            };
			$scope.baifubao = {
		...
		$("#payform").submit(function(){
			...
			if($scope.alipay.switch == 'true') {
				...
				if($.trim($(':text[name="alipay[bilv]"]').val()) == '') {
					util.message('必须输入澳大利亚币对人民币汇率.', '', 'error');
					return false;
				}
...
<div class="main">
	<form id="payform" action="<?php  echo url('profile/payment')?>" enctype="multipart/form-data" method="post" class="form-horizontal form" onsubmit="return validate();" ng-controller="paySetting">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div ng-show="alipay.switch == 'true'">
					...
					<div class="form-group">
						<label class="col-xs-12 col-sm-3 col-md-2 control-label">澳大利亚币对人民币比率</label>
						<div class="col-sm-9 col-xs-12">
							<input type="text" name="alipay[bilv]" class="form-control" value="<?php  echo $pay['alipay']['bilv'];?>" autocomplete="off"/>
							<span class="help-block">国际支付宝收款货币为澳大利亚币，此处输入澳大利亚币对人民币比率</span>
						</div>
					</div>
					<div class="form-group">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div class="form-group">
                    <label class="col-xs-12 col-sm-3 col-md-2 control-label">农行支付</label>
                    <div class="col-sm-9 col-xs-12">
                        <label class="radio-inline">
                            <input type="radio" name="abcchinapay[switch]" <?php  if($pay['abcchinapay']['switch']) { ?>checked=checked<?php  } ?> ng-model="abcchinapay.switch" value="true"/>
                            开启
                        </label>
                        <label class="radio-inline">
                            <input type="radio" name="abcchinapay[switch]" <?php  if(!$pay['abcchinapay']['switch']) { ?>checked=checked<?php  } ?> ng-model="abcchinapay.switch" value="false"/>
                            关闭
                        </label>
                    </div>
                </div>
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div ng-show="wechat.switch == 'true'">
					...
					<div class="form-group s" id="apikey" <?php  if($pay['wechat']['version'] == 1) { ?> style="display:none;" <?php  } ?>>
						...
						<div class="col-sm-9 col-xs-12">
							...
							<span class="help-block">此值需要手动在腾讯商户后台API密钥保持一致。<?php  if($_W['isfounder']) { ?><a href="http://www.iseasoft.cn/thread-5788-1-1.html" target="_blank">查看设置教程</a><?php  } ?></span>
						</div>
					</div>
				</div>
				<?php  } else { ?>
				<div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						<div class="alert alert-warning">
							你必须向微信公众平台提交企业信息以及银行账户资料，审核通过并签约后才能使用微信支付功能 <a href="http://www.iseasoft.cn/thread-1354-1-1.html" target="_blank">申请及详情请查看这里</a>.
						</div>
