            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:left;">
                        <?php if(cv('shop.adpc.view|shop.adpc.edit')) { ?><a href="<?php  echo $this->createWebUrl('shop/adpc', array('op' => 'post', 'id' => $row['id']))?>" class="btn btn-default btn-sm" 
                                                               title="<?php if(cv('shop.adpc.edit')) { ?>修改<?php  } else { ?>查看<?php  } ?>"><i class="fa fa-edit"></i></a><?php  } ?>
                        <?php if(cv('shop.adpc.delete')) { ?><a href="<?php  echo $this->createWebUrl('shop/adpc', array('op' => 'delete', 'id' => $row['id']))?>"class="btn btn-default btn-sm" onclick="return confirm('确认删除此幻灯片?')" title="删除"><i class="fa fa-times"></i></a><?php  } ?>
                    </td>
                </tr>
                <?php  } } ?> 
                <tr>
                    <td colspan='6'>
                        <?php if(cv('shop.adpc.add')) { ?>
                          <a class='btn btn-default' href="<?php  echo $this->createWebUrl('shop/adpc',array('op'=>'post'))?>"><i class='fa fa-plus'></i> 添加广告</a>
...
<div class="main">
    <form <?php if( ce('shop.adpc' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form" enctype="multipart/form-data" onsubmit='return formcheck()'>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <select class="form-control" name="location">
                            <?php  if(is_array($location)) { foreach($location as $k => $v) { ?>
                            <option value="<?php  echo $k;?>"<?php  if($item['location']==$k) { ?> selected=""<?php  } ?>><?php  echo $v;?></option>
                            <?php  } } ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                           <?php if( ce('shop.adpc' ,$item) ) { ?>
                            ...
                            <input type="hidden" name="token" value="<?php  echo $_W['token'];?>" />
                        <?php  } ?>
                       <input type="button" name="back" onclick='history.back()' <?php if(cv('shop.adpc.add|shop.adpc.edit')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default" />
                    </div>
            </div>
