...
<div class="main">
    <form  <?php if( ce('shop.brand' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form" enctype="multipart/form-data" >
        ...
        <div class="panel panel-default">
            ...
            <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        ...
                        <?php  } ?>
                       <input type="button" name="back" onclick='history.back()' <?php if(cv('shop.brand.add|shop.brand.edit')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default col-lg-1" />
                    </div>
            </div>
                               ...
                               <?php  if(is_array($category)) { foreach($category as $row) { ?> 
                                ...
                                <li class="dd-item" data-id="<?php  echo $row['id'];?>">
                                    ...
                                    <div class="dd-handle"  style='width:100%;'>
                                        [ID: <?php  echo $row['id'];?>] <?php  echo $row['name'];?> <img style=" width:20px;" src="/attachment/<?php  echo $row['thumb'];?> ">
                                        
                                        <span class="pull-right">
                                           <?php if(cv('shop.brand.edit|shop.brand.view')) { ?>
                                             <a class='btn btn-default btn-sm' href="<?php  echo $this->createWebUrl('shop/brand', array('id' => $row['id'], 'op' => 'post'))?>" title="<?php if(cv('shop.brand.edit')) { ?>修改<?php  } else { ?>查看<?php  } ?>" ><i class="fa fa-edit"></i></a>
                                            <?php  } ?>
                                            <?php if(cv('shop.brand.delete')) { ?><a class='btn btn-default btn-sm' href="<?php  echo $this->createWebUrl('shop/brand', array('id' => $row['id'], 'op' => 'delete'))?>" title='删除' onclick="return confirm('确认删除此品牌吗？');return false;"><i class="fa fa-remove"></i></a><?php  } ?>
