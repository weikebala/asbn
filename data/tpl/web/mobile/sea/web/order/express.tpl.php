...
<?php  if(empty($arr['Traces'])) { ?>
        <p>未查询到物流信息</p>
<?php  } else { ?>
  <div class="list-main">
            <?php  if(is_array($arr['Traces'])) { foreach($arr['Traces'] as $index => $row) { ?>
             <div class="list">
                 <div class="info <?php  if($index==0) { ?>infoon<?php  } ?>" <?php  if($index==0) { ?>style='border:none'<?php  } ?>>
                     <div class='step'><?php  echo $row['AcceptStation'];?></div>
                     <div class='time'><?php  echo $row['AcceptTime'];?></div>
                 </div>
