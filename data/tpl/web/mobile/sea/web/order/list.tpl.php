    ...
    <div class="panel-body">
        <form action="./index.php" method="get" class="form-horizontal" role="form" id="form1">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-3">
                    <div class='input-group'>
                        <div class='input-group-addon'>加盟订单级数</div>
                    ...
                    <td colspan='8'  style='border-bottom:1px solid #ccc;background:#efefef;' > 
                        ...
                        <b>下单时间:  </b><?php  echo date('Y-m-d H:i:s', $item['createtime'])?>
                         <?php  if(!empty($item['refundid'])) { ?>
						  <label class='label label-danger'><?php  if($item['rtype']==2) { ?> 换货 <?php  } ?><?php  if($item['rtype']==1) { ?> 退货退款 <?php  } ?><?php  if($item['rtype']==0) { ?> 退款 <?php  } ?>申请</label><?php  } ?>
							<?php  if($item['rstatus'] == 4) { ?><label class='label label-primary'>客户已经寄出快递</label><?php  } ?>
						 <?php  if(!empty($agentid)) { ?>
						 <b>加盟订单级别:</b> <?php  echo $item['level'];?>级 <b>加盟佣金:</b> <?php  echo $item['commission'];?> 元
						 ...
						 <?php  } ?>
						<span class="label label-info" style=" background-color:#42989a;"> <?php  if(!empty($item['username'])) { ?><b>供货商:</b><?php  echo $item['username'];?><?php  } else { ?>总店<?php  } ?> </span>
                    <td style='border-bottom:1px solid #ccc;background:#efefef;text-align: center' >
                          <?php  if(empty($item['statusvalue'])) { ?>
                           <?php if(cv('order.op.close')) { ?>
                            ...
                            <?php  } ?>
                            <?php  } ?>
                        <?php  if($item['statusvalue']==1) { ?>
                        <?php if(cv('order.op.close')) { ?>
                        <a class="btn btn-default btn-sm" style="margin-top: 10px" href="javascript:;" data-item="<?php  echo $item['id'];?>" onclick="closeOrder(this)" data-toggle="modal" data-target="#modal-close">关闭订单</a>
                        <?php  } ?>
                        <?php  } ?>
                    </td>
		...
		<br/>数量: <?php  echo $g['total'];?>			 
                    ...
                    <td  rowspan="<?php  echo count($item['goods'])?>" style='width:200px;'>
						<table style='width:100%;'>
                            ...
                            <?php  } ?>
                            <?php  if($item['deductenoughzk']>0) { ?>
                            <tr>
                                <td  style='border:none;text-align:right;'>满额折扣：</td>
                                <td  style='border:none;text-align:right;;'>-￥<?php  echo number_format( $item['deductenoughzk'],2)?></td>
                            </tr>
                            <?php  } ?>
							...
							<tr>
								<td  style='border:none;text-align:right;'>税：</td>
								<td  style='border:none;text-align:right;;'>￥<?php  echo number_format( $item['customs_price'],2)?></td>
							</tr>
    ...
    <div class="panel-body">
        <table class="table table-hover table-responsive">
            <thead class="navbar-inner" >
                <tr>
                    <th style='width:50px;'>供应商ID</th>
                    <th style='width:200px;'>提现单号</th>
                    <th style='width:150px;'>开户名</th>
