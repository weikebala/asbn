...
<div class="main">
    <form action="" method="post" class="form-horizontal form" enctype="multipart/form-data">
        ...
        <div class='panel panel-default'>
            <div class='panel-heading'><?php  if(empty($_GPC['id'])) { ?>新建<?php  } else { ?>编辑 (id:<?php  echo $_GPC['id'];?>)<?php  } ?> - 自定义表单模板 - 拖动可改变字段位置</div>
            <div class='panel-body'>
                ...
                <div class="alert alert-info">警告：当模板中已经添加数据后改变模板结构有可能导致无法使用！
                    ...
                    <br/>加盟商申请资料 (<?php  if($use_flag2) { ?>正在使用<?php  } else { ?>未使用<?php  } ?>)
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12" style="width:707px;">
                        <?php if( ce('diyform.temp' ,$item) ) { ?>
                        <input type="text" name="tp_title" class="form-control tp_title" value="<?php  echo $item['title'];?>" placeholder="模版名称，例：用户资料" />
                        <?php  } else { ?>
                ...
                <div id="type-items">
                    ...
                    <?php  $kw=0;?>
                    <?php  if(is_array($item['fields'])) { foreach($item['fields'] as $k1 => $v1) { ?>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9 col-xs-12">
                    ...
                    <?php  } ?>
                    <a href="<?php  echo $this->createPluginWebUrl('diyform')?>"  <?php if( ce('diyform.temp' ,$item) ) { ?>style='margin-left:10px;'<?php  } ?>><span class="btn btn-default" style='margin-left:10px;'>返回列表</span></a>
                </div>
            </div>
        </div>
</div>
    ...
    <div class='panel-body'>
        ...
        <table class="table">
            ...
            <tr>
                ...
                <!--td>
                    <?php if(cv('diyform.data.view')) { ?>
                    <a href="<?php  echo $this->createPluginWebUrl('diyform/data', array('typeid'=>$item['id']))?>" title="点击查看/编辑"><?php  echo $item['count_num'];?> 详细</a>


                    <?php  } else { ?>
                    <?php  echo $item['count_num'];?>
                    <?php  } ?>
                </td-->
                <td>
                    <?php if(cv('diyform.temp.edit|diyform.temp.view')) { ?><a class='btn btn-default' href="<?php  echo $this->createPluginWebUrl('diyform/temp', array('op' => 'post', 'id' => $item['id']))?>"><i class='fa fa-edit'></i></a><?php  } ?>
                    <?php if(cv('diyform.data.view')) { ?><a class='btn btn-default' href="<?php  echo $this->createPluginWebUrl('diyform/data', array('typeid' => $item['id']))?>" title='查看已有数据'><i class='fa fa-list'></i></a><?php  } ?>
                    <?php if(cv('diyform.data.add')) { ?><!--a class='btn btn-primary' href="<?php  echo $this->createPluginWebUrl('diyform/data', array('op' => 'post', 'typeid' => $item['id']))?>" title='添加数据'><i class='fa fa-plus'></i></a--><?php  } ?>
                    <?php if(cv('diyform.temp.edit')) { ?><!--a class='btn btn-default'  href="<?php  echo $this->createPluginWebUrl('diyform/temp', array('op' => 'delete', 'id' => $item['id']))?>" onclick="return confirm('确认删除此模版吗？');return false;" title='删除模板'><i class='fa fa-remove'--></i></a><?php  } ?>
                    <?php if(cv('diyform.temp.edit')) { ?><a class='btn btn-default'  href="javascript:void(0)" onclick="del_temp(<?php  echo $item['id']?>)" title='删除模板'><i class='fa fa-remove'></i></a><?php  } ?>
                </td>
            </tr>
            <?php  } } ?>
            <?php  if(!empty($pager)) { ?>
        ...
        <table class="table">
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php if(cv('diyform.temp.edit|diyform.temp.view')) { ?><a class='btn btn-default' href="<?php  echo $this->createPluginWebUrl('diyform', array('op' => 'postdata', 'id' => $item['id'],'typeid'=>$item['typeid']))?>" title="<?php if(cv('diyform.temp.edit')) { ?>编辑<?php  } else { ?>查看<?php  } ?>"><i class='fa fa-edit'></i></a><?php  } ?>
                    <?php if(cv('diyform.temp.edit')) { ?><a class='btn btn-default'  href="<?php  echo $this->createPluginWebUrl('diyform', array('op' => 'deletedata','typeid'=>$item['typeid'],'id' => $item['id']))?>" onclick="return confirm('确认删除此条数据吗？');return false;" title='删除'><i class='fa fa-remove'></i></a><?php  } ?>
                    <?php  } else { ?>
