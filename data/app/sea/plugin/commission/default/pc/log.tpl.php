...
.log_list {height:40px; padding:10px; background:#fff;margin-top:5px;    border-bottom: 1px #eee solid;
...
<div class="log_top">
    ...
    <div class='num'>预计<?php  echo $this->set['texts']['commission']?>: +<span id='commissioncount'></span>元</div>
</div>
...
<script id='tpl_empty' type='text/html'>
    <div class="log_no"><i class="fa fa-file-text-o" style="font-size:100px;"></i><br><br><span style="line-height:18px; font-size:16px;">您还没有相关<?php  echo $this->set['texts']['commission_detail']?>~</span></div>
</script>
...
function bindScroller(){
        ...
        $(window).scroll(function () {
            ...
            if ($(document).height() <= totalheight) {
                ...
                if (stop == true) {
                    ...
                    page++;
                    core.pjson('commission/log', {status:current_status,page: page}, function (morejson) {
                        stop = true;
                        $('#log_loading').remove();
        ...
        function getLog(status) {
            ...
            core.pjson('commission/log', {status:status, page: page}, function (json) {
               ...
               $('#commissioncount').html( json.result.commissioncount);
                ...
                $('#container').html(tpl('tpl_log', json.result));
                 $('.log_list').unbind('click').click(function(){
                    ...
                    var url = core.getUrl('plugin/commission/log',{'op':'detail','id':applyid});
                    location.href = url;
                    return;
                 });
