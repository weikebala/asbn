...
<script id='tpl_main' type='text/html'>
    <div class="topbar">
        ...
        <div class="user_info">
            <div class="user_name" <%if set.levelurl!=''%>onclick='location.href="<%set.levelurl%>"'<%/if%>>
					<%if member.agentlevels==0%>
						<%member.nickname%> <span>[<%if level%><%level.levelname%><%else%>
						<%if set.levelname!=''%><%set.levelname%><%else%>普通等级<%/if%>
						<%/if%>] 	
					<%else%>
						<%member.nickname%> <span>[渠道商]
					<%/if%>
    ...
    <div class="top">
        <div class="top_1">
            <div class="text">
            <?php  echo $this->set['texts']['commission_total']?>：<%member.commission_total%> 元<br><?php  echo $this->set['texts']['commission_ok']?>（元）</div>
    ...
    <div class="menu">  
        ...
        <a href="<%member.sharesurl%>"><div class="nav nav2"><i class="fa fa-qrcode  fa-3x" style="color:#53bdec;"></i><div class="title">二维码</div><div class="con">推广二维码</div></div></a>
