...
.info_main .line {margin:0 10px; height:40px; border-bottom:1px solid #e8e8e8; line-height:40px; color:#666;}
.info_main .line2 { height:30px;line-height:30px;}
.info_main .line  span { color:#f30;}
...
<script id='tpl_main' type='text/html'>
	<div class="top">
        <img src="<%if set.regbg!=''%><%set.regbg%><%else%>../addons/sz_yi/plugin/commission/template/mobile/default/static/images/bg.png<%/if%>" />
        </div>
  <%if status==0%>
	<div class="info_main" style='margin-bottom: 5px;'>
	         <div class="line line2"  style='border:none' >
			 欢迎加入<span><%shop_set.name%></span>，请填写申请信息
		   </div>
		 <div class="line  line2" style='border:none' >
			 邀请人：<span><%if agent%><%agent.nickname%><%else%>总店<%/if%></span> (请核对)
		   </div>
	</div>
		 ...
		 return;
            ...
            <?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('diyform/common_js', TEMPLATE_INCLUDEPATH)) : (include template('diyform/common_js', TEMPLATE_INCLUDEPATH));?>
                ...
                $('#submit').click(function(){
                   ...
                   core.pjson('commission/register',{memberdata:diydata},function(pjson){
