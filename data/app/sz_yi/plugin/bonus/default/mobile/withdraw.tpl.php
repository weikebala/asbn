...
.gold_top .num a {height:40px; width:auto; background:url(../addons/sz_yi/plugin/commission/template/mobile/default/static/images/gold_ico1.png) right 16px no-repeat; float:right; padding-right:26px; line-height:50px; color:#fff; font-size:16px;}
...
<script id='tpl_main' type='text/html'>
    ...
    <div class="gold_num" style="border:0px;height:70px">
        ...
        <div class="nav" style="border:0px; width:44%;"><div class="title">待<?php  echo $this->set['texts']['commission']?></div><div class="num"><%member.commission_ok%></div><div class='tip'><?php  echo $this->set['texts']['commission_apply']?></div></div>
    </div>
    <div class="gold_num" style="border:0px;height:70px">
        <div class="nav"><div class="title">已<?php  echo $this->set['texts']['commission']?></div><div class="num"><%member.commission_pay%></div><div class='tip'>已发放<?php  echo $this->set['texts']['commission']?></div></div>
    ...
    <div class="gold_num">买家确认收货后，立即获得<?php  echo $this->set['texts']['commission1']?>。<%if set.settledays>0%>结算期（<%set.settledays%>天）后，<?php  echo $this->set['texts']['commission']?>可<?php  echo $this->set['texts']['withdraw']?>。结算期内，买家退货，<?php  echo $this->set['texts']['commission']?>将自动扣除。<%/if%>
        <%if set.consume_withdraw>0%><br/><br/>注意： 需自购订单完成，共计 <span style='color:red'><%set.consume_withdraw%></span> 元后才能得分红<br/><%/if%>
    </div> 
</div>
