...
<style type="text/css">
    ...
    .header .user .user-info {height:48px; width:122px; float:left; margin-left:14px; color:#fff;padding-top:5px;}
    .header .user .user-info .user-name {height:24px; width:100%; font-size:16px; line-height:24px;background: none !important;
    ...
    <div class="header">
        <div class="user">
            ...
            <div class="user-info">
                ...
                <div class="user-other" <?php  if(!empty($this->this->yzShopSet['levelurl'])) { ?>onclick='location.href="<?php  echo $this->this->yzShopSet['levelurl']?>"'<?php  } ?>>[<%level.levelname%>] <?php  if(!empty($this->this->yzShopSet['levelurl'])) { ?><i class='fa fa-question-circle' ></i><?php  } ?>
                </div>
            </div>
...
<div class="cart">
    ...
    <%/if%>
    <%if trade.withdraw || !trade.closerecharge%>
    <a href="<?php  echo $this->createMobileUrl('member/log')?>"><div class="list1" style="<?php  if(isset($set['trade']) && $set['trade']['withdraw']==1) { ?>margin:0px;<?php  } ?>"><i class="fa fa-file-text"></i><?php  if(isset($set['trade']) && $set['trade']['withdraw']==1) { ?>余额明细<?php  } else { ?>充值记录<?php  } ?>
